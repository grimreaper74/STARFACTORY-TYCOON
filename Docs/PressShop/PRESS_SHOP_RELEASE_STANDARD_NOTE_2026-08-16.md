# Press shop â€” release-standard implementation note, 2026-08-16
...
prepared everything and was never executed. The v449 complete runtime visual â€”
the accepted v438 Train A as one 306-section mesh with 13 pinned materials â€” is
...
Verified by registered world bounds: 13.6 Ã— 57.7 Ã— 9.4 m centred on the datum.
...
fixed âˆ’0.50 exposure bias on the management camera, department concrete aprons
...
100 required a plain static-mesh component â€” an ISM instance of it never
rendered.
...
â€” station, stage, cursor and mid-cycle progress â€” with one unit, no duplicate
...
| Section | Position |
|---|---|
| 1. Outcome and authority | **Pass** â€” press layout authority owns state; coordinator owns runtime; presentation holds none. |
| 2. Gameplay integration | **Partial** â€” flow, buffers, blocking and quality events work and are tested; unlock/price/refund progression is not wired into this shell. |
| 3. UX and feedback | **Partial** â€” keyboard reaches every action; HUD states distinct; controller input, inspector and multi-resolution captures open. |
| 4. Visual and asset quality | **Partial** â€” v449 is pinned to the accepted v438 look with authored materials; formal promotion to the owned `DetailedPresentation_v001` root (copy-and-rebind) remains, as does the grouped 337-component renderer for selection and animation seams. |
| 5. Save and reliability | **Pass in editor, packaged proof in flight** â€” exact round-trip shown above; packaged round-trip is the v004 verification. |
| 6. Localization | **Open** â€” HUD text is hard-coded English, as project-wide. |
| 7. Verification and performance | **Partial** â€” 275/275 suite green; packaged journey green in v003; performance budget not yet agreed or measured. |
...
the accepted look. The full internal revision â€” owned-root promotion, staged
`ConfigureFromLayout`, updated exact-pair contract and regenerated tests â€” is
the deliberate next step and should not be done quietly.

## Remaining before "release standard" can be claimed without caveats

1. ~~Packaged v004 round-trip evidence~~ — **closed**: the identical save/run/load round trip reproduced exactly in `PlayableShell_v004` (7/57 cycle 25% -> 15/57 -> 7/57 cycle 25%), and the packaged capture shows the v449 train standing in the bay (`../OneFactory/Captures/20260816_12_Packaged_v004_PressTrain.png`).
