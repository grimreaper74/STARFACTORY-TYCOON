...
| 7 | TRELLIS integration | **Wired as agent tools** (2026-08-31): `generate_3d_asset` (detached launch of the v002 lane - tool execution is synchronous on the game thread, so a 60-290 s generation must not block), `check_asset_job` (on-disk job state, 15 s bounded in-tool wait), `import_generated_asset` (Blender join+scale+ground via `Tools/trellis_prepare_import_v001.py`, FBX import with combine, Nanite, bounds verified vs declared size ±3 %, receipt under `Saved/Audits/Spacecraft/`). Executor `FAgentZetAssetGenerationActions`; all three names in the essential set and category map. Lane superseded v001→v002: v001 dies under redirected stderr (PS 5.1 wraps native stderr in NativeCommandError; with ErrorActionPreference=Stop the CLI's first progress message terminated the lane) - it validated interactively and failed on its first unattended call, exactly the class of bug this framework hunts. |
| 8 | Prove image→asset→Unreal automatically | In progress. Eval `step8a`: model behavior PERFECT (started job, polled, got the v001 lane failure, reported it honestly with the log tail - the honest-failure architecture worked on its first real exercise); generation itself failed on the v001 stderr bug above. `step8b` rerun with v002 pending. Foreground CLI repro on the same image: 60.5 s to GLB with editor AND Ollama both resident - VRAM triple-tenancy is fine at res 512. |
