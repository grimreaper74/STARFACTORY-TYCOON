...
| Weld shop (MIG, spot welding, panel picking) | **Running production loop with a versioned native presentation** | The runtime coordinator runs all 18 body/weld stations with a QA gate inside the 57-station route, proven in editor and in packaged builds. The frozen starter presentation is at **contract v002** (489 instances, 26 batches): robot pairs at the pack-validated ±300 cm contact positions with a tool on all 36 arms, authored framing/underbody fixtures, fail-closed visibility in builder and save preflight. The superseded MIG/spot-arm review rows refer to the retired `WeldRobotRuntime_v001` kit, which the v002 validator now explicitly forbids. See [the v002 contract](../OneFactory/ONE_FACTORY_BODY_WELD_STARTER_INTEGRATION_v002.md). |
