...
bool FLBInboundDeliveryContinuousCycleTest::RunTest(const FString& Parameters)
{
    ...
    Saved.SaveVersion = 6;
    Saved.TrailerRelativeYawDegrees = 999.0f;
    TestTrue(TEXT("Ordinary lorry still accepts a retained v6 save without hitch data"),
        Delivery->RestoreSaveState(Saved));
    ...
    for (int32 DeliveryIndex = 0; DeliveryIndex < 4; ++DeliveryIndex)
    {
            ...
            && Delivery->GetPhase() != ELBInboundDeliveryPhase::Fault; ++Step)
        {
            ...
            if (!bSaveChecked && Delivery->GetPhase() == ELBInboundDeliveryPhase::CoilLift)
            {
                ...
                TestEqual(TEXT("Visible unload save schema is v7"), MidUnload.SaveVersion, 7);
...
bool FLBPlayerBuiltModularInboundUnloadTest::RunTest(const FString& Parameters)
{
        ...
        && Delivery->GetPhase() != ELBInboundDeliveryPhase::Fault; ++Step)
    {
        ...
        if (!bVerifiedMidUnloadRestore && Delivery->GetPhase() == ELBInboundDeliveryPhase::CoilLift)
        {
            ...
            TestEqual(TEXT("Player-built unload save schema is v7"), MidUnload.SaveVersion, 7);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FLBInboundArticulatedCarrierControllerIntegrationTest,
    "LineBoss.FactoryBuilder.MaterialFlow.InboundArticulatedCarrierControllerIntegration",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLBInboundArticulatedCarrierControllerIntegrationTest::RunTest(
    const FString& Parameters)
{
    (void)Parameters;
    UWorld* World = UWorld::CreateWorld(EWorldType::Game, false,
        TEXT("LB_InboundArticulatedCarrierController"));
    TestNotNull(TEXT("Transient articulated inbound world exists"), World);
    if (!World) return false;
    ...
    AStaticMeshActor* AGVBody = SpawnMovable(FVector(20000.0f, 20000.0f, 29.0f));
    AStaticMeshActor* AGVDeck = SpawnMovable(FVector(20000.0f, 20000.0f, 64.0f));
    AStaticMeshActor* AGVLoad = SpawnMovable(FVector(20000.0f, 20000.0f, 156.0f));
    if (AGVBody) AGVBody->Tags.Add(TEXT("LB.Vehicle.CoilAGV"));
    if (AGVDeck) AGVDeck->Tags.Add(TEXT("LB.Vehicle.CoilAGV.LiftDeck"));
    if (AGVLoad) AGVLoad->Tags.Add(TEXT("LB.Inventory.InTransfer"));

    ALBCoilAGVController* AGV = World->SpawnActor<ALBCoilAGVController>();
    ALBFactoryBuildMachine* Inbound = World->SpawnActor<ALBFactoryBuildMachine>(
        ALBFactoryBuildMachine::StaticClass(), FTransform(FVector(25000.0f, 25000.0f, 0.0f)));
    ALBFactoryBuildMachine* PR002 = World->SpawnActor<ALBFactoryBuildMachine>(
        ALBFactoryBuildMachine::StaticClass(), FTransform(FVector(27000.0f, 25000.0f, 0.0f)));
    ALBPressShopStorageZone* Storage = World->SpawnActor<ALBPressShopStorageZone>(
        ALBPressShopStorageZone::StaticClass(), FTransform(FVector(30000.0f, 30000.0f, 0.0f)));
    ALBInboundDeliveryController* Delivery = World->SpawnActor<ALBInboundDeliveryController>();
    ALBInboundArticulatedCarrierActor* Carrier = World->SpawnActor<ALBInboundArticulatedCarrierActor>();
    ULBFactoryConnectionSubsystem* Connections = NewObject<ULBFactoryConnectionSubsystem>(World);

    TestTrue(TEXT("Articulated fixture AGV binds"), AGV && AGV->DiscoverAndBind());
    TestTrue(TEXT("Articulated fixture AGV route configures"), AGV && AGV->ConfigureRoute(
        FVector(20000.0f, 20000.0f, 29.0f), FVector(20300.0f, 20000.0f, 29.0f),
        FVector(20300.0f, 20300.0f, 29.0f)));
    TestTrue(TEXT("Articulated fixture inbound dock configures"), Inbound && Inbound->Configure(
        TEXT("INBOUND-ARTICULATED-001"), ELBFactoryBuildMachineType::InboundDeliveryDock));
    TestTrue(TEXT("Articulated fixture PR002 configures"), PR002 && PR002->Configure(
        TEXT("PR002-ARTICULATED-001"), ELBFactoryBuildMachineType::CoilWeighInspectionCell));
    TestTrue(TEXT("Articulated fixture storage configures"), Storage && Storage->Configure(
        TEXT("COIL-STORAGE-ARTICULATED-001"), ELBPressShopStorageType::BareCoils, 8,
        FVector(600.0f, 600.0f, 100.0f)));
    ALBFactoryTransportLink* Link = nullptr;
    FString Reason;
    TestTrue(TEXT("Articulated fixture process link connects"), Connections && Connections->Connect(
        Inbound->OutputPort, PR002->InputPort, Link, Reason));
    TestTrue(TEXT("Articulated delivery authority binds endpoints"), Delivery
        && Delivery->Configure(Inbound, PR002, AGV));

    AStaticMeshActor* Bridge = SpawnMovable(FVector(800.0f, 0.0f, 600.0f));
    AStaticMeshActor* Trolley = SpawnMovable(FVector(800.0f, 0.0f, 560.0f));
    AStaticMeshActor* Hoist = SpawnMovable(FVector(800.0f, 0.0f, 500.0f));
    AStaticMeshActor* Hook = SpawnMovable(FVector(800.0f, 0.0f, 450.0f));
    AStaticMeshActor* Saddle = SpawnMovable(FVector(2300.0f, 1100.0f, 100.0f));
    TArray<AActor*> TrailerCoils;
    for (int32 Index = 0; Index < 4; ++Index)
        TrailerCoils.Add(SpawnMovable(FVector(650.0f + Index * 100.0f, 0.0f, 100.0f)));
    const FVector DockPoint(1800.0f, 600.0f, 0.0f);
    TestTrue(TEXT("Native tractor/trailer binds through the retained visual API"), Delivery
        && Delivery->ConfigureVisualSequence(Carrier, Bridge, Trolley, Hoist, Hook,
            Saddle, TrailerCoils, FVector::ZeroVector, DockPoint));
    for (AActor* Coil : TrailerCoils)
        TestTrue(TEXT("Available wrapped coil is parented to the trailer cargo root"),
            Coil && Coil->GetAttachParentActor() == Carrier);

    TestTrue(TEXT("Articulated delivery reserves its first exact coil"), Delivery
        && Delivery->StartDelivery(TEXT("COIL-ARTICULATED-0001"), Reason));
    for (int32 Step = 0; Step < 12 && Delivery
        && Delivery->GetPhase() == ELBInboundDeliveryPhase::TruckReverse; ++Step)
    {
        Delivery->Tick(0.05f);
    }
    const FLBInboundDeliverySaveState MidRoute = Delivery->CaptureSaveState();
    TestEqual(TEXT("Articulated inbound persistence uses schema v7"), MidRoute.SaveVersion, 7);
    TestTrue(TEXT("Mid-route save retains a finite independent hitch angle"),
        FMath::IsFinite(MidRoute.TrailerRelativeYawDegrees));
    TestTrue(TEXT("Mid-route tractor has moved from the certified approach point"),
        FVector::Dist2D(MidRoute.LorryTransform.GetLocation(), FVector::ZeroVector) > 1.0f);

    FString CarrierReason;
    TestTrue(TEXT("Fixture can be displaced before persistence restore"), Carrier
        && Carrier->ResetStraightAtTractorPose(
            FTransform(FRotator::ZeroRotator, FVector(-500.0f, -500.0f, 0.0f)),
            CarrierReason));
    TestTrue(TEXT("v7 restores tractor pose, trailer yaw and active identity"), Delivery
        && Delivery->RestoreSaveState(MidRoute));
    TestTrue(TEXT("Restored tractor pose is exact"), Carrier
        && Carrier->GetActorTransform().Equals(MidRoute.LorryTransform, 0.01f));
    TestTrue(TEXT("Restored trailer yaw is exact"), Carrier
        && FMath::IsNearlyEqual(Carrier->GetTrailerRelativeYawDegrees(),
            MidRoute.TrailerRelativeYawDegrees, 0.01f));
    TestTrue(TEXT("Pre-lift active coil remains attached after restore"),
        TrailerCoils[0] && TrailerCoils[0]->GetAttachParentActor() == Carrier);

    for (int32 Step = 0; Step < 1200 && Delivery
        && Delivery->GetPhase() == ELBInboundDeliveryPhase::TruckReverse; ++Step)
    {
        Delivery->Tick(0.05f);
    }
    TestEqual(TEXT("Rounded articulated route reaches dock proving without fault"),
        Delivery ? Delivery->GetPhase() : ELBInboundDeliveryPhase::Fault,
        ELBInboundDeliveryPhase::DockProving);
    TestTrue(TEXT("Tractor reaches the exact dock datum"), Carrier
        && Carrier->GetActorLocation().Equals(DockPoint, 1.0f));
    for (int32 Step = 0; Step < 3000 && Delivery
        && Delivery->GetPhase() != ELBInboundDeliveryPhase::HookEngage
        && Delivery->GetPhase() != ELBInboundDeliveryPhase::Fault; ++Step)
    {
        Delivery->Tick(0.05f);
    }
    if (Delivery && Delivery->GetPhase() == ELBInboundDeliveryPhase::HookEngage)
        Delivery->Tick(0.05f);
    TestTrue(TEXT("Hook engagement detaches only the active coil with world pose preserved"),
        TrailerCoils[0] && TrailerCoils[0]->GetAttachParentActor() != Carrier);
    for (int32 Index = 1; Index < TrailerCoils.Num(); ++Index)
        TestTrue(TEXT("Remaining coils continue to ride on the articulated trailer"),
            TrailerCoils[Index] && TrailerCoils[Index]->GetAttachParentActor() == Carrier);
