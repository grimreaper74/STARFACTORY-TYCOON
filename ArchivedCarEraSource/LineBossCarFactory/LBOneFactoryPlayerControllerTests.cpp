...
namespace LBOneFactoryPlayerControllerTestsPrivate
{
    ...
    bool BuildAndCaptureEnvelope(const TCHAR* WorldName,
        const bool bRooflessPresentation, FEnvelopeCounts& OutCounts,
        FString& OutReason)
    {
        FPlayerWorld Fixture;
        if (!Fixture.Create(WorldName, OutReason))
        {
            Fixture.Destroy();
            return false;
        }

        if (bRooflessPresentation)
        {
            AActor* Marker = Fixture.World->SpawnActor<AActor>();
            if (!Marker)
            {
                OutReason = TEXT("ROOFLESS PRESENTATION MARKER FAILED TO SPAWN");
                Fixture.Destroy();
                return false;
            }
            Marker->Tags.AddUnique(
                TEXT("LB.PressShop.RooflessPresentation.v002"));
        }

        if (!Fixture.Controller->ActivatePrebuiltFactory(OutReason))
        {
            Fixture.Destroy();
            return false;
        }
        ...
        for (TActorIterator<ALBOneFactoryDevEnvelopeActor> It(Fixture.World);
            It; ++It)
        {
            if (IsValid(*It))
            {
        ...
        if (!Envelope || EnvelopeCount != 1)
        {
            OutReason = FString::Printf(
                TEXT("EXPECTED ONE RUNTIME ENVELOPE; FOUND %d"),
                EnvelopeCount);
            Fixture.Destroy();
            return false;
        }

        OutCounts.RoofDecks = Envelope->GetRoofDeckCount();
        OutCounts.Walls = Envelope->GetWallInstanceCount();
        OutCounts.Dado = Envelope->GetDadoInstanceCount();
        OutCounts.Clerestory = Envelope->GetClerestoryInstanceCount();
        OutCounts.SiteSlab = Envelope->GetSiteSlabInstanceCount();
        OutCounts.Pieces = Envelope->GetPieceCount();
        Fixture.Destroy();
        OutReason = TEXT("RUNTIME ENVELOPE COUNTS CAPTURED");
        return true;
    }
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FLBOneFactoryRooflessEnvelopePolicyTest,
    "LineBoss.OneFactory.ActualPlayer.RooflessPresentationSkipsOnlyRoofDecks",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLBOneFactoryRooflessEnvelopePolicyTest::RunTest(
    const FString& Parameters)
{
    (void)Parameters;
    using namespace LBOneFactoryPlayerControllerTestsPrivate;
    ...
    FString Reason;
    FEnvelopeCounts Legacy;
    if (!TestTrue(Reason, BuildAndCaptureEnvelope(
            TEXT("LBLegacyEnvelopePolicyWorld"), false, Legacy, Reason)))
    {
        return false;
    }

    FEnvelopeCounts Roofless;
    if (!TestTrue(Reason, BuildAndCaptureEnvelope(
            TEXT("LBRooflessEnvelopePolicyWorld"), true, Roofless, Reason)))
    {
        return false;
    }

    TestEqual(TEXT("legacy presentation retains one deck per department"),
        Legacy.RoofDecks, 4);
    TestEqual(TEXT("roofless presentation creates no runtime roof decks"),
        Roofless.RoofDecks, 0);
    TestTrue(TEXT("legacy envelope contains wall panels"), Legacy.Walls > 0);
    TestTrue(TEXT("legacy envelope contains clerestory strips"),
        Legacy.Clerestory > 0);
    TestEqual(TEXT("legacy envelope contains one shared site slab"),
        Legacy.SiteSlab, 1);
    TestEqual(TEXT("roofless policy leaves wall count unchanged"),
        Roofless.Walls, Legacy.Walls);
    TestEqual(TEXT("roofless policy leaves dado count unchanged"),
        Roofless.Dado, Legacy.Dado);
    TestEqual(TEXT("roofless policy leaves clerestory count unchanged"),
        Roofless.Clerestory, Legacy.Clerestory);
    TestEqual(TEXT("roofless policy leaves site slab count unchanged"),
        Roofless.SiteSlab, Legacy.SiteSlab);
    TestEqual(TEXT("piece-count delta is exactly the four omitted decks"),
        Roofless.Pieces + 4, Legacy.Pieces);
    return true;
}
