    ...
    RunTest(const FString& Parameters)
{
    ...
    TestEqual(TEXT("Body/Weld presentation commits all 29 HISM batches"),
        Presentation->GetVisualBatchCount(), 29);
    TestEqual(TEXT("Body/Weld presentation commits 597 native items"),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
    FLBOneFactoryLegacyPressPresentationIsolationTest,
    "LineBoss.OneFactory.PlayerBuilder.RetiresLegacyPressTrainBeforeNativePresentation",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FLBOneFactoryLegacyPressPresentationIsolationTest::RunTest(
    const FString& Parameters)
{
    using namespace LBOneFactoryPlayerBuilderTestsPrivate;
    FReadyFixture Fixture;
    if (!TestTrue(TEXT("OneFactory fixture is valid"),
            Fixture.Create(TEXT("LBOneFactoryLegacyPressIsolationTest"))))
    {
        Fixture.Destroy();
        return false;
    }

    ALBPressTrainAStation* LegacyTrain = Fixture.World->SpawnActor<
        ALBPressTrainAStation>(ALBPressTrainAStation::StaticClass(),
            FTransform(FVector(4000.0f, 4000.0f, 0.0f)));
    TestNotNull(TEXT("Legacy build-your-own press train is present before curation"),
        LegacyTrain);

    UStaticMesh* OverheadUnitPlane = LoadObject<UStaticMesh>(nullptr,
        TEXT("/Game/LineBoss/Candidates/PressShop/PressShop2126_OverheadAssets_v001/Geometry/SM_CA_MW_PS_OverheadUnitPlane_v001.SM_CA_MW_PS_OverheadUnitPlane_v001"));
    TestNotNull(TEXT("Guarded overhead runtime plane is available"),
        OverheadUnitPlane);
    ALBPressShopOverheadVisualLayerActor* CurrentOverheadLayer =
        Fixture.World->SpawnActor<ALBPressShopOverheadVisualLayerActor>();
    AStaticMeshActor* TaggedGenericCandidate =
        Fixture.World->SpawnActor<AStaticMeshActor>();
    TestNotNull(TEXT("Dedicated overhead runtime layer is present before curation"),
        CurrentOverheadLayer);
    TestNotNull(TEXT("Tagged generic candidate actor is present before curation"),
        TaggedGenericCandidate);
    if (OverheadUnitPlane && CurrentOverheadLayer && TaggedGenericCandidate)
    {
        CurrentOverheadLayer->GetStaticMeshComponent()->SetStaticMesh(
            OverheadUnitPlane);
        TaggedGenericCandidate->GetStaticMeshComponent()->SetStaticMesh(
            OverheadUnitPlane);
        TaggedGenericCandidate->Tags.AddUnique(
            ALBPressShopOverheadVisualLayerActor::GetLayerTag());
        TaggedGenericCandidate->Tags.AddUnique(TEXT("LB.Environment.VisualOnly"));
        TaggedGenericCandidate->Tags.AddUnique(TEXT("LB.NotProcessWIP"));
    }
    FString Reason;
    TestTrue(TEXT("Curated OneFactory press materialises"),
        Fixture.Builder->CreateNewFactory(Reason));
    TestTrue(TEXT("Legacy train is no longer live in the OneFactory route"),
        LegacyTrain && LegacyTrain->IsActorBeingDestroyed());
    TestEqual(TEXT("No legacy press train remains alongside native presentation"),
        CountLiveActors<ALBPressTrainAStation>(Fixture.World), 0);
    TestEqual(TEXT("Exactly one native press presentation remains"),
        CountLiveActors<ALBOneFactoryPressStarterPresentationActor>(
            Fixture.World), 1);
    TestTrue(TEXT("Dedicated overhead runtime layer survives legacy curation"),
        CurrentOverheadLayer && !CurrentOverheadLayer->IsActorBeingDestroyed());
    TestEqual(TEXT("Exactly one dedicated overhead runtime layer remains"),
        CountLiveActors<ALBPressShopOverheadVisualLayerActor>(Fixture.World), 1);
    TestTrue(TEXT("Generic candidate actor cannot bypass retirement with runtime tags"),
        TaggedGenericCandidate && TaggedGenericCandidate->IsActorBeingDestroyed());
    Fixture.Destroy();
    return !HasAnyErrors();
}
