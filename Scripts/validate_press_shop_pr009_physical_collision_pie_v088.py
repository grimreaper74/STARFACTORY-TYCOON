...
def finish(payload,failures):
    ...
    payload.update({"$schema":"cairnwell/audit/pr009-physical-collision-pie-v088/v1",
    ...
    unreal.log(f"PR009_V088_PHYSICAL_COLLISION_{'PASS' if not failures else 'FAIL'} output={out}")
...
def tick_impl(_delta):
    ...
    # Pro Sheet 02: 2600 mm along material flow x 1800 mm across strip.
    # Station yaw -90 maps those local axes to world X and world Y respectively.
    ...
    finish({"guard_perimeter_traces":guard_rows,"intended_material_aperture_traces":material_apertures,
            ...
            "transferred_blank_envelope_authority":{"measured_v088_carrier_half_extent_cm":measured_half,
...
def tick(_delta):
    try:
        ...
        unreal.log_error(f"PR009_V088_PHYSICAL_COLLISION_VALIDATOR_EXCEPTION: {exc!r}")
