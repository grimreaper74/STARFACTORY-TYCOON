"""Capture v319 after giving its 298 imported material shaders time to settle."""
import os,time
from pathlib import Path
import unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True)
MAP="/Game/LineBoss/Maps/LB_PressTrainA_FixedExposureReviewCandidate_v319";CAP={"operator":("LB_V309_CAM_Operator","v320_train_a_operator.png"),"rear":("LB_V309_CAM_Rear","v320_train_a_rear.png"),"elevated":("LB_V309_CAM_Elevated","v320_train_a_elevated.png")};cid=os.environ.get("LB_V320_CAPTURE","operator").lower();label,name=CAP[cid];out=Path(unreal.Paths.project_saved_dir())/"ValidationScreenshots/PressShopIntegration/v320_train_a_shader_settled_review"/name
...
task=unreal.AutomationLibrary.take_high_res_screenshot(1920,1080,str(out),camera=cam,mask_enabled=False,capture_hdr=False,comparison_tolerance=unreal.ComparisonTolerance.LOW,comparison_notes=f"Train A v320 shader-settled {cid}",delay=15,force_game_view=True)
if not task.is_valid_task():raise RuntimeError("invalid screenshot")
started=time.monotonic();handle=None
def done(_):
 global handle
 elapsed=time.monotonic()-started
 if elapsed>=16 and out.exists() and out.stat().st_size>=1024:unreal.log(f"LB_V320_CAPTURE_PASS id={cid} path={out}")
 elif elapsed<90:return
 else:unreal.log_error(f"LB_V320_CAPTURE_FAIL id={cid}")
