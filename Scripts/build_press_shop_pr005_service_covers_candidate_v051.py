"""Import and place the dimensioned removable PR-005 service covers v051."""
...
DEST = "/Game/LineBoss/Stations/Press/PR005/Candidate_v051/HydraulicRouting"
ASSET = DEST + "/SM_PR005_HydraulicServiceCovers_Candidate_v051"
FBX = Path(unreal.Paths.project_dir()) / (
    "SourceAssets/PR005/HydraulicRouting/Candidate_v051/"
    "SM_PR005_HydraulicServiceCovers_Candidate_v051.fbx")
OUT = Path(unreal.Paths.project_saved_dir()) / "Audits/press_shop_pr005_service_covers_candidate_v051.json"
PREFIX = "LB_PR005_V051_"
...
if any(value is None for value in materials.values()):
    raise RuntimeError("Missing retained v048/v050 service-cover materials")
...
task.set_editor_properties({
    ...
    "destination_name": "SM_PR005_HydraulicServiceCovers_Candidate_v051",
...
if not (75.0 <= bounds.x <= 80.0 and 353.0 <= bounds.y <= 357.0 and 13.5 <= bounds.z <= 15.5):
    raise RuntimeError(f"PR-005 service-cover bounds out of gate: {[bounds.x, bounds.y, bounds.z]}")
...
if len(bindings) != 3:
    raise RuntimeError(f"Expected three semantic cover bindings, found {len(bindings)}: {bindings}")
...
cover.set_actor_label(PREFIX + "HydraulicServiceCovers_Removable")
cover.tags = [
    unreal.Name("LB.Asset.Candidate.v051"), unreal.Name("LB.Asset.CandidateNotPromoted"),
    unreal.Name("LB.Service.HydraulicRouting.RemovableCovers"),
    unreal.Name("LB.Service.Access.FromAbove"), unreal.Name("LB.Module.StationLocal")]
...
def camera(label, location, target, fov, bias):
    ...
    actor.tags = [
        unreal.Name("LB.Camera.Validation"), unreal.Name("LB.Camera.Fixed.PR005.v051"),
        unreal.Name("LB.Asset.Candidate.v051"), unreal.Name("LB.Asset.CandidateNotPromoted")]
...
cameras = [
    camera("ServiceCoversPlayer", (-4660.0, -700.0, 255.0), (-3820.0, -1510.0, 45.0), 42.0, 0.06),
    camera("ServiceCoversElevated", (-4650.0, -850.0, 520.0), (-3820.0, -1510.0, 40.0), 46.0, 0.05),
    camera("ServiceCoversWholeLine", (-5200.0, -720.0, 700.0), (-3920.0, -1900.0, 110.0), 54.0, 0.06),
]
...
payload = {
    "$schema": "line-boss/audit/press-shop-pr005-service-covers-candidate-v051/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "DIMENSIONED_REMOVABLE_HYDRAULIC_SERVICE_COVERS_BUILT__FULL_RUNTIME_AND_VISUAL_GATES_REQUIRED__NOT_PROMOTED",
    "map": MAP, "base_map": "/Game/LineBoss/Maps/LB_PressShop_PR005ServiceRoutingCandidate_v050",
    "source_fbx": str(FBX), "asset": ASSET, "mesh_bounds_cm": [bounds.x, bounds.y, bounds.z],
    "panel_count": 5, "covered_run_dimensions_mm": [3480, 680],
    "flexible_end_zones_preserved": True, "material_bindings": bindings,
...
unreal.log(f"LINE_BOSS_PR005_SERVICE_COVERS_V051_BUILD_PASS bounds={payload['mesh_bounds_cm']} bindings={len(bindings)}")
