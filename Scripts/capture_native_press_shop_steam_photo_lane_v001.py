...
the Press cell.  It never saves a map or writes Content.  Every output is a
new 2560x1440 PNG and one evidence receipt under Saved/ValidationScreenshots.
"""
...
OUTPUT = ROOT / "Saved/ValidationScreenshots/OneFactory/NativePressShopSteamPhotoLane_v001"
RECEIPT = OUTPUT / "press_shop_steam_photo_lane_v001.json"
    ...
    try:
        ...
        if phase == "wait_screenshot":
            ...
            if shot_index < len(SHOTS):
                ...
                return
            finish({
                "$schema": "lineboss/evidence/onefactory/native-press-steam-photo-lane-v001/v1",
                "status": "PASS__NATIVE_PRESS_STEAM_PHOTO_PROBES",
                "real_rhi": True,
        ...
        finish({
            "$schema": "lineboss/evidence/onefactory/native-press-steam-photo-lane-v001/v1",
            "status": "FAIL__NATIVE_PRESS_STEAM_PHOTO_PROBES",
            "error": str(error),
            "captures": captures,
            "map_loaded_or_saved": [],
            "content_writes": [],
