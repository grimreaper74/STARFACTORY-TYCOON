...
param(
    ...
    [string]$OutputRoot = 'C:\Users\greg_\Projects\LineBossCarFactory_Unreal 5.8\SourceAssets\Candidate\Spacecraft\GroundDrones_v005',
    [int]$PollSeconds = 20,
    [int]$TimeoutMinutes = 25
)

$ErrorActionPreference = 'Stop'
if ($Acknowledgement -ne 'I ACCEPT MESHY CREDIT SPEND FOR GROUND DRONES V005') {
...
$Jobs = @(
    # v004 returned FOUR big truck wheels on all three: "eight-wheeled"
    # read as "wheeled". The fix is to make the wheel ROW the subject
    # and to say the wheels are small, twice - a generator will give
    # you four large wheels by default because most vehicles have them.
    @{ Name = 'Ground01_LifterDrone'
       Subject = 'A long low rover with a row of FOUR SMALL WHEELS ALONG EACH SIDE - eight small wheels in total, like a planetary rover - carrying a flat scissor-lift deck.'
       Anchor  = 'Scale: the wheels are small, no taller than a persons shin, and there are four of them in a line on each flank; the deck sits at knee height.' }
    @{ Name = 'Ground02_AssemblyDrone'
       Subject = 'A long low rover with THREE SMALL WHEELS ALONG EACH SIDE - six small wheels in total - carrying a folded articulated work arm.'
       Anchor  = 'Scale: small shin-high wheels, three in a line on each flank; the folded arm sits at knee height on the deck.' }
    @{ Name = 'Ground03_SprayerDrone'
       Subject = 'A long low rover with FOUR SMALL WHEELS ALONG EACH SIDE - eight small wheels in total - carrying two fluid tanks and a pair of spray booms.'
       Anchor  = 'Scale: small shin-high wheels, four in a line on each flank; the booms rise to waist height above a knee-high deck.' }
)
...
@{
    '$schema'      = 'lineboss/audit/meshy-ground-drones-v005/v1'
