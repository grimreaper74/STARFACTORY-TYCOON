"""Read-only PIE inventory of environment dressing around the native Press Shop."""
...
OUT = ROOT / "Saved/Audits/OneFactory/Press/PhotoEnvironment_v002/photo_environment_v002.json"
...
def environment_actor_row(actor):
    ...
    return {
        ...
        "components": [component_row(component) for component in
...
def finish(payload):
    ...
    try:
        ...
        unreal.log_warning("PRESS_PHOTO_ENVIRONMENT_V002_END_PIE_WARN " + str(error))
        ...
        if phase == "audit":
            ...
            for actor in unreal.GameplayStatics.get_all_actors_of_class(world, unreal.Actor):
                ...
                if (any("environment" in tag or "envelope" in tag for tag in tags)
                    ...
                    rows.append(environment_actor_row(actor))
            finish({
                "$schema": "lineboss/evidence/onefactory/press-photo-environment-v002/v1",
                "status": "PASS__PIE_ENVIRONMENT_DRESSING_INVENTORY",
                "environment_actor_count": len(rows),
                "environment_actors": sorted(rows, key=lambda row: row["label"]),
                "map_loaded_or_saved": [],
                "content_writes": [],
            })
    except Exception as error:
        unreal.log_error("PRESS_PHOTO_ENVIRONMENT_V002_FAIL " + str(error))
        finish({
            "$schema": "lineboss/evidence/onefactory/press-photo-environment-v002/v1",
...
try:
    ...
    unreal.log_error("PRESS_PHOTO_ENVIRONMENT_V002_START_FAIL " + str(error))
    finish({
        "$schema": "lineboss/evidence/onefactory/press-photo-environment-v002/v1",
        "status": "FAIL__PIE_ENVIRONMENT_DRESSING_INVENTORY",
        "error": str(error),
