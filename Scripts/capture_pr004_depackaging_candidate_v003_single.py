"""Request one deterministic PR-004 Candidate_v003 screenshot per session."""
...
MAP = "/Game/LineBoss/Developer/Validation/PR004/LB_PR004_Depackaging_Candidate_v003"
CAMERAS = {
...
OUT = REPO / "Saved/ValidationScreenshots/PR004/Candidate_v003"
MARKER = REPO / f"Saved/Audits/pr004_capture_marker_{CAPTURE_ID}_v003.json"
OUT.mkdir(parents=True, exist_ok=True)
output = OUT / f"pr004_candidate_v003_{CAPTURE_ID}_pbr.png"
if output.exists():
    output.unlink()
...
# The automation screenshot is latent. Keep Slate ticking until Unreal has
# actually written the file, then close this isolated capture session cleanly.
# A one-second latent delay lets the forced Lit mode reach the active viewport;
# requesting the latent task now also prevents -ExecutePythonScript from closing
# the editor before the delayed capture can run.
...
task = unreal.AutomationLibrary.take_high_res_screenshot(
    ...
    comparison_notes=f"Line Boss PR-004 Candidate_v003 PBR fixed review: {CAPTURE_ID}",
    delay=1.0,
...
MARKER.write_text(json.dumps({
    "$schema": "line-boss/audit/pr004-unreal-single-capture-marker-v003/v1",
...
unreal.log(f"LINE_BOSS_PR004_V003_SCREENSHOT_REQUESTED id={CAPTURE_ID} path={output}")
...
def finish_when_ready(_delta_seconds):
    ...
    if passed:
        unreal.log(f"LINE_BOSS_PR004_V003_SCREENSHOT_PASS id={CAPTURE_ID} path={output}")
    else:
        unreal.log_error(f"LINE_BOSS_PR004_V003_SCREENSHOT_FAIL id={CAPTURE_ID} path={output}")
