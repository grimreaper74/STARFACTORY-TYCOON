"""Finalize and sanity-check the stable PR-009 v096 map in a fresh editor process."""
...
DEST = "/Game/LineBoss/Maps/LB_PressShop_PR009Accepted_v096"
OUT = ROOT / "Saved/Audits/PR009_Accepted_v096/promotion_receipt.json"
...
for actor in actors:
    ...
    if actor.get_actor_label().startswith("LB_PR009_V096_"):
        for tag in ("LB.Asset.AcceptedBaseline", "LB.Asset.Accepted.PR009.v096"):
            if tag not in tags:
                tags.append(tag)
        changed += 1
...
if not levels.save_current_level():
    raise RuntimeError("Could not save accepted PR-009 v096 map")
...
modules = [actor for actor in actors if actor.get_actor_label().startswith("LB_PR009_V096_ENC_SM_CA_MW_ENC_PR009_")]
candidate_tags = sum(str(tag) == "LB.Asset.CandidateNotPromoted" for actor in actors for tag in actor.tags)
failures = []
...
receipt = {
    "$schema": "cairnwell/audit/pr009-accepted-baseline-v096/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "accepted_map": DEST,
    "status": "PASS__PR009_V096_ACCEPTED_BASELINE_CREATED" if not failures else "FAIL__ACCEPTED_MAP_SANITY",
...
unreal.log(f"LINE_BOSS_PR009_V096_ACCEPTED_FINAL {receipt['status']} {OUT}")
if failures:
