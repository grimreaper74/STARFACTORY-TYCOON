"""Build v025 by configuring persistent actors before adding the PR-004 sublevel."""
...
STAGE = "/Game/LineBoss/Maps/LB_PR004_CCTVStageCandidate_v005"
SOURCE_CONTROL = "/Game/LineBoss/Maps/LB_MainControlRoom_PR004ConsoleCandidate_v020"
MAP = "/Game/LineBoss/Maps/LB_MainControlRoom_PR004LiveCCTVCandidate_v025"
OUT = ROOT / "Saved/Audits/ControlRoom/main_control_room_pr004_live_cctv_build_v025.json"
...
# Isolated stage derivative: retain the actual PR-004 authority and visuals,
# remove only navigation helpers that are irrelevant to a remote camera world,
# and suppress global lighting/post-process that could leak into the room.
if not levels.new_level_from_template(STAGE, SOURCE_STAGE):
    raise RuntimeError(f"could not derive {STAGE}")
disabled_stage_actors = []
for actor in actors_api.get_all_level_actors():
    ...
    if isinstance(actor, unreal.NavMeshBoundsVolume) or actor.get_class().get_name() == "LBPressShopNavigationBootstrap":
        ...
        disabled_stage_actors.append(label)
        continue
...
if stage_station is None:
    failures.append("PR-004 authority station missing from stage")
levels.save_current_level()

# Build persistent room and configure all persistent actors before streaming.
...
if console is None:
    ...
    console.set_actor_label("LB_MCR_V025_PR004_AuthorityConsole")
    console.tags = [unreal.Name("LB.ControlRoom.v025"), unreal.Name("LB.PR004.AuthorityConsole"),
...
if feed_class is None:
    ...
    feed = actors_api.spawn_actor_from_class(
        feed_class, unreal.Vector(68.0, -312.5, 170.0), unreal.Rotator())
    if feed is None:
        failures.append("could not spawn selected CCTV feed actor")
    else:
        feed.set_actor_label("LB_MCR_V025_SELECTED_CCTV_PR004")
        feed.tags = [unreal.Name("LB.ControlRoom.v025"), unreal.Name("LB.CCTV.Selected.PR004"),
                     unreal.Name("LB.Asset.CandidateNotPromoted")]
...
streaming = unreal.EditorLevelUtils.add_level_to_world_with_transform(
    world, STAGE, unreal.LevelStreamingAlwaysLoaded, unreal.Transform(location=STAGE_OFFSET))
...
# Adding the sublevel changes the editor's current level; save the persistent
# control-room package explicitly so the feed actor and streaming descriptor stay there.
if not library.save_asset(MAP, only_if_is_dirty=False):
    failures.append("could not save persistent v025 control-room map")

payload = {
    "$schema": "cairnwell/audit/main-control-room-pr004-live-cctv-build-v025/v1",
    ...
    "disabled_stage_actors": disabled_stage_actors,
