"""One-shot UE 5.8 importer for the frozen original-procedural Assembly kit."""
...
PASS_STATUS = "PASS__HASH_GUARDED_FRESH_IMPORT__8_ASSETS__24_AUTHORED_LODS__ASSEMBLY_NATIVE_KIT_V001"
...
def append_custom_lods(meshes: dict, baseline: dict, subsystem) -> dict:
    ...
    evidence = {"name": lane.INTERCHANGE_FBX_CVAR, "previous_value": previous,
                ...
                "custom_lods_requested": 16, "custom_lods_imported": [],
...
def configure_mesh(key: str, spec: dict, baseline: dict, subsystem, materials: dict, mesh) -> None:
...
def main() -> None:
    ...
    record = {"$schema": "lineboss/audit/assembly-native-kit-v001/unreal-import/v1",
              "generated_utc": lane.now(), "process_id": __import__("os").getpid(),
    ...
    try:
        ...
        tasks = [make_task(baseline["assets"][key]) for key in sorted(baseline["assets"])]
        ...
        for key in keys:
            configure_mesh(key, baseline["assets"][key], baseline, subsystem, materials, meshes[key])
        ...
        if registry != wanted_registry or set(disk) != wanted_disk:
            lane.fail("exact eight-package registry/disk inventory drift")
        ...
        record.update({"status": PASS_STATUS, "baseline_status": baseline["status"],
                       ...
                       "asset_count": 8, "lod_count_per_asset": 3, "source_fbx_count": 24,
                       "custom_lods_appended": 16, "new_material_or_texture_assets": 0,
                       "strict_per_asset_monotonic_triangles_verified": True,
                       "exact_one_uv_channel_per_lod_verified": True,
                       "per_asset_collision_suitability_verified": True, "failures": [],
                       "automatic_cleanup": "NOT_PERFORMED__PARTIAL_ARTIFACTS_PRESERVED_FOR_EXPLICIT_REVIEW"})
        receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log("LINE_BOSS_ASSEMBLY_NATIVE_KIT_V001_IMPORT_PASS")
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record.update({"status": "FAIL_CLOSED__ASSEMBLY_NATIVE_KIT_V001_UNREAL_IMPORT",
        ...
        unreal.log_error("LINE_BOSS_ASSEMBLY_NATIVE_KIT_V001_IMPORT_FAIL: " + str(error))
        try:
            unreal.SystemLibrary.quit_editor()
        finally:
            raise
