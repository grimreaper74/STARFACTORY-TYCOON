"""Capture one exact-v288 whole-shop fixed view per clean Unreal process."""
import os
import time
...
CAPTURES = {
    "control": ("LB_WHOLE_V224_CAM_ControlRoomWalkUp", "v288_control_room_walk_up.png"),
    "management": ("LB_WHOLE_V223_CAM_FourTrainsManagement", "v288_four_trains_management.png"),
    "south": ("LB_WHOLE_V223_CAM_FourTrainsSouth", "v288_four_trains_south.png"),
    "front": ("LB_WHOLE_V223_CAM_FrontEndToTrains", "v288_front_end_to_trains.png"),
}
capture_id = os.environ.get("LB_V288_RELEASE_CAPTURE", "control").lower()
if capture_id not in CAPTURES:
    raise RuntimeError(capture_id)
camera_label, filename = CAPTURES[capture_id]
output = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShopIntegration/v288_release_gate" / filename
...
task = unreal.AutomationLibrary.take_high_res_screenshot(
    ...
    comparison_notes=f"Cairnwell Moorcross exact v288 release gate {capture_id}",
...
def finish(_delta):
    ...
    if elapsed >= 3.0 and output.exists() and output.stat().st_size >= 1024:
        unreal.log(f"LB_V288_RELEASE_CAPTURE_PASS id={capture_id} path={output}")
    elif elapsed < 65.0:
        return
    else:
        unreal.log_error(f"LB_V288_RELEASE_CAPTURE_FAIL id={capture_id} path={output}")
