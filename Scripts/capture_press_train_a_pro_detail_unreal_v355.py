"""Fresh fixed-camera inherited-hall evidence for isolated v354 Pro-detail intake."""
import os, time
from pathlib import Path
import unreal

unreal.EditorPythonScripting.set_keep_python_script_alive(True)
view=os.environ.get("LB_V355_VIEW","operator_oblique").lower()
views={
 "operator_oblique":((4850.0,-5600.0,820.0),(3850.0,-4300.0,390.0),118.0),
 "operator_elevated":((4750.0,-5550.0,1500.0),(3850.0,-4300.0,360.0),112.0),
 "full_broadside":((3850.0,-5600.0,820.0),(3850.0,-4300.0,400.0),126.0),
}
if view not in views: raise RuntimeError(view)
out=Path(unreal.Paths.project_saved_dir())/f"ValidationScreenshots/PressShopIntegration/v355_train_a_pro_detail/{view}.png"
if out.exists(): raise RuntimeError(f"Refusing to overwrite {out}")
levels=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
if not levels.load_level("/Game/LineBoss/Maps/LB_PressShop_TrainAProDetailVisualCandidate_v354"):
 raise RuntimeError("v354 load failed")
...
if not task.is_valid_task(): raise RuntimeError("Invalid v355 task")
started=time.monotonic();handle=None
def tick(_):
 global handle
 elapsed=time.monotonic()-started
 if elapsed>=16 and out.exists() and out.stat().st_size>=1024:
  unreal.log(f"LB_TRAIN_A_V355_CAPTURE_PASS view={view}")
 elif elapsed<90:return
 else:unreal.log_error(f"LB_TRAIN_A_V355_CAPTURE_FAIL view={view}")
