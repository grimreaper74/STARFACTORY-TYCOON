"""import_runway_furniture_v001.py - the hover-test pad (owner's
evening drop 2026-08-26, generated from the LaunchRunway_v001
blockout). Runway SITE FURNITURE, never player-built. Derivatives
measured 25.5 x 25.5 x 3.2 m against the 26 x 26 blockout envelope.
Albedo measured 0.266 linear -> boost 0.83 (dimmed to the 0.22
fleet target). Imports the two runtime derivatives, bounds-gated,
imports the three textures with correct colour-space flags, creates
MI_LB_HoverPad on the current master (M_LB_MeshyPBR_v003), assigns
the MI to every slot of both LODs. Fails closed at every step.
"""
...
FBX = SRC + r"\FBX"
TEX_SRC = SRC + r"\TexturesByModel\HoverPad"
...
unreal.log("HOVER PAD IMPORT DONE: 2 LODs, MI on v003 master, "
           "boost %.2f" % BOOST)
