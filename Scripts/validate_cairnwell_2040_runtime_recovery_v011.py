...
def run_root() -> Path:
    ...
    if os.environ.get(ACK_ENV, "").strip() != recovery.RUN_ACK_TOKEN:
        raise RuntimeError("v011 guarded-run acknowledgement absent")
    path = Path(raw).resolve()
    if (path.parent != recovery.RECOVERY_AUDIT_ROOT.resolve()
            or not re.fullmatch(r"\d{8}T\d{6}Z-[0-9a-f]{8}", path.name)
            or not path.is_dir()):
        raise RuntimeError("v011 run root escapes or is absent: " + str(path))
    return path
...
def main() -> None:
    ...
    try:
        ...
        if receipt.exists() or failure.exists():
            raise RuntimeError("current v011 run already contains a validation result")
        baseline, contract, state = load_validation_baseline()
        imported, _summary, quarantine = recovery.prior.validate_v009_receipts(
            state["v009"])
        import_pid = int(imported.get("process_id", -1))
        if import_pid != 36612 or import_pid == os.getpid():
            raise RuntimeError("v011 validator is not distinct from exact v009 import process")
        if core.sha256(
                recovery.prior.V009_RUN / "import_receipt_recovery_v009.json") \
                != recovery.prior.V009_IMPORT_RECEIPT_SHA256:
        ...
        if (namespace_before != imported.get("namespace_disk_files")
            ...
            raise RuntimeError("v009 package bytes changed before v011 fresh asset loads")
        ...
        if registry_before != expected_registry:
            raise RuntimeError("v011 pre-load asset-registry closure drift")
        ...
        if package_hashes_after != package_hashes_before:
            raise RuntimeError("runtime package bytes changed during v011 read-only validation")
        if namespace_after != namespace_before:
            raise RuntimeError("runtime namespace changed during v011 read-only validation")
        if registry_after != registry_before:
            raise RuntimeError("asset-registry closure changed during v011 read-only validation")
        if (source_after != source_before or protected_after != protected_before
                or prepared_lane_after != prepared_lane_before):
            raise RuntimeError("source/protected/prepared lane changed during v011 validation")
        ...
        evidence.update({
            ...
            "v010_recovery_contract_sha256": recovery.V010_CONTRACT_SHA256,
            "v009_recovery_contract_sha256": recovery.prior.V009_CONTRACT_SHA256,
            "v009_import_receipt_sha256": recovery.prior.V009_IMPORT_RECEIPT_SHA256,
            "v009_wrapper_failure_summary_sha256": recovery.prior.V009_SUMMARY_SHA256,
            "v009_quarantine_receipt_sha256": (
                recovery.prior.V009_QUARANTINE_RECEIPT_SHA256),
            ...
            "quarantine_receipt": {
                "path": core.relative(
                    recovery.prior.V009_RUN / "quarantine_receipt_v009.json"),
                "bytes": recovery.V009_QUARANTINE_RECEIPT_BYTES,
                "sha256": recovery.prior.V009_QUARANTINE_RECEIPT_SHA256,
        ...
        recovery.validate_v011_receipt(evidence, contract, state, root)
        ...
        unreal.log_error(
            "LINE_BOSS_CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_VALIDATION_FAIL: "
            + str(error))
        print(json.dumps(evidence, indent=2))
        raise
