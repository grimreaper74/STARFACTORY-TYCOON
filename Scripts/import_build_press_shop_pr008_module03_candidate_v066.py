"""Import detailed Pro PR-008 Module 03 into isolated candidate v066."""
import json
...
RECORDS = json.loads((SOURCE / "pr008_module03_servo_feed_manifest_v001.json").read_text(encoding="utf-8"))
BASE = "/Game/LineBoss/Maps/LB_PressShop_PR008Module02Candidate_v065"
MAP = "/Game/LineBoss/Maps/LB_PressShop_PR008Module03Candidate_v066"
DEST = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Module03"
MAT = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Materials"
PREFIX = "LB_PR008_V066_"
DATUM = unreal.Vector(-500.0, -2000.0, 0.0)
YAW = -90.0
OUT = ROOT / "Saved/Audits/press_shop_pr008_module03_candidate_v066.json"
...
map_file = ROOT / "Content/LineBoss/Maps/LB_PressShop_PR008Module03Candidate_v066.umap"
if not map_file.exists():
    if not library.duplicate_asset(BASE, MAP) or not library.save_asset(MAP, only_if_is_dirty=False):
        raise RuntimeError("Could not prepare v066 map")
    unreal.log("LINE_BOSS_PR008_V066_PREPARE_PASS__RERUN_FOR_POPULATION")
...
cage = next((actor for actor in actors_api.get_all_level_actors()
             if actor.get_actor_label() == "LB_PR008_V062_03_ServoFeedRollModule"), None)
if cage is None:
    raise RuntimeError("Missing Pro Module 03 engineering cage")
...
for record in RECORDS:
    ...
    if not isinstance(mesh, unreal.StaticMesh):
        raise RuntimeError(f"Missing detailed Module 03 mesh {record['name']}")
    ...
    actor.tags = [unreal.Name(value) for value in ("LB.Asset.Candidate.v066", "LB.Asset.CandidateNotPromoted",
        "LB.Station.PR008", "LB.Module.PR008.03.ServoFeed", "LB.Authority.CairnwellRemainingMachineryPack.v1")]
...
def text(label, value, location, size, colour):
    ...
    actor.tags = [unreal.Name("LB.Asset.Candidate.v066"), unreal.Name("LB.Asset.CandidateNotPromoted"), unreal.Name("LB.Station.PR008.Identity")]
...
identity = [
    text("Brand", "CAIRNWELL / MOORCROSS", (-820.2, -2000.0, 153.5), 2.7, unreal.Color(45, 130, 105, 255)),
    text("Station", "PR-008  SERVO FEED", (-820.2, -2000.0, 146.5), 3.0, unreal.Color(35, 38, 40, 255)),
]
...
def camera(label, location, target, fov):
    ...
    actor.tags = [unreal.Name("LB.Camera.Validation"), unreal.Name("LB.Camera.Fixed.PR008.v066"), unreal.Name("LB.Asset.CandidateNotPromoted")]
...
cameras = [
    camera("Module03Inspection", (-1040, -2480, 500), (-750, -2000, 108), 43),
    camera("Module03Drive", (-980, -1540, 360), (-750, -2000, 110), 48),
    camera("Module03Elevated", (-1350, -3000, 760), (-750, -2000, 105), 54),
    camera("Module03Connected", (-1850, -3500, 850), (-950, -2000, 112), 59),
]
...
expected_min, expected_max = [-822.5, -2130.0, -2.5], [-677.5, -1870.0, 192.5]
...
if not within:
    ...
    raise RuntimeError(f"Imported Module 03 exceeds Pro envelope: min={mins} max={maxs} worst_world_x={worst_x}")

if not levels.save_current_level():
    raise RuntimeError(f"Could not save {MAP}")
library.save_directory(DEST, only_if_is_dirty=False, recursive=True)
payload = {
    "$schema": "line-boss/audit/press-shop-pr008-module03-candidate-v066/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PRO_MODULE03_DETAILED_IMPORT_ROTARY_PIVOT_AND_ENVELOPE_CONTAINMENT_PASS__RUNTIME_COLLISION_NAVIGATION_AND_VISUAL_GATES_REQUIRED__NOT_PROMOTED",
    "map": MAP, "base_map": BASE, "source": str(SOURCE), "module_id": "03",
    "semantic_actor_count": len(created), "moving_actor_count": sum(1 for record in RECORDS if record["movable"]),
    "moving_contract": "top and bottom feed-roll cores/sleeves rotate continuously about local X at 12 rpm; safe pose stop/brake", "placements": placements,
...
unreal.log(f"LINE_BOSS_PR008_V066_BUILD_PASS actors={len(created)} moving={payload['moving_actor_count']}")
unreal.SystemLibrary.quit_editor()
