"""Import detailed Pro PR-008 Module 06 into isolated candidate v069."""
import json
...
RECORDS = json.loads((SOURCE / "pr008_module06_shear_manifest_v001.json").read_text(encoding="utf-8"))
BASE = "/Game/LineBoss/Maps/LB_PressShop_PR008Module05Candidate_v068"
MAP = "/Game/LineBoss/Maps/LB_PressShop_PR008Module06Candidate_v069"
DEST = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Module06"
MAT = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Materials"
PREFIX = "LB_PR008_V069_"
DATUM = unreal.Vector(-500.0, -2000.0, 0.0)
YAW = -90.0
OUT = ROOT / "Saved/Audits/press_shop_pr008_module06_candidate_v069.json"
...
map_file = ROOT / "Content/LineBoss/Maps/LB_PressShop_PR008Module06Candidate_v069.umap"
if not map_file.exists():
    if not library.duplicate_asset(BASE, MAP) or not library.save_asset(MAP, only_if_is_dirty=False):
        raise RuntimeError("Could not prepare v069 map")
    unreal.log("LINE_BOSS_PR008_V069_PREPARE_PASS__RERUN_FOR_POPULATION")
...
cage = next((actor for actor in actors_api.get_all_level_actors()
             if actor.get_actor_label() == "LB_PR008_V062_06_CutToLengthShear"), None)
if cage is None:
    raise RuntimeError("Missing Pro Module 06 engineering cage")
...
for record in RECORDS:
    ...
    if not isinstance(mesh, unreal.StaticMesh):
        raise RuntimeError(f"Missing detailed Module 06 mesh {record['name']}")
    ...
    actor.tags = [unreal.Name(value) for value in ("LB.Asset.Candidate.v069", "LB.Asset.CandidateNotPromoted",
        "LB.Station.PR008", "LB.Module.PR008.06.CutToLengthShear", "LB.Authority.CairnwellRemainingMachineryPack.v1")]
...
def text(label, value, location, size, colour):
    ...
    actor.tags = [unreal.Name("LB.Asset.Candidate.v069"), unreal.Name("LB.Asset.CandidateNotPromoted"), unreal.Name("LB.Station.PR008.Identity")]
...
identity = [
    text("Brand", "CAIRNWELL / MOORCROSS", (-314.0, -2000.0, 222.0), 2.7, unreal.Color(45, 130, 105, 255)),
    text("Station", "PR-008  CUT-TO-LENGTH SHEAR", (-314.0, -2000.0, 215.0), 2.8, unreal.Color(35, 38, 40, 255)),
]
...
def camera(label, location, target, fov):
    ...
    actor.tags = [unreal.Name("LB.Camera.Validation"), unreal.Name("LB.Camera.Fixed.PR008.v069"), unreal.Name("LB.Asset.CandidateNotPromoted")]
...
if not within:
    offenders = [item for item in actor_bounds if any(item["min"][i] < expected_min[i] - 0.1 or item["max"][i] > expected_max[i] + 0.1 for i in range(3))]
    raise RuntimeError(f"Imported Module 06 exceeds Pro envelope: min={mins} max={maxs} offenders={offenders}")

if not levels.save_current_level():
    raise RuntimeError(f"Could not save {MAP}")
library.save_directory(DEST, only_if_is_dirty=False, recursive=True)
payload = {
    "$schema": "line-boss/audit/press-shop-pr008-module06-candidate-v069/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PRO_MODULE06_DETAILED_IMPORT_BLADE_PIVOT_PROCESS_SAFETY_AND_ENVELOPE_CONTAINMENT_PASS__RUNTIME_COLLISION_NAVIGATION_AND_VISUAL_GATES_REQUIRED__NOT_PROMOTED",
    "map": MAP, "base_map": BASE, "source": str(SOURCE), "module_id": "06",
    "semantic_actor_count": len(created), "moving_actor_count": sum(1 for record in RECORDS if record["movable"]),
    "moving_contract": "shear blade beam local Z down 0-180 mm at 300 mm/s top-safe", "placements": placements,
...
unreal.log(f"LINE_BOSS_PR008_V069_BUILD_PASS actors={len(created)} moving={payload['moving_actor_count']}")
unreal.SystemLibrary.quit_editor()
