...
MAP='/Game/LineBoss/Maps/LB_PressShop_CleanConnectedS07_v20260809_v791';ROOT=Path(unreal.Paths.project_dir())
OUT=ROOT/r'Saved\ValidationScreenshots\PressShopIntegration\v895_original_pr009_pr010_v897';AUDIT=ROOT/r'Saved\Audits\PressShopIntegration\v895_original_pr009_pr010_capture_v897.json'
VIEWS=[
 ('01_PR009_Hero_v897.png',(-300,-4100,800),(600,-2000,180),62.0),
 ('04_PR010_Hero_v897.png',(2700,-5000,950),(1350,-2000,150),64.0),
]
...
for station,loc in {'PR009':(600,-2000,0),'PR010':(1350,-2000,0)}.items():
 mesh=lib.load_asset(f'/Game/LineBoss/Candidates/PressShop/PR009_PR010_OriginalFBX_v884/{station}/SM_CA_MW_{station}_OriginalHighPoly_v883')
 if not isinstance(mesh,unreal.StaticMesh):raise RuntimeError(f'missing {station} mesh')
 a=actors.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector(*loc),unreal.Rotator(0,-90,0));a.set_actor_label(f'LB_{station}_OriginalHighPoly_TRANSIENT_v897');a.static_mesh_component.set_static_mesh(mesh);a.static_mesh_component.set_collision_enabled(unreal.CollisionEnabled.NO_COLLISION);a.static_mesh_component.set_editor_property('visible',True);a.static_mesh_component.set_editor_property('hidden_in_game',False);targets.add(a.get_actor_label())
...
floor=actors.spawn_actor_from_class(unreal.StaticMeshActor,unreal.Vector(900,-2000,-12),unreal.Rotator());floor.static_mesh_component.set_static_mesh(cube);floor.set_actor_scale3d(unreal.Vector(22,22,.1));floor.static_mesh_component.set_cast_shadow(False)
sun=actors.spawn_actor_from_class(unreal.DirectionalLight,unreal.Vector(900,-2000,1000),unreal.Rotator(pitch=-42,yaw=-35,roll=0));sun.light_component.set_editor_property('intensity',2.5)
sky=actors.spawn_actor_from_class(unreal.SkyLight,unreal.Vector(900,-2000,500),unreal.Rotator());cube_map=unreal.EditorAssetLibrary.load_asset('/Engine/EngineSky/DefaultTextureCube.DefaultTextureCube')
...
for loc,intensity,radius in [((0,-2800,750),3500,2200),((1800,-2800,750),4000,2400),((900,-800,600),2500,2200)]:
