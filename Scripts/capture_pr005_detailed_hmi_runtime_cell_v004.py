...
OUT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShop/PR005_DetailedHMI_v001/pr005_runtime_cell_overview_v004.png"
TAG = unreal.Name("LB.Capture.PR005DetailedHMI.RuntimeCell.v004")
...
camera.set_actor_label("LB_CAPTURE_PR005_DETAILED_HMI_OVERVIEW_V004_NOT_SAVED")
camera.tags = [TAG]
...
def tick(_delta):
    ...
    if not runtime_checked:
        ...
        overlaps = hmi.get_generate_overlap_events()
        materials = hmi.get_materials()
        unreal.log(
            "LINE_BOSS_PR005_DETAILED_HMI_RUNTIME_CELL "
            f"visible={visible} mesh={mesh} collision={collision} overlaps={overlaps} "
            f"materials={materials} transform={hmi.get_relative_transform()}"
        )
        if not visible or mesh is None or collision != unreal.CollisionEnabled.NO_COLLISION or overlaps:
