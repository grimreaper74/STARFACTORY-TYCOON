"""Independent fresh-process, read-only validator for native Paint-kit intake."""
...
EXPECTED_IMPORT_STATUS = "PASS__HASH_GUARDED_FRESH_IMPORT__7_ASSETS__21_AUTHORED_LODS__PAINT_NATIVE_KIT_V001"
PASS_STATUS = "PASS__INDEPENDENT_FRESH_PROCESS_RELOAD__7_ASSETS__21_AUTHORED_LODS__PAINT_NATIVE_KIT_V001"
...
def main() -> None:
    ...
    record = {"$schema": "lineboss/audit/paint-native-kit-v001/fresh-load-validation/v1",
    ...
    try:
        ...
        if (imported.get("$schema") != "lineboss/audit/paint-native-kit-v001/unreal-import/v1" or
                imported.get("status") != EXPECTED_IMPORT_STATUS or
                imported.get("baseline_sha256") != lane.EXPECTED_BASELINE_SHA256 or
                import_pid <= 0 or import_pid == os.getpid() or imported.get("asset_count") != 7 or
                imported.get("source_fbx_count") != 21 or imported.get("custom_lods_appended") != 14):
            lane.fail("same-run import receipt/fresh-process identity drift")
        guard = imported.get("interchange_fbx_legacy_custom_lod_guard", {})
        if (guard.get("custom_lods_requested") != 14 or len(guard.get("custom_lods_imported", [])) != 14 or
        ...
        if (imported.get("both_x_end_portals_and_body_skid_rails_clear_by_exact_geometry_collision") is not True or
                imported.get("black_box_no_robot_no_window_no_side_door_source_contract_verified") is not True):
            lane.fail("import receipt lacks Paint direction/collision proof")
        ...
        record.update({"status": PASS_STATUS, "baseline_sha256": lane.sha256(lane.BASELINE),
                       ...
                       "asset_count": 7, "lod_count_per_asset": 3,
                       ...
                       "spray_booth_namespace_and_pass_receipts_unchanged": True,
                       "native_body_robot_support_and_assembly_namespaces_unchanged": True,
                       "source_manifest_geometry_roundtrip_freeze_provenance_and_visual_receipts_reverified": True,
                       "manual_lod_screen_sizes_persisted": True,
                       "exact_triangles_one_uv_bounds_pivots_material_semantics_persisted": True,
                       "portal_and_body_skid_clearance_collision_persisted": True,
                       "black_box_no_robot_no_window_no_side_door_contract_persisted": True,
                       "nanite_off_all_assets": True, "new_material_or_texture_assets": 0, "failures": []})
        receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log("LINE_BOSS_PAINT_NATIVE_KIT_V001_FRESH_LOAD_VALIDATION_PASS")
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record.update({"status": "FAIL_CLOSED__PAINT_NATIVE_KIT_V001_FRESH_LOAD_VALIDATION",
        ...
        unreal.log_error("LINE_BOSS_PAINT_NATIVE_KIT_V001_FRESH_LOAD_VALIDATION_FAIL: " + str(error))
        try:
            unreal.SystemLibrary.quit_editor()
        finally:
            raise
