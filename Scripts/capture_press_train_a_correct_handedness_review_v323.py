"""Capture the v323 handedness candidate after shaders settle."""
import time
from pathlib import Path
import unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True);MAP="/Game/LineBoss/Maps/LB_PressTrainA_DoubleMirrorReviewCandidate_v324";out=Path(unreal.Paths.project_saved_dir())/"ValidationScreenshots/PressShopIntegration/v324_train_a_double_mirror_review/v324_train_a_operator.png";levels=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);api=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
if not levels.load_level(MAP):raise RuntimeError(MAP)
cam=next((a for a in api.get_all_level_actors() if a.get_actor_label()=="LB_V307_CAM_Operator"),None)
...
def done(_):
 ...
 if elapsed>=16 and out.exists() and out.stat().st_size>=1024:unreal.log(f"LB_V324_CAPTURE_PASS path={out}")
 elif elapsed<90:return
 else:unreal.log_error("LB_V323_CAPTURE_FAIL")
