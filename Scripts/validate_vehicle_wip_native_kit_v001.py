"""Independent fresh-process read-only validator for vehicle-WIP native intake."""
...
EXPECTED_IMPORT_STATUS = "PASS__HASH_GUARDED_FRESH_IMPORT__16_NATIVE_VEHICLE_ROLES__48_AUTHORED_LODS__V001"
PASS_STATUS = "PASS__INDEPENDENT_FRESH_PROCESS_RELOAD__16_NATIVE_VEHICLE_ROLES__48_AUTHORED_LODS__V001"
...
def main() -> None:
    ...
    record = {"$schema": "lineboss/audit/vehicle-wip-native-kit-v001/fresh-load-validation/v1",
    ...
    try:
        ...
        if (imported.get("$schema") != "lineboss/audit/vehicle-wip-native-kit-v001/unreal-import/v1"
                or imported.get("status") != EXPECTED_IMPORT_STATUS
                or imported.get("baseline_sha256") != lane.sha256(lane.BASELINE)
                or import_pid <= 0 or import_pid == os.getpid()
                or imported.get("asset_count") != 16 or imported.get("source_fbx_count") != 48
                or imported.get("custom_lods_appended") != 32):
            lane.fail("same-run import receipt/fresh-process identity drift")
        guard = imported.get("interchange_fbx_legacy_custom_lod_guard", {})
        if (guard.get("custom_lods_requested") != 32 or len(guard.get("custom_lods_imported", [])) != 32
                or guard.get("restore_attempted_in_finally") is not True
                or guard.get("restored_value") != guard.get("previous_value")):
            lane.fail("import receipt lacks guarded custom-LOD CVar restoration proof")
        ...
        if registry != wanted_registry:
            lane.fail("fresh-process native asset registry inventory drift")
        ...
        if source_after != source_before or protected_after != protected_before or prepared_lane_after != prepared_lane_before:
            lane.fail("fresh reload changed protected source/project/lane")
        record.update({"status": PASS_STATUS, "baseline_sha256": lane.sha256(lane.BASELINE),
                       ...
                       "prepared_lane_before": prepared_lane_before, "prepared_lane_after": prepared_lane_after,
                       ...
                       "asset_count": 16, "lod_count_per_asset": 3,
                       "target_package_hashes_unchanged_by_fresh_load": True,
                       "complete_source_content_config_maps_saves_and_lane_unchanged": True,
                       "source_manifest_geometry_roundtrip_provenance_and_freeze_reverified": True,
                       "semantic_material_slots_persisted": True,
                       "zero_collision_all_moving_wip_persisted": True,
                       "nanite_off_all_assets": True, "maps_loaded_or_saved": [], "failures": []})
        receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log("LINE_BOSS_VEHICLE_WIP_NATIVE_KIT_V001_FRESH_VALIDATION_PASS")
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record.update({"status": "FAIL_CLOSED__VEHICLE_WIP_NATIVE_KIT_V001_FRESH_VALIDATION",
                       "error": str(error), "traceback": traceback.format_exc()})
        failure.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log_error("LINE_BOSS_VEHICLE_WIP_NATIVE_KIT_V001_FRESH_VALIDATION_FAIL: " + str(error))
        try:
            unreal.SystemLibrary.quit_editor()
        finally:
            raise
