"""Capture Train A with both editor viewport APIs forced to Lit mode."""
import os,time
from pathlib import Path
import unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True)
MAP="/Game/LineBoss/Maps/LB_PressTrainA_MovableLitReviewCandidate_v315"
CAP={"operator":("LB_V309_CAM_Operator","v318_train_a_operator.png"),"rear":("LB_V309_CAM_Rear","v318_train_a_rear.png"),"elevated":("LB_V309_CAM_Elevated","v318_train_a_elevated.png")}
cid=os.environ.get("LB_V318_CAPTURE","operator").lower();label,name=CAP[cid]
out=Path(unreal.Paths.project_saved_dir())/"ValidationScreenshots/PressShopIntegration/v318_train_a_forced_lit_review"/name
...
unreal.AutomationLibrary.set_editor_viewport_view_mode(unreal.ViewModeIndex.VMI_LIT);unreal.AutomationLibrary.set_editor_active_viewport_view_mode(unreal.ViewModeIndex.VMI_LIT)
task=unreal.AutomationLibrary.take_high_res_screenshot(1920,1080,str(out),camera=cam,mask_enabled=False,capture_hdr=False,comparison_tolerance=unreal.ComparisonTolerance.LOW,comparison_notes=f"Train A v318 forced lit {cid}",delay=0,force_game_view=True)
if not task.is_valid_task():raise RuntimeError("invalid screenshot")
started=time.monotonic();handle=None
def done(_):
 global handle
 elapsed=time.monotonic()-started
 if elapsed>=3 and out.exists() and out.stat().st_size>=1024:unreal.log(f"LB_V318_CAPTURE_PASS id={cid} path={out}")
 elif elapsed<70:return
 else:unreal.log_error(f"LB_V318_CAPTURE_FAIL id={cid}")
