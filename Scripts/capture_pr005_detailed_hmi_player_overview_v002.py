"""Non-mutating player-overview evidence for the PR005 detailed-HMI candidate.
Loads v913, enters temporary simulation, asserts the candidate is visual-only, captures,
ends simulation and quits. It never saves or edits the protected map.
"""
import time
...
OUT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShop/PR005_DetailedHMI_v001/player_overview_candidate_v002.png"
TAG = unreal.Name("LB.Capture.PR005DetailedHMI.v002")
...
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, unreal.Vector(-6051.84, 4237.55, 9456.13), unreal.Rotator())
camera.set_actor_label("LB_CAPTURE_PR005DetailedHMI_v002_NOT_SAVED")
camera.tags = [TAG]
...
def finish(ok, message):
    ...
    (unreal.log if ok else unreal.log_error)(
        f"LINE_BOSS_PR005_DETAILED_HMI_PLAYER_CAPTURE_{'PASS' if ok else 'FAIL'} {message}")
...
def tick(_delta):
    ...
    if not runtime_checked:
        ...
        unreal.log(
            "LINE_BOSS_PR005_DETAILED_HMI_RUNTIME "
            f"component={hmi.get_name()} visible={visible} mesh={mesh} "
            f"collision={collision} nav={nav} transform={hmi.get_relative_transform()}"
        )
    ...
    if capture_started is None and now - started >= 7.0 and runtime_cameras:
