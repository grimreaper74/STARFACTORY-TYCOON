"""Import detailed Pro PR-008 Module 05 into isolated candidate v068."""
import json
...
RECORDS = json.loads((SOURCE / "pr008_module05_prepunch_manifest_v001.json").read_text(encoding="utf-8"))
BASE = "/Game/LineBoss/Maps/LB_PressShop_PR008Module04Candidate_v067"
MAP = "/Game/LineBoss/Maps/LB_PressShop_PR008Module05Candidate_v068"
DEST = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Module05"
MAT = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Materials"
PREFIX = "LB_PR008_V068_"
DATUM = unreal.Vector(-500.0, -2000.0, 0.0)
YAW = -90.0
OUT = ROOT / "Saved/Audits/press_shop_pr008_module05_candidate_v068.json"
...
map_file = ROOT / "Content/LineBoss/Maps/LB_PressShop_PR008Module05Candidate_v068.umap"
if not map_file.exists():
    if not library.duplicate_asset(BASE, MAP) or not library.save_asset(MAP, only_if_is_dirty=False):
        raise RuntimeError("Could not prepare v068 map")
    unreal.log("LINE_BOSS_PR008_V068_PREPARE_PASS__RERUN_FOR_POPULATION")
...
cage = next((actor for actor in actors_api.get_all_level_actors()
             if actor.get_actor_label() == "LB_PR008_V062_05_PrePunchPress"), None)
if cage is None:
    raise RuntimeError("Missing Pro Module 05 engineering cage")
...
for record in RECORDS:
    ...
    if not isinstance(mesh, unreal.StaticMesh):
        raise RuntimeError(f"Missing detailed Module 05 mesh {record['name']}")
    ...
    actor.tags = [unreal.Name(value) for value in ("LB.Asset.Candidate.v068", "LB.Asset.CandidateNotPromoted",
        "LB.Station.PR008", "LB.Module.PR008.05.PrePunchPress", "LB.Authority.CairnwellRemainingMachineryPack.v1")]
...
def text(label, value, location, size, colour):
    ...
    actor.tags = [unreal.Name("LB.Asset.Candidate.v068"), unreal.Name("LB.Asset.CandidateNotPromoted"), unreal.Name("LB.Station.PR008.Identity")]
...
identity = [
    text("Brand", "CAIRNWELL / MOORCROSS", (-501.8, -2000.0, 222.0), 2.7, unreal.Color(45, 130, 105, 255)),
    text("Station", "PR-008  PRE-PUNCH PRESS", (-501.8, -2000.0, 215.0), 2.8, unreal.Color(35, 38, 40, 255)),
]
...
def camera(label, location, target, fov):
    ...
    actor.tags = [unreal.Name("LB.Camera.Validation"), unreal.Name("LB.Camera.Fixed.PR008.v068"), unreal.Name("LB.Asset.CandidateNotPromoted")]
...
cameras = [
    camera("Module05Inspection", (-850, -1500, 470), (-410, -2000, 125), 44),
    camera("Module05Drive", (-850, -2500, 390), (-410, -2000, 125), 48),
    camera("Module05Elevated", (-1120, -1420, 760), (-410, -2000, 135), 54),
    camera("Module05Connected", (-1700, -3150, 850), (-720, -2000, 120), 59),
]
...
expected_min, expected_max = [-502.5, -2142.5, -85.0], [-317.5, -1857.5, 265.0]
...
if not within:
    ...
    raise RuntimeError(f"Imported Module 05 exceeds Pro envelope: min={mins} max={maxs} worst_world_x={worst_x} worst_min_world_x={worst_min_x}")

if not levels.save_current_level():
    raise RuntimeError(f"Could not save {MAP}")
library.save_directory(DEST, only_if_is_dirty=False, recursive=True)
payload = {
    "$schema": "line-boss/audit/press-shop-pr008-module05-candidate-v068/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PRO_MODULE05_DETAILED_IMPORT_SLIDE_DOOR_FLAP_PIVOT_AND_ENVELOPE_CONTAINMENT_PASS__RUNTIME_COLLISION_NAVIGATION_AND_VISUAL_GATES_REQUIRED__NOT_PROMOTED",
    "map": MAP, "base_map": BASE, "source": str(SOURCE), "module_id": "05",
    "semantic_actor_count": len(created), "moving_actor_count": sum(1 for record in RECORDS if record["movable"]),
    "moving_contract": "pre-punch slide local Z down 0-220 mm at 120 mm/s top-safe; service doors 0-110 degrees closed-safe; scrap flap 0-70 degrees at 15 deg/s closed-safe", "placements": placements,
...
unreal.log(f"LINE_BOSS_PR008_V068_BUILD_PASS actors={len(created)} moving={payload['moving_actor_count']}")
unreal.SystemLibrary.quit_editor()
