...
Run only through ``run_body_shop_support_kit_native_unreal_import_lane_v003.ps1``.
...
def fail(message: str) -> None:
    raise RuntimeError("BODYSHOP_SUPPORT_KIT_NATIVE_UNREAL_IMPORT_LANE_V003_FAIL: " + message)
...
def main() -> None:
    ...
    evidence = {
        "$schema": "lineboss/audit/bodyshop-support-kit-native-v002-unreal-import/v3",
        "generated_utc": now(),
    ...
    try:
        ...
        if existing_results:
            fail("v003 refuses every pre-existing lane result: " + repr(existing_results))
        ...
        evidence.update({
            "status": "PASS__HASH_GUARDED_FROZEN_BODYSHOP_SUPPORT_KIT_NATIVE_V002_BASELINE_V003_UNREAL_INTAKE",
        ...
        unreal.log("LINE_BOSS_BODYSHOP_SUPPORT_KIT_NATIVE_V002_LANE_V003_HASH_GUARDED_IMPORT_PASS")
        print(json.dumps(evidence, indent=2))
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record = dict(evidence)
        record.update({
            "status": "FAIL_CLOSED__BODYSHOP_SUPPORT_KIT_NATIVE_V002_UNREAL_IMPORT_LANE_V003",
            "error": str(error),
            ...
            "automatic_cleanup": "NOT_PERFORMED__PARTIAL_ARTIFACTS_PRESERVED_FOR_EXPLICIT_REVIEW",
            "recovery": "Archive this run; do not rerun lane v003 or delete/replace a package implicitly.",
        })
        failure_receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log_error("LINE_BOSS_BODYSHOP_SUPPORT_KIT_NATIVE_V002_LANE_V003_IMPORT_FAIL: " + str(error))
