...
import time,unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True);MAP="/Game/LineBoss/Developer/Validation/LB_InboundCoilDeliveryReview_v494";OUT=Path(unreal.Paths.project_saved_dir())/"ValidationScreenshots/PressShopIntegration/inbound_coil_delivery_v494/inbound_overview.png";levels=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
if not levels.load_level(MAP):raise RuntimeError("Could not load v494")
cam=next((a for a in actors.get_all_level_actors() if a.get_actor_label()=="LB_CAM_InboundCoilDelivery_v494"),None)
if cam is None:raise RuntimeError("Missing v494 camera")
...
def done(_dt):
 ...
 unreal.log(("LINE_BOSS_INBOUND_REVIEW_V494_CAPTURE_PASS " if OUT.exists() and OUT.stat().st_size>1024 else "LINE_BOSS_INBOUND_REVIEW_V494_CAPTURE_FAIL ")+str(OUT));unreal.SystemLibrary.quit_editor()
