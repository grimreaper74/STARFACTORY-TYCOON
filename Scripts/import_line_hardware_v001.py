"""Import the CONVEYOR SECTION as a Nanite static mesh.
...
The track was the most prominent thing on the factory floor and was
rendering as a flat painted stripe. Both reference games lean on the
conveyor as the spine of the picture, so this is the model the floor
most needed (owner 2026-08-28: "use meshy api if you need to make
anything").

A SECTION, not a machine: flat cut ends so pieces butt together along a
laid track. Declared at 400 cm on its long axis because that is exactly
the track authority's piece length - a conveyor that did not match the
piece grid would either gap or overlap at every joint.

Its sibling in the same drop, a telescopic hydraulic ram, came back as
an organic blob on a pedestal and is NOT imported. The engine-cylinder
ram already in the presenter reads correctly as nested stages, so
nothing is lost by rejecting it.
"""
...
source_dir = root / "SourceAssets/Candidate/Spacecraft/LineHardware_v008/export"
dest = "/Game/LineBoss/Candidates/Spacecraft/LineHardware_v001"
out = root / "Saved/Audits/Spacecraft/line_hardware_import_v001.json"
if out.exists():
...
EXPECTED = {
    "SM_LB_TR_ConveyorSection": ("x", 400),
}
...
for name, (axis, target_cm) in sorted(EXPECTED.items()):
    ...
    rows.append({
        ...
        "provenance": "Meshy text-to-3D preview, generated 2026-08-28 "
                      "by Scripts/submit_meshy_line_hardware_v008.ps1",
        "defining_axis": axis,
...
report = {
    "$schema": "lineboss/audit/line-hardware-import-v001/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": ("PASS__LINE_HARDWARE_IMPORTED__NANITE_ON" if not failures
               else "FAIL_CLOSED__LINE_HARDWARE_IMPORT"),
    "destination": dest,
    "assets": rows,
    "failures": failures,
    "not_proven": [
        "A Meshy PREVIEW mesh - untextured draft geometry wearing the "
        "project's graphite palette, per the rule that materials are "
        "authored in Unreal.",
        "Nobody has seen it laid along a track yet. Whether the "
        "sections butt cleanly at every joint is the thing to look for "
        "first; a tiling section that does not tile is worse than the "
        "painted stripe it replaces.",
        "Its sibling in the same drop - a telescopic hydraulic ram - "
        "came back unusable and was not imported. 40 credits spent, "
        "one asset kept.",
    ],
}
