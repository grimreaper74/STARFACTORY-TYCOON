"""Read-only v087 static/simple collision, binding and isolation audit."""
import json
import sys
...
from press_shop_pr009_release_collision_v087_config import TARGET_MAP, STATIC_DEST, MOVING_DEST
...
fixed = [actor for actor in actors if unreal.Name("LB.Collision.FixedChassis.v087") in actor.tags]
movers = [actor for actor in actors if unreal.Name("LB.Collision.SelectiveMover.v087") in actor.tags]
modular = [actor for actor in actors if actor.get_actor_label().startswith("LB_PR009_V087_MOD_")]
...
if any("COMPLEX_AS_SIMPLE" in (row["trace_flag"] or "").upper() for row in static_rows + fixed_rows + mover_rows + interface_rows):
    failures.append("complex-as-simple remains in v087 release scope")
...
payload = {"$schema":"cairnwell/audit/pr009-release-collision-static-v087/v1","generated_utc":datetime.now(timezone.utc).isoformat(),
...
unreal.log(f"PR009_V087_STATIC_COLLISION_{'PASS' if not failures else 'FAIL'} output={out}")
