"""Prepare isolated v008 without reopening it in the creating process."""
...
TARGET = "/Game/LineBoss/Developer/Validation/LB_ServiceDockActualRobotFit_v008"
SOURCE_FILE = ROOT / "Content/LineBoss/Developer/Validation/LB_ServiceDockFamilyVisual_v005.umap"
TARGET_FILE = ROOT / "Content/LineBoss/Developer/Validation/LB_ServiceDockActualRobotFit_v008.umap"
V253_FILE = ROOT / "Content/LineBoss/Maps/LB_PressShop_PlayableManagementCandidate_v253.umap"
OUT = ROOT / "Saved/Audits/SupportRobots/service_dock_actual_robot_fit_prepare_v008.json"
...
if LIB.does_asset_exist(TARGET) or TARGET_FILE.exists():
    raise RuntimeError("Refusing to overwrite preserved v008 fit candidate")
source_before = sha256(SOURCE_FILE)
v253_before = sha256(V253_FILE)
if not LIB.duplicate_asset(SOURCE, TARGET):
    raise RuntimeError("Could not duplicate retained v005 source to v008")
if not LIB.save_asset(TARGET, only_if_is_dirty=False):
    raise RuntimeError("Could not save duplicated v008 package")
if not TARGET_FILE.exists():
    raise RuntimeError("v008 package was not written")
...
payload = {
    "$schema": "cairnwell/audit/service-dock-actual-robot-fit-prepare-v008/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PASS__FRESH_V008_DUPLICATED_AND_SAVED_WITHOUT_REOPENING__BUILD_PROCESS_REQUIRED",
    "source_map": SOURCE,
    "target_map": TARGET,
...
unreal.log("LINE_BOSS_SERVICE_DOCK_FIT_V008_PREPARE_PASS")
