...
The Hero source handoff reported a Blender-side convention conflict.  This
read-only lane measures the actual UE transforms after the OneFactory builder
materialises the approved native S02 and StagePack components.  It opens no
map for save and writes evidence only beneath Saved/Audits.
"""
...
OUT = ROOT / "Saved/Audits/OneFactory/Press/NativeS02TrainOrientation_v001/orientation_receipt.json"
    ...
    for x in (bounds.min.x, bounds.max.x):
        for y in (bounds.min.y, bounds.max.y):
            for z in (bounds.min.z, bounds.max.z):
                points.append(transform.transform_position(unreal.Vector(x, y, z)))
...
def tick(_delta):
    try:
        ...
        finish({
            "$schema": "lineboss/onefactory/press/native-s02-train-orientation/v1",
