"""Add localized movable high-bay bounce lights close to the roof in a fresh v242 child."""
...
MAP = "/Game/LineBoss/Maps/LB_PressShop_PlayableManagementCandidate_v245"
OUT = ROOT / "Saved/Audits/PressShopIntegration/press_shop_high_bay_roof_bounce_build_v245.json"
BASE_FILE = ROOT / "Content/LineBoss/Maps/LB_PressShop_PlayableManagementCandidate_v242.umap"
MAP_FILE = ROOT / "Content/LineBoss/Maps/LB_PressShop_PlayableManagementCandidate_v245.umap"
...
for row_id, y_value in enumerate((-4300.0, -1700.0, 900.0), 1):
    for bay_id, x_value in enumerate((1500.0, 3500.0, 5500.0, 7500.0, 9500.0), 1):
        ...
        if light is None:
            ...
            continue
        label = f"LB_WHOLE_V245_HIGH_BAY_BOUNCE_{row_id:02d}_{bay_id:02d}"
        ...
        light.tags = [
            ...
            unreal.Name("LB.VisualCorrection.RoofReadability.v245"),
            unreal.Name("LB.Asset.CandidateNotPromoted"),
        ]
        added.append({
            "label": label,
            "location_cm": [x_value, y_value, 1810.0],
            "intensity_preview_only": 650.0,
            "attenuation_radius_cm": 950.0,
            "mobility": "MOVABLE",
        })

if len(added) != 15:
    failures.append(f"expected 15 high-bay bounce lights, added {len(added)}")
if not levels.save_current_level():
    failures.append("could not save v245")
parent_hash_after = sha256(BASE_FILE)
if parent_hash_after != parent_hash_before:
    failures.append("protected v242 parent changed")

payload = {
    "$schema": "cairnwell/audit/press-shop-high-bay-roof-bounce-build-v245/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PASS__LOCALIZED_MOVABLE_HIGH_BAY_BOUNCE_ADDED__FRESH_VISUAL_AND_RUNTIME_GATES_REQUIRED__NOT_PROMOTED" if not failures else "FAIL__NOT_PROMOTED",
    "base": BASE,
    "map": MAP,
    ...
    "added_lights": added,
    "contract": {
        "purpose": "preview-only roof and high-bay readability; no lux authority",
