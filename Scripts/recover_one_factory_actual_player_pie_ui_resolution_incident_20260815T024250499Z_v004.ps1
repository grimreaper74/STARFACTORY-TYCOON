...
$RejectedV003Root = Join-Path $Root "Saved\Audits\OneFactory\v001\ActualPlayerPIE\IncidentRecovery\Incident_${SecondFailedStamp}_v003"
$RecoveryRoot = Join-Path $Root "Saved\Audits\OneFactory\v001\ActualPlayerPIE\IncidentRecovery\Incident_${SecondFailedStamp}_v004"
$PreRetryReceipt = Join-Path $RecoveryRoot 'pre_retry_evidence_v004.json'
$RetryConsole = Join-Path $RecoveryRoot 'fresh_retry_console_v004.log'
$RecoverySummary = Join-Path $RecoveryRoot 'retry_summary_v004.json'
...
function Assert-ExactDirectory([string]$Directory, [object]$Expected, [switch]$Png) {
    ...
    if (@(Compare-Object $ExpectedNames $ActualNames -CaseSensitive).Count -ne 0) { throw "Evidence inventory drift under $Directory" }
    $Rows = @()
    foreach ($Relative in $Expected.Keys) {
        ...
        if ($Png) {
            ...
            if ([uint32]$Dimensions.width -ne [uint32]$Pinned.width -or [uint32]$Dimensions.height -ne [uint32]$Pinned.height) {
                throw "Evidence PNG dimension drift: $Relative"
            }
            $Record.dimensions = @([uint32]$Dimensions.width, [uint32]$Dimensions.height)
        }
        $Rows += $Record
    }
    return @($Rows)
...
function Get-ProtectedSnapshot {
    ...
    foreach ($Path in @($Runner, $Validator, $BridgeHeader, $BridgeSource, $RejectedV003, $ParserRegression, $PSCommandPath)) {
        [void]$Paths.Add($Path)
    }
...
Assert-Leaf $PSCommandPath 'Corrected UI-resolution incident recovery script'
...
if ((Get-Sha256 $RejectedV003) -cne $ExpectedRejectedV003Sha256) { throw 'Rejected v003 wrapper is not preserved exactly' }
if ((Get-Sha256 $ParserRegression) -cne $ExpectedParserRegressionSha256) { throw 'PS5.1 PNG parser regression hash drift' }
...
if (Test-Path -LiteralPath $RejectedV003Root) { throw "Unexpected partial v003 recovery root appeared after the audited pre-root refusal: $RejectedV003Root" }
if (Test-Path -LiteralPath $RecoveryRoot) { throw "One-use v004 recovery destination already exists: $RecoveryRoot" }

$ParserRegressionOutput = @(& $ParserRegression)
if ($ParserRegressionOutput.Count -ne 1 `
        -or [string]$ParserRegressionOutput[0] -cne 'PASS__WINDOWS_POWERSHELL_5_1_PNG_IHDR_1920X1080_AND_1300X740_V004') {
    throw "PS5.1 PNG parser regression did not pass exactly: $($ParserRegressionOutput -join '; ')"
}
...
$PreRetry = [ordered]@{
    '$schema' = 'lineboss/audit/one-factory/actual-player-ui-resolution-incident-recovery-v004/v1'
    generated_utc = (Get-Date).ToUniversalTime().ToString('o')
    status = 'READY__V003_PRE_ROOT_PARSER_REFUSAL_PRESERVED__PS51_IHDR_REGRESSION_PASS__ONE_RETRY_NOT_STARTED'
    first_incident_stamp = $FirstFailedStamp
    second_incident_stamp = $SecondFailedStamp
    second_incident_cause = $ExactSecondCause
    rejected_v003 = [ordered]@{
        script = Get-Relative $RejectedV003
        sha256 = $ExpectedRejectedV003Sha256
        recovery_root_created = $false
        unreal_or_ubt_launched = $false
        refusal = 'Windows PowerShell 5.1 PNG IHDR byte-shift precedence bug in wrapper preflight'
    }
    parser_regression = [ordered]@{
        script = Get-Relative $ParserRegression
        sha256 = $ExpectedParserRegressionSha256
        powershell = $PSVersionTable.PSVersion.ToString()
        result = $ParserRegressionOutput[0]
        dimensions_proved = @(@(1920,1080), @(1300,740))
    }
...
try {
    ...
    $Success = [ordered]@{
        '$schema' = 'lineboss/audit/one-factory/actual-player-ui-resolution-incident-retry-summary-v004/v1'
        generated_utc = (Get-Date).ToUniversalTime().ToString('o')
        status = 'PASS__V003_PARSER_REFUSAL_PRESERVED__EXACTLY_ONE_NATIVE_SLATE_1920X1080_UMG_RETRY_PASSED'
    ...
    $FailureSummary = [ordered]@{
        '$schema' = 'lineboss/audit/one-factory/actual-player-ui-resolution-incident-retry-summary-v004/v1'
        generated_utc = (Get-Date).ToUniversalTime().ToString('o')
        status = 'FAIL__UI_RESOLUTION_INCIDENT_ONE_USE_RETRY_V004'
...
Write-Host 'PASS: preserved the v003 pre-root refusal and completed exactly one native 1920x1080 Slate/UMG retry.'
