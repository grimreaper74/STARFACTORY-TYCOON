"""Export deduplicated local-pivot authored meshes for exact v002 Unreal reconstruction."""
...
BLEND = SOURCE / "CA_MW_PressTrainA_AssemblyStudy_v002.blend"
MANIFEST_PATH = SOURCE / "PRESS_TRAIN_A_ASSEMBLY_STUDY_MANIFEST_v002.json"
STAGING = ROOT / "Saved/ImportStaging/PressTrainAAssemblyDetail_v009"
OUT = ROOT / "Saved/Audits/PressTrains/press_train_a_assembly_detail_staging_v009.json"
if STAGING.exists() or OUT.exists():
    raise RuntimeError("Refusing to overwrite v009 authored staging")
...
for record in manifest["instances"]:
    if not str(record["source_fbx"]).startswith("ASSEMBLY_STUDY"):
        continue
    ...
    if asset_name is None:
        asset_name = f"SM_CA_MW_PTA_Detail_{sig[:12]}_v009"
...
report = {
    "$schema": "cairnwell/import-staging/press-train-a-assembly-detail-v009/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PASS__DEDUPLICATED_V001_V002_AUTHORED_INSTANCE_STAGING__SOURCE_BLEND_UNCHANGED",
    "source_blend": str(BLEND.relative_to(ROOT)).replace("\\", "/"),
    "source_sha256": sha(BLEND),
