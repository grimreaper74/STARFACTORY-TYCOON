...
OUT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShop/PlayerBuildable_v915/clean_builder_runtime.png"
TAG = unreal.Name("LB.Capture.CleanBuilder.v915")
...
camera.set_actor_label("LB_CAPTURE_CleanBuilder_v915_NOT_SAVED")
camera.tags = [TAG]
...
def finish(ok, message):
    global handle
    (unreal.log if ok else unreal.log_error)(f"LINE_BOSS_CLEAN_BUILDER_CAPTURE_{'PASS' if ok else 'FAIL'} {message}")
...
def tick(_delta):
    global capture_started, runtime_audited
    now = time.monotonic()
    ...
    if not runtime_audited:
        ...
        unreal.log(f"LINE_BOSS_CLEAN_BUILDER_RUNTIME_MESHES count={len(meshes)} floors={len(floors)}")
        for roof in roofs:
            unreal.log(f"LINE_BOSS_CLEAN_BUILDER_RUNTIME_ROOF name={roof.get_name()} hidden={roof.get_editor_property('hidden')} component_visible={roof.static_mesh_component.is_visible()} z={roof.get_actor_location().z}")
        for sample in meshes[:8]:
            unreal.log(f"LINE_BOSS_CLEAN_BUILDER_RUNTIME_SAMPLE name={sample.get_name()} loc={sample.get_actor_location()} scale={sample.get_actor_scale3d()}")
        for floor in floors:
            component = floor.static_mesh_component
            unreal.log(
                f"LINE_BOSS_CLEAN_BUILDER_RUNTIME_FLOOR name={floor.get_name()} "
