"""Import the three GROUND DRONES as Nanite static meshes.
...
in the game's own white-futuristic language so the site does not mix
two art directions.
"""
...
source_dir = root / "SourceAssets/Candidate/Spacecraft/GroundDrones_Export_v001"
dest = "/Game/LineBoss/Candidates/Spacecraft/GroundDrones_v001"
out = root / "Saved/Audits/Spacecraft/ground_drones_import_v001.json"
if out.exists():
...
EXPECTED = {
    "SM_LB_GD_Lifter": ("x", 320),
    "SM_LB_GD_Assembly": ("x", 300),
    "SM_LB_GD_Sprayer": ("x", 320),
}
# Materials are authored in Unreal now (owner 2026-08-28), so these
# take the project palette rather than any imported map.
PALETTE = "/Game/LineBoss/Materials/Surfaces/MI_LB_Surface_PalePanel"
...
for name, (axis, target_cm) in sorted(EXPECTED.items()):
    ...
    rows.append({
        ...
        "provenance": "Meshy text-to-3D preview, generated 2026-08-28 "
                      "by Scripts/submit_meshy_site_scenery_v001/v002.ps1",
        "defining_axis": axis,
...
report = {
    "$schema": "lineboss/audit/ground-drones-import-v001/v1",
    ...
    "not_proven": [
        ...
        "WHEEL COUNT: the owner asked for six or eight wheels. Two "
        "generations (v004, v005) both returned four - the second with "
        "the wheel row made the subject of the prompt and the word "
        "SMALL repeated. The generator does not honour wheel counts. "
        "The bodies are low, wheeled and read as ground crew, which is "
        "the functional requirement; the count is the owner's call to "
        "accept or to have added in Blender.",
    ],
}
