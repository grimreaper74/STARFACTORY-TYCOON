...
# v010: v009 refused itself before spending - the tower prompt was 836
# characters against the API's 800 cap. Subjects trimmed; nothing else.
...
param(
    ...
    [string]$OutputRoot = 'C:\Users\greg_\Projects\LineBossCarFactory_Unreal 5.8\SourceAssets\Candidate\Spacecraft\StationDress_v010',
    [int]$PollSeconds = 20,
    [int]$TimeoutMinutes = 40
)

$ErrorActionPreference = 'Stop'
if ($Acknowledgement -ne 'I ACCEPT MESHY CREDIT SPEND FOR STATION DRESS V010') {
    throw 'Acknowledgement string does not match; refusing to spend credits.'
}
if (Test-Path -LiteralPath $OutputRoot) {
    throw "Result root already exists: $OutputRoot. Author v011 rather than rerunning."
}
...
$Jobs = @(
    @{ Name = 'Tower01_StationToolTower'
       Subject = 'A single freestanding INDUSTRIAL TOOL TOWER for a robot assembly bay: a tall rectangular cabinet on a low plinth, flat cap, one robot arm folded low on its front face, a vertical light strip recessed in that face. One tower only, no gantry, no arch.'
    ...
    @{ Name = 'Rack01_WallStorageRack'
       Subject = 'A long INDUSTRIAL PALLET RACK: two upright frames, three shelf beams, closed crates and a few cylinders on the shelves, flat ends so racks butt together in a row.'
    ...
    @{ Name = 'Light01_CeilingLightBar'
       Subject = 'A long INDUSTRIAL CEILING LIGHT FITTING: a slim rectangular housing, a flat diffuser strip underneath, two short hanging brackets on top, flat ends.'
    ...
    @{ Name = 'Cabinet01_StationToolCabinet'
       Subject = 'A low INDUSTRIAL TOOL AND CONTROL CABINET: a waist-high box on a plinth, an angled control panel on top, two closed doors on the front, a cable duct along the base.'
...
@{
    '$schema'      = 'lineboss/audit/meshy-station-dress-v010/v1'
