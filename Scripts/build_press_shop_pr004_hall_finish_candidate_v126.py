"""Build v126 directly from retained v124 with an attenuated v041 downlight grid."""
...
code = code.replace("v119", "v126").replace("V119", "V126")
...
for legacy_actor in actors_api.get_all_level_actors():
    ...
    if not legacy_label.startswith("LB_PR004_V041_Downlight_"):
        continue
    ...
    new_intensity = 320.0 if old_intensity <= 1000.0 else 440.0
    ...
    legacy_actor.tags = [unreal.Name(value) for value in dict.fromkeys(
        ...
        ["LB.Asset.Candidate.v126", "LB.Lighting.FactoryDownlightAttenuated"])]
...
code = code.replace(
    ...
    '"wall_wash_lights": wall_wash,\n    "attenuated_v041_downlights": attenuated_downlights,')
code = code.replace(
    "NONREPEATING_HALL_FINISH_AND_WALL_WASH_BUILT",
    "NONREPEATING_HALL_FINISH_AND_ATTENUATED_V041_DOWNLIGHT_GRID")
