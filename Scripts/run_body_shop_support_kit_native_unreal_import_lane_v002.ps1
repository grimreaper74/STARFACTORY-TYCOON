...
param(
    ...
    [ValidateSet('IMPORT_FROZEN_BODYSHOP_SUPPORT_KIT_NATIVE_V001_BASELINE_V002_ONCE')]
...
$Baseline = Join-Path $Root 'Scripts\body_shop_support_kit_native_unreal_import_baseline_v002.json'
$Freezer = Join-Path $Root 'Scripts\freeze_body_shop_support_kit_native_unreal_import_baseline_v002.py'
$Importer = Join-Path $Root 'Scripts\import_body_shop_support_kit_native_v001_lane_v002.py'
$Validator = Join-Path $Root 'Scripts\validate_body_shop_support_kit_native_v001_lane_v002.py'
$Destination = Join-Path $Root 'Content\LineBoss\Candidates\WeldShop\BodyShopSupportKitNative_v001'
$AuditRoot = Join-Path $Root 'Saved\Audits\BodyShop\SupportKitNative_v001\UnrealImportLane_v002'
...
$SummaryPath = Join-Path $RunRoot 'lane_summary_v002.json'
$ImportReceipt = Join-Path $RunRoot 'import_receipt_v002.json'
$ImportFailure = Join-Path $RunRoot 'import_failure_v002.json'
$ValidationReceipt = Join-Path $RunRoot 'fresh_load_validation_receipt_v002.json'
$ValidationFailure = Join-Path $RunRoot 'fresh_load_validation_failure_v002.json'

# Re-pinned after every intentional baseline/importer/validator/freezer change.
$ExpectedHashes = [ordered]@{
    baseline = 'E563879DC47887E5F99C9E7DD5D77308F080E6B0A7ECA2C185439669376A5915'
    freezer = '7D0CD8FD35637CB3528FC13A5DF138731FFB3883FBCBE777D6E237E365F08E9E'
    importer = '810007BC43A8C854DD4497571DA37D2A5462031EEF0C28F97327095D82344841'
    validator = 'D5F6BD6C997DA860BA19E08EB70234DDF3E1F7BA21162B236E02071572652072'
}
$ExpectedImportStatus = 'PASS__HASH_GUARDED_FROZEN_BODYSHOP_SUPPORT_KIT_NATIVE_V001_BASELINE_V002_UNREAL_INTAKE'
$ExpectedValidationStatus = 'PASS__INDEPENDENT_FRESH_PROCESS_LOAD_12_ASSETS_3_LODS_BODYSHOP_SUPPORT_KIT_NATIVE_V001_LANE_V002'
$RunRootEnvironmentName = 'LINEBOSS_BS_SUPPORT_KIT_NATIVE_V002_RUN_ROOT'
$AcknowledgementEnvironmentName = 'LINEBOSS_BS_SUPPORT_KIT_NATIVE_V002_ACK'
$ResultNames = @(
    'import_receipt_v002.json',
    'import_failure_v002.json',
...
function Assert-NoPriorLaneResult {
    ...
    if ($Found.Count -gt 0) {
        ...
        throw "One-shot lane v002 refuses every pre-existing result (PASS or FAIL): $Details"
    }
}
...
$Summary = [ordered]@{
        '$schema' = 'lineboss/audit/bodyshop-support-kit-native-v001-unreal-import-lane-summary/v2'
...
try {
    ...
    $PreflightText = Get-Content -Raw -LiteralPath $PreflightStdout
        if ($PreflightText -notmatch 'PASS__BODYSHOP_SUPPORT_KIT_NATIVE_V001_BASELINE_V002_MATCHES_SOURCE_AND_PROTECTED_FILES') {
        throw 'Offline immutable-baseline preflight PASS marker missing'
    }
    $Summary.preflight = $Preflight

    $env:LINEBOSS_BS_SUPPORT_KIT_NATIVE_V002_RUN_ROOT = $RunRoot
    $env:LINEBOSS_BS_SUPPORT_KIT_NATIVE_V002_ACK = $Acknowledgement
    Assert-NoActiveUnrealOrBuildProcess
    ...
    $Summary.status = 'PASS__HASH_GUARDED_IMPORT_AND_INDEPENDENT_FRESH_LOAD_BODYSHOP_SUPPORT_KIT_NATIVE_V001_LANE_V002'
}
catch {
    $Summary.status = 'FAIL_CLOSED__BODYSHOP_SUPPORT_KIT_NATIVE_V001_UNREAL_IMPORT_LANE_V002'
    ...
    Write-Output "LINE_BOSS_BODYSHOP_SUPPORT_KIT_NATIVE_V001_LANE_V002_SUMMARY=$SummaryPath"
}
