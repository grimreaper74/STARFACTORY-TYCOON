...
EXPECTED_IMPORT_STATUS = "PASS__HASH_GUARDED_FROZEN_BODYSHOP_SUPPORT_KIT_NATIVE_V001_UNREAL_INTAKE"

library = unreal.EditorAssetLibrary


def fail(message: str) -> None:
    raise RuntimeError("BODYSHOP_SUPPORT_KIT_NATIVE_FRESH_VALIDATION_V001_FAIL: " + message)
...
def main() -> None:
    ...
    evidence = {
        "$schema": "lineboss/audit/bodyshop-support-kit-native-v001-fresh-load-validation/v1",
        "generated_utc": now(),
    ...
    try:
        ...
        if (imported.get("$schema") != "lineboss/audit/bodyshop-support-kit-native-v001-unreal-import/v1"
        ...
        evidence.update({
            "status": "PASS__INDEPENDENT_FRESH_PROCESS_LOAD_12_ASSETS_3_LODS_BODYSHOP_SUPPORT_KIT_NATIVE_V001",
        ...
        unreal.log("LINE_BOSS_BODYSHOP_SUPPORT_KIT_NATIVE_V001_FRESH_LOAD_VALIDATION_PASS")
        print(json.dumps(evidence, indent=2))
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record = dict(evidence)
        record.update({
            "status": "FAIL_CLOSED__BODYSHOP_SUPPORT_KIT_NATIVE_V001_FRESH_LOAD_VALIDATION",
            "error": str(error),
            ...
            "recovery": "Preserve the importer/validator evidence and packages; do not rerun v001.",
        })
        failure_receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log_error("LINE_BOSS_BODYSHOP_SUPPORT_KIT_NATIVE_V001_VALIDATION_FAIL: " + str(error))
