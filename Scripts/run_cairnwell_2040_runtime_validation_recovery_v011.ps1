...
param(
    ...
    [ValidateSet('VALIDATE_CAIRNWELL_2040_RUNTIME_V001_V009_IMPORT_V011_ONCE')]
...
$RecoveryContract = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v011_contract.json'
$RecoverySidecar = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v011_contract.sha256'
$RecoveryTool = Join-Path $Root 'Scripts\prepare_cairnwell_2040_runtime_v001_recovery_v011.py'
$LogRetryHelper = Join-Path $Root 'Scripts\cairnwell_2040_runtime_log_retry_v003.ps1'
$Validator = Join-Path $Root 'Scripts\validate_cairnwell_2040_runtime_recovery_v011.py'
...
$RecoveryAuditRoot = Join-Path $AuditRoot 'Recovery_v011'
...
$SummaryPath = Join-Path $RunRoot 'lane_summary_recovery_v011.json'
$ValidationReceipt = Join-Path $RunRoot 'fresh_process_validation_receipt_recovery_v011.json'
$ValidationFailure = Join-Path $RunRoot 'fresh_process_validation_failure_recovery_v011.json'
$ValidationStatus = 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_DISTINCT_FRESH_PROCESS__READ_ONLY_RELOAD_OF_V009_PASS_IMPORT__11_PACKAGE_HASHES_UNCHANGED'
$SummaryPass = 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_GUARDED_VALIDATION_ONLY_OF_V009_PASS_IMPORT'
...
function Invoke-GuardedValidator {
    $LogPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v011.log'
    $StdoutPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v011.stdout.log'
    $StderrPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v011.stderr.log'
    $Arguments = @(
        ('"{0}"' -f $Project),
        $EngineEntry,
    ...
    if (-not $Exited) {
        ...
        throw 'Cairnwell v011 read-only validator timed out after 3600 seconds'
    }
    $Process.WaitForExit()
    $Process.Refresh()
    if ($null -eq $Process.ExitCode) {
        throw 'Cairnwell v011 read-only validator lost its process exit code'
    }
    $LogEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $LogPath -Label 'v011 validator abslog' -TimeoutMilliseconds 15000
    $StdoutEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $StdoutPath -Label 'v011 validator redirected stdout' -TimeoutMilliseconds 15000
    $StderrEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $StderrPath -Label 'v011 validator redirected stderr' -TimeoutMilliseconds 15000
    ...
    if ([int]$Process.ExitCode -ne 0 -or $FoundFatal.Count -gt 0) {
        throw "V011 validator failed strict exit/log/zero-UBT gate: exit=$($Process.ExitCode) fatal=$($FoundFatal -join ',')"
    }
    return [ordered]@{
...
foreach ($Path in @(
        ...
        $V009Contract,$V009Sidecar,$V010Contract,$V010Sidecar,
        $RecoveryContract,$RecoverySidecar,$RecoveryTool,$LogRetryHelper,$Validator)) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required prepared v011 validation input missing: $Path"
    }
}
...
if (Test-Path -LiteralPath $RecoveryAuditRoot) {
    throw "Recovery v011 is one-use and a result root already exists: $RecoveryAuditRoot"
}
...
$RecoveryHash = Assert-Sidecar $RecoveryContract $RecoverySidecar 'Validation-only recovery v011 contract'
$RecoveryPayload = Get-Content -Raw -LiteralPath $RecoveryContract | ConvertFrom-Json
if ([string]$RecoveryPayload.'$schema' -cne 'lineboss/cairnwell-2040-runtime-v001/recovery-contract/v11' `
        -or [string]$RecoveryPayload.status -cne 'FROZEN__CAIRNWELL_2040_RUNTIME_V001_INCIDENT_CHAINED_RECOVERY_V011__READY_FOR_ONE_SHOT_VALIDATION_ONLY_OF_V009_PASS_IMPORT' `
    ...
    throw 'Incident-bound v011 validation contract identity/safety drift'
}
...
$PreflightVerify = Invoke-RecoveryVerify '--verify-pre-validation' `
    ...
    'V011 pre-validation protected/source/v009/q6/package/v010 reverify'
Assert-NoProcesses

New-Item -ItemType Directory -Path $RunRoot | Out-Null
$Summary = [ordered]@{
    '$schema' = 'lineboss/audit/cairnwell-2040-runtime-v001/recovery-v011/validation-only-lane-summary/v11'
...
try {
    ...
    if (Test-Path -LiteralPath $ValidationFailure) {
        throw 'V011 read-only validator emitted a failure receipt despite strict exit gate'
    }
    if (-not (Test-Path -LiteralPath $ValidationReceipt -PathType Leaf)) {
        throw 'V011 read-only validator PASS receipt is absent'
    }
    Assert-NoProcesses
    $Summary.post_exit_reverify = Invoke-RecoveryVerify '--verify-post-validation' `
        ...
        'V011 post-exit exact receipt/log/package/source/protected reverify' $RunRoot
...
if (-not $Summary.environment_restoration_verified -and $null -eq $CaughtError) {
    $CaughtError = "V011 environment restoration failed: $($RestoreErrors -join ',')"
}
try {
    Assert-NoProcesses
}
...
if ($null -ne $CaughtError) {
    $Summary.status = 'FAIL_CLOSED__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_VALIDATION_ONLY_LANE'
...
try {
    $FinalVerify = Invoke-RecoveryVerify '--verify-final' `
        ...
        'V011 final exact five-file/summary/receipt/process/log/package reverify' $RunRoot
    Assert-NoProcesses
}
catch {
    $Summary.status = 'FAIL_CLOSED__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_VALIDATION_ONLY_LANE'
...
Write-Output 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_VALIDATION_ONLY_LANE'
Write-Output "LINE_BOSS_CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V011_SUMMARY=$SummaryPath"
