"""Capture fixed-camera evidence for the preserved mothballed v004 candidate."""
...
CAPTURES = {
    "whole": ("LB_CAM_PressShop_ManagementOverview", "press_shop_v004_mothballed_whole.png"),
    "front_end": ("LB_INT_PR004_V009_CAM_FrontEndDirty", "press_shop_v004_mothballed_front_end.png"),
    "pr004": ("LB_INT_PR004_V009_CAM_PR004CloseDirty", "press_shop_v004_mothballed_pr004.png"),
}
capture_id = os.environ.get("LB_PRESS_V004_CAPTURE", "whole").lower()
camera_label, filename = CAPTURES[capture_id]
output = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShopIntegration/v004_mothballed" / filename
...
if camera is None:
    raise RuntimeError(f"Missing camera {camera_label}")
...
def finish(_delta):
    ...
    if elapsed >= 3.0 and output.exists() and output.stat().st_size >= 1024:
        unreal.log(f"LINE_BOSS_PRESS_V004_CAPTURE_PASS id={capture_id} path={output}")
    elif elapsed < 55.0: return
    else: unreal.log_error(f"LINE_BOSS_PRESS_V004_CAPTURE_FAIL id={capture_id}")
