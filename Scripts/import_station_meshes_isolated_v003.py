"""import_station_meshes_isolated_v003.py - supersedes v002, which
mislabelled the owner's morning drop. Owner corrected: the model is the
ELECTRONICS STATION (catalogue ElectronicsStation, 12 x 9 m), so v002's
AssemblyStation assets carry a wrong name and a wrong baked scale. This
run imports the re-baked derivatives under the correct name and DELETES
the four mislabelled assets.
...
KEY = "ElectronicsStation"
BOOST = 1.14
FOOT_X, FOOT_Y = 1200.0, 900.0
...
# Remove the mislabelled v002 assets - nothing references them (the
# presenter was reverted before this run).
for stale in ("%s/SM_LB_ST_AssemblyStation_LOD0" % MESH_DIR,
              "%s/SM_LB_ST_AssemblyStation_LOD1" % MESH_DIR,
              "%s/MI_LB_AssemblyStation" % MAT_DIR,
              "%s/T_LB_AssemblyStation_BaseColor" % TEX_DIR,
              "%s/T_LB_AssemblyStation_Normal" % TEX_DIR,
              "%s/T_LB_AssemblyStation_MR" % TEX_DIR):
    if lib.does_asset_exist(stale):
        if lib.delete_asset(stale):
            unreal.log("DELETED mislabelled %s" % stale)
        else:
            raise RuntimeError("FAIL CLOSED: could not delete %s" % stale)
unreal.log("ELECTRONICS STATION IMPORT DONE: 2 LODs at 12x9, MI on v003 "
           "master, boost %.2f, mislabelled assets removed" % BOOST)
