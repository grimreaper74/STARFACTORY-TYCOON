"""Capture the fixed-exposure v002 support-robot material preview."""
...
CAMERA = "LB_CAM_SupportRobot_MaterialPreview_v002"
OUTPUT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/SupportRobots/Materials/Candidate_v002/lb_support_robot_materials_v002.png"
...
if not task.is_valid_task():
    raise RuntimeError("Invalid v002 material-preview screenshot task")
...
def finish(_delta):
    ...
    if elapsed >= 3.0 and OUTPUT.exists() and OUTPUT.stat().st_size >= 1024:
        unreal.log(f"LINE_BOSS_SUPPORT_ROBOT_MATERIAL_PREVIEW_V002_CAPTURE_PASS path={OUTPUT}")
    elif elapsed < 55.0:
        return
    else:
        unreal.log_error(f"LINE_BOSS_SUPPORT_ROBOT_MATERIAL_PREVIEW_V002_CAPTURE_FAIL path={OUTPUT}")
