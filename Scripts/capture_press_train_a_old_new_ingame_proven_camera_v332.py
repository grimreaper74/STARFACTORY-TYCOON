"""Capture matched old/new whole-shop views using retained v301's proven interior camera."""
import os
import time
...
label = "LB_V301_CAM_TrainAOperatorClear"
out = Path(unreal.Paths.project_saved_dir()) / f"ValidationScreenshots/PressShopIntegration/v332_train_a_old_new_ingame_proven_camera/train_a_{variant}_ingame.png"
...
def done(_):
    ...
    if elapsed >= 16 and out.exists() and out.stat().st_size >= 1024:
        unreal.log(f"LB_TRAIN_A_PROVEN_CAMERA_CAPTURE_PASS variant={variant} path={out}")
    elif elapsed < 90:
        return
    else:
        unreal.log_error(f"LB_TRAIN_A_PROVEN_CAMERA_CAPTURE_FAIL variant={variant}")
