"""Capture LB-CR01 v004 fixed-camera Unreal evidence."""
...
CAPTURES = {
    "oblique": ("LB_CR01_V004_CAM_Oblique", "lb_cr01_v004_oblique.png"),
    "side": ("LB_CR01_V004_CAM_Side", "lb_cr01_v004_side.png"),
    "top": ("LB_CR01_V004_CAM_Top", "lb_cr01_v004_top.png"),
}
capture_id = os.environ.get("LB_CR01_CAPTURE", "oblique").lower()
camera_label, filename = CAPTURES[capture_id]
output = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/LB_CR01/Candidate_v004" / filename
...
task = unreal.AutomationLibrary.take_high_res_screenshot(
    ...
    comparison_notes=f"LB-CR01 Candidate v004 {capture_id}", delay=0.0, force_game_view=True,
)
...
def finish(_delta):
    ...
    if elapsed >= 3.0 and output.exists() and output.stat().st_size >= 1024:
        unreal.log(f"LINE_BOSS_LB_CR01_V004_CAPTURE_PASS id={capture_id} path={output}")
    elif elapsed < 55.0:
        return
    else:
        unreal.log_error(f"LINE_BOSS_LB_CR01_V004_CAPTURE_FAIL id={capture_id}")
