"""Create v018 by calibrating v017 local task lights and fixture lenses."""
...
SOURCE = "/Game/LineBoss/Maps/LB_PressTrainAInstalledServiceCandidate_v017"
TARGET = "/Game/LineBoss/Maps/LB_PressTrainATaskFixtureCandidate_v018"
MAT_ROOT = "/Game/LineBoss/Candidates/PressTrains/Shared/Materials_v018"
OUT = ROOT / "Saved/Audits/PressTrains/press_train_a_task_fixture_v018.json"
...
if not levels.new_level_from_template(TARGET, SOURCE):
    raise RuntimeError(f"Could not create v018 from v017: {TARGET}")

path = f"{MAT_ROOT}/M_CA_MW_PT_TaskLens_v018"
material = library.load_asset(path) if library.does_asset_exist(path) else asset_tools.create_asset(
    "M_CA_MW_PT_TaskLens_v018", MAT_ROOT, unreal.Material, unreal.MaterialFactoryNew())
if material is None:
    raise RuntimeError(path)
mel.delete_all_material_expressions(material)
base = mel.create_material_expression(material, unreal.MaterialExpressionConstant3Vector, -320, -100)
base.set_editor_property("constant", unreal.LinearColor(0.075, 0.095, 0.088, 1.0))
rough = mel.create_material_expression(material, unreal.MaterialExpressionConstant, -320, 20)
rough.set_editor_property("r", 0.42)
emit = mel.create_material_expression(material, unreal.MaterialExpressionConstant3Vector, -320, 120)
emit.set_editor_property("constant", unreal.LinearColor(0.025, 0.050, 0.038, 1.0))
...
for actor in actors_api.get_all_level_actors():
    ...
    if "LB.Validation.LocalTaskLighting" in actor_tags:
        ...
        component.set_editor_property("intensity", 85.0)
    ...
    if "LB.PressTrain.TrainA.Isolated" in actor_tags:
        scope_count += 1
        if "LB.Asset.Candidate.v018" not in actor_tags:
            actor_tags.append("LB.Asset.Candidate.v018")
...
if not levels.save_current_level():
    failures.append("could not save v018 calibrated-task-fixture candidate")
library.save_directory(MAT_ROOT, only_if_is_dirty=False, recursive=True)
report = {
    "$schema": "cairnwell/audit/press-train-a-task-fixture-v018/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PASS__PRESS_TRAIN_A_V018_INSTALLED_SERVICE_PRESERVED_AND_TASK_FIXTURE_OUTPUT_CALIBRATED__EARLY_DRAW_CAMERA_GATE_REQUIRED__NOT_PROMOTED" if not failures else "FAIL__PRESS_TRAIN_A_V018_TASK_FIXTURE__NOT_PROMOTED",
    "source_map": SOURCE, "map": TARGET, "local_task_light_count": light_count,
    "local_task_light_intensity": 85.0, "fixture_lens_count": fixture_count,
