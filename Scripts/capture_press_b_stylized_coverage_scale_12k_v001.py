...
standard explicitly permits fixture intensity to scale for ceiling height and
covered area, but it also requires a player-view readability gate.  This
runtime-only probe leaves the B master settings intact, applies 12,000 lm to
the six already-proven B fixtures after the native rig has initialized, and
captures just the matching management hero view.  It is evidence for a scale
decision, not a promotion or a permanent lighting change.
"""
...
replacements = {
    "NativePressShopSteamPhotoLane_v005": "PressBStylizedCoverageScale12k_v001",
    "press_shop_steam_photo_lane_v005": "press_b_stylized_coverage_scale_12k_v001",
    "native-press-steam-photo-lane-v005": "press-b-stylized-coverage-scale-12k-v001",
    "PASS__NATIVE_PRESS_STEAM_PHOTO_CUTAWAY_PROBES": "PASS__PRESS_B_COVERAGE_SCALE_12K_V001",
    "FAIL__NATIVE_PRESS_STEAM_PHOTO_CUTAWAY_PROBES": "FAIL__PRESS_B_COVERAGE_SCALE_12K_V001",
...
def apply_coverage_scale(world):
    ...
    if len(fixtures) != 6:
        raise RuntimeError(
            f"PRESS_B_SCALE_PROBE_FAIL: expected six B fixtures, found {{len(fixtures)}}"
        )
    ...
    return f"B base 1200 lm scaled to {{FIXTURE_LUMENS:.0f}} lm per fixture for hall coverage"
