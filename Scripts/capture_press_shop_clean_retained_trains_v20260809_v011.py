"""Fixed-camera visual gate for the fresh clean inbound + retained Train A-D child."""
from pathlib import Path
from datetime import datetime,timezone
import json,time,unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True)
MAP="/Game/LineBoss/Maps/LB_PressShop_CleanInboundRetainedTrains_v20260809_v011"
ROOT=Path(unreal.Paths.project_dir()); OUT=ROOT/r"Saved\ValidationScreenshots\PressShopIntegration\clean_retained_trains_v20260809_v011"
AUDIT=ROOT/r"Saved\Audits\PressShopIntegration\clean_retained_trains_capture_v20260809_v011.json"
VIEWS=[
 ('press_trains_abcd_overhead.png',(4000,-20,1500),(4000,0,0),74.0),
 ('press_trains_abcd_west_to_east.png',(-2500,-5480,1250),(5000,0,340),70.0),
 ('press_trains_abcd_east_end.png',(10000,-5450,1050),(5500,0,300),66.0),
 ('inbound_storage_to_trains.png',(-10000,-5450,1450),(1200,0,240),72.0),
]
levels=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem); actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
if AUDIT.exists(): raise RuntimeError("Refusing overwrite")
...
for i,(_,loc,target,fov) in enumerate(VIEWS):
 c=actors.spawn_actor_from_class(unreal.CameraActor,unreal.Vector(*loc),unreal.Rotator()); c.set_actor_label(f'LB_TRANSIENT_CleanRetainedTrainCamera_{i+1:02d}')
...
def tick(_):
 ...
 AUDIT.write_text(json.dumps({'generated_utc':datetime.now(timezone.utc).isoformat(),'status':'CAPTURE_PASS__OWNER_VISUAL_REVIEW_REQUIRED' if passed else 'CAPTURE_FAIL','map':MAP,'captures':records,'map_saved_during_capture':False},indent=2),encoding='utf-8')
 (unreal.log if passed else unreal.log_error)('LINE_BOSS_CLEAN_RETAINED_TRAINS_CAPTURE_V011_'+('PASS' if passed else 'FAIL'))
