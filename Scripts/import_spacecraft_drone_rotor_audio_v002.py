"""Import the v002 placeholder drone rotor loop as a looping SoundWave.

Supersedes the v001 lane rather than rerunning it in place: v001
stays imported as evidence of what was tried and why it was not good
enough (harmonic partials only, so it sounded like an organ). The
presenter points at v002.
...
source_dir = root / "SourceAssets/Candidate/Spacecraft/Audio_v002"
manifest_path = source_dir / "spacecraft_audio_manifest_v002.json"
dest = "/Game/LineBoss/Audio/SFX"
out = root / "Saved/Audits/Spacecraft/spacecraft_drone_rotor_audio_import_v002.json"
...
if out.exists():
    raise RuntimeError(
        "Refusing to rerun: a receipt for v002 already exists. Author v002.")
...
report = {
    "$schema": "lineboss/audit/spacecraft-drone-rotor-audio-import-v002/v1",
