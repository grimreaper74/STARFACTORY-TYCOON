"""Capture one fixed-camera v022 wound-steel view per Unreal process."""
...
VIEWS = {
    "front": ("LB_INT_PR004_V009_CAM_FrontEndDirty", "press_shop_v022_front.png"),
    "pr004": ("LB_INT_PR004_V009_CAM_PR004CloseDirty", "press_shop_v022_pr004.png"),
}
key = os.environ.get("LB_PRESS_V022_CAPTURE", "front").lower()
camera_label, filename = VIEWS[key]
output = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShopIntegration/v022_wound_steel" / filename
...
def tick(_delta_seconds):
    ...
    if elapsed >= 3.0 and output.exists() and output.stat().st_size >= 1024:
        unreal.log(f"LINE_BOSS_PRESS_V022_CAPTURE_PASS view={key} output={output}")
    elif elapsed < 55.0:
        return
    else:
        unreal.log_error(f"LINE_BOSS_PRESS_V022_CAPTURE_FAIL view={key} output={output}")
