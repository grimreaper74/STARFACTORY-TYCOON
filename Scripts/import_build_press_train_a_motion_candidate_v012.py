"""Fresh v012 retry after the preserved v011 native-class reflection failure."""
...
DEST = "/Game/LineBoss/Candidates/PressTrains/TrainA/AssemblyStudyMotion_v012"
...
OUT = ROOT / "Saved/Audits/PressTrains/press_train_a_motion_build_v012.json"
SOURCE_MAP_FILE = ROOT / "Content/LineBoss/Maps/LB_PressTrainARuntimeCandidate_v010.umap"
TARGET_MAP_FILE = ROOT / "Content/LineBoss/Maps/LB_PressTrainAMotionCandidate_v012.umap"
...
if library.does_asset_exist(MAP) or library.does_directory_exist(DEST) or OUT.exists():
    raise RuntimeError("Refusing to overwrite Train A motion v012")
...
for role in material_roles:
    ...
    target_material = f"{MATERIAL_DEST}/M_CA_MW_PTA_{role}_Motion_v012"
...
if not levels.new_level_from_template(MAP, SOURCE_MAP):
    raise RuntimeError("Could not create v012 from retained native runtime v010")
...
for actor in actors_api.get_all_level_actors():
    add_tags(actor, "LB.PressTrain.TrainA.Motion.v012", "LB.Asset.Candidate.v012",
...
for record in manifest["instances"]:
    ...
    actor.set_actor_label(record["name"] + "_UEv012")
    add_tags(actor,
             "LB.PressTrain.TrainA.Motion.v012", "LB.Asset.Candidate.v012", "LB.Asset.CandidateNotPromoted",
...
add_tags(authorities[0], "LB.PressTrain.TrainA.Motion.v012")
...
live_hmis[0].set_actor_label("CA_MW_PTA_HMI_LiveState_v012")

if not levels.save_current_level():
    raise RuntimeError("Could not save Train A motion v012")
library.save_directory(DEST, only_if_is_dirty=False, recursive=True)
if not TARGET_MAP_FILE.exists():
    raise RuntimeError("Train A motion v012 map file missing after save")
if sha(SOURCE_MAP_FILE) != protected_source_hash:
    raise RuntimeError("Protected v010 source map changed during v012 build")
...
report = {
    "$schema": "cairnwell/audit/press-train-a-motion-build-v012/v1",
