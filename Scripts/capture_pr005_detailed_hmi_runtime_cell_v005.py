...
OUT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShop/PR005_DetailedHMI_v001/pr005_runtime_cell_overview_v005.png"
TAG = unreal.Name("LB.Capture.PR005DetailedHMI.RuntimeCell.v005")
...
camera.set_actor_label("LB_CAPTURE_PR005_DETAILED_HMI_OVERVIEW_V005_NOT_SAVED")
camera.tags = [TAG]
...
def tick(_delta):
    ...
    if not runtime_checked:
        ...
        unreal.log(
            ...
            f"visible={visible} mesh={mesh} collision={collision} "
            f"materials={materials} transform={hmi.get_relative_transform()}"
        )
        if not visible or mesh is None or collision != unreal.CollisionEnabled.NO_COLLISION:
            finish(False, "HMI visibility/collision assertion failed")
            return
