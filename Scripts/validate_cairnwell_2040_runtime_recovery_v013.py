"""Read-only fresh-process validation for the completed v009 Cairnwell import."""
...
def run_root() -> Path:
    ...
    if os.environ.get(ACK_ENV, "").strip() != recovery.RUN_ACK_TOKEN:
        raise RuntimeError("v013 guarded-run acknowledgement absent")
    path = Path(raw).resolve()
    if (path.parent != recovery.RECOVERY_AUDIT_ROOT.resolve()
            or not re.fullmatch(r"\d{8}T\d{6}Z-[0-9a-f]{8}", path.name)
            or not path.is_dir()):
        raise RuntimeError("v013 run root escapes or is absent: " + str(path))
    return path
...
def main() -> None:
    root = run_root()
    receipt = root / recovery.VALIDATION_RECEIPT
    failure = root / recovery.VALIDATION_FAILURE
    evidence = {
        "$schema": recovery.result_topology()["validation"]["$schema"],
    ...
    try:
        ...
        if receipt.exists() or failure.exists():
            raise RuntimeError("current v013 run already contains a validation result")
        baseline, contract, state = load_validation_baseline()
        command_line = str(unreal.SystemLibrary.get_command_line())
        cache_flag_matches = re.findall(
            r"(?i)(?:^|\s)-NoAssetRegistryCacheWrite(?:\s|$)", command_line)
        if len(cache_flag_matches) != 1:
            raise RuntimeError(
                "v013 requires exactly one -NoAssetRegistryCacheWrite command-line flag")
        cache_before = copy.deepcopy(state["asset_registry_cache"])
        legacy_cache_absence_before = (
            recovery.verify_legacy_cache_deletion_surface_absent())

        imported, _summary, quarantine = recovery.prior.prior.prior.validate_v009_receipts(
            state["v009"])
        import_pid = int(imported.get("process_id", -1))
        if import_pid != 36612 or import_pid == os.getpid():
            raise RuntimeError("v013 validator is not distinct from exact v009 import process")
        if core.sha256(
                recovery.prior.prior.prior.V009_RUN / "import_receipt_recovery_v009.json") \
                != recovery.prior.prior.prior.V009_IMPORT_RECEIPT_SHA256:
        ...
        if (namespace_before != imported.get("namespace_disk_files")
            ...
            raise RuntimeError("v009 package bytes changed before v013 fresh asset loads")
        ...
        if registry_before != expected_registry:
            raise RuntimeError("v013 pre-load asset-registry closure drift")
        ...
        cache_after = recovery.prior.verify_asset_registry_cache_snapshot()
        legacy_cache_absence_after = (
            recovery.verify_legacy_cache_deletion_surface_absent())
        if package_hashes_after != package_hashes_before:
            raise RuntimeError("runtime package bytes changed during v013 read-only validation")
        if namespace_after != namespace_before:
            raise RuntimeError("runtime namespace changed during v013 read-only validation")
        if registry_after != registry_before:
            raise RuntimeError("asset-registry closure changed during v013 read-only validation")
        if (source_after != source_before or protected_after != protected_before
                or prepared_lane_after != prepared_lane_before):
            raise RuntimeError("source/protected/prepared lane changed during v013 validation")
        if cache_after != cache_before:
            raise RuntimeError(
                "Intermediate/CachedAssetRegistry changed during v013 validation")
        if legacy_cache_absence_after != legacy_cache_absence_before:
            raise RuntimeError(
                "legacy AssetRegistry cache deletion surface changed during v013 validation")

        evidence.update({
            "status": recovery.VALIDATION_PASS,
            ...
            "v010_recovery_contract_sha256": (
                recovery.prior.prior.V010_CONTRACT_SHA256),
            "v011_recovery_contract_sha256": recovery.prior.V011_CONTRACT_SHA256,
            "v011_failure_receipt_sha256": recovery.prior.V011_RUN_FILES[
                recovery.prior.V011_FAILURE][1],
            "v011_incident_binding_sha256": state[
                "v011_incident"]["binding_sha256"],
            "v012_recovery_contract_sha256": recovery.V012_CONTRACT_SHA256,
            "v012_validation_receipt_sha256": recovery.V012_RECEIPT_SHA256,
            "v012_wrapper_failure_summary_sha256": recovery.V012_SUMMARY_SHA256,
            "v012_wrapper_incident_binding_sha256": state[
                "v012_incident"]["binding_sha256"],
            "v009_recovery_contract_sha256": (
                recovery.prior.prior.prior.V009_CONTRACT_SHA256),
            "v009_import_receipt_sha256": (
                recovery.prior.prior.prior.V009_IMPORT_RECEIPT_SHA256),
            "v009_wrapper_failure_summary_sha256": (
                recovery.prior.prior.prior.V009_SUMMARY_SHA256),
            "v009_quarantine_receipt_sha256": (
                recovery.prior.prior.prior.V009_QUARANTINE_RECEIPT_SHA256),
            ...
            "quarantine_receipt": {
                "path": core.relative(
                    recovery.prior.prior.prior.V009_RUN
                    / "quarantine_receipt_v009.json"),
                "bytes": recovery.prior.prior.V009_QUARANTINE_RECEIPT_BYTES,
                "sha256": (
                    recovery.prior.prior.prior.V009_QUARANTINE_RECEIPT_SHA256),
            ...
            "vehicle_model_id": recovery.prior.VEHICLE_MODEL_ID,
            "production_recipe_id": recovery.prior.DEVELOPMENT_RECIPE_ID,
            "current_geometry_authority_id": (
                recovery.prior.CURRENT_GEOMETRY_AUTHORITY_ID),
            "failures": [],
        })
        ...
        recovery.validate_v013_receipt(evidence, contract, state, root)
        ...
        evidence.update({
            "status": (
                "FAIL_CLOSED__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_"
        ...
        unreal.log_error(
            "LINE_BOSS_CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_VALIDATION_FAIL: "
            + str(error))
        print(json.dumps(evidence, indent=2))
        raise
