"""v087 adapter for the established PR-009 process/motion/save/authority PIE validator."""
...
if token not in code: raise RuntimeError("v087 motion-bounds injection token missing")
...
if token not in code: raise RuntimeError("v087 motion sampling injection token missing")
code = code.replace(token, injection, 1)
...
if token not in code: raise RuntimeError("v087 payload injection token missing")
code = code.replace(token, injection, 1)
