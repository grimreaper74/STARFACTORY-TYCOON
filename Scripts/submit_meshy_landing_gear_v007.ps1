...
param(
    ...
    [string]$OutputRoot = 'C:\Users\greg_\Projects\LineBossCarFactory_Unreal 5.8\SourceAssets\Candidate\Spacecraft\LandingGear_v007',
    [int]$PollSeconds = 20,
    [int]$TimeoutMinutes = 25
)

$ErrorActionPreference = 'Stop'
if ($Acknowledgement -ne 'I ACCEPT MESHY CREDIT SPEND FOR LANDING GEAR V007') {
    throw 'Acknowledgement string does not match; refusing to spend credits.'
}
if (Test-Path -LiteralPath $OutputRoot) {
    throw "Result root already exists: $OutputRoot. Author v008 rather than rerunning."
}
...
$Jobs = @(
    # v006 returned a TRIPOD LANDER FOOT with no wheel at all. The words
    # "spacecraft" and "leg" together pull a generator straight to a
    # lunar-lander pad, and "fork" did not pull it back.
    #
    # Three changes, each aimed at that failure:
    #  - the subject is now an AIRLINER nose wheel assembly, an object
    #    with one unambiguous shape, rather than a spacecraft part;
    #  - the wheel is named FIRST, because whatever comes first is what
    #    the generator builds around;
    #  - what it must NOT be is stated outright, since v006 proves the
    #    lander reading is the default rather than an accident.
    @{ Name = 'Gear01_NoseLeg'
       Subject = 'A single rubber tyred WHEEL in a two-sided fork at the foot of a slim vertical hydraulic oleo strut, with a torque link and a diagonal drag brace: an airliner NOSE WHEEL assembly.'
       Anchor  = 'Scale: waist height, wheel about motorcycle sized. A WHEELED leg - not a tripod, not a lander foot, not a pad, no splayed feet.' }
)
...
@{
    '$schema'      = 'lineboss/audit/meshy-landing-gear-v007/v1'
