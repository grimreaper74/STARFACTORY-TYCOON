...
import time,unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True);MAP="/Game/LineBoss/Developer/Validation/LB_InboundCoilDeliveryOrientation_v503";OUT=Path(unreal.Paths.project_saved_dir())/"ValidationScreenshots/PressShopIntegration/inbound_coil_delivery_v503/inbound_orientation_test.png";levels=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);actors=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
if not levels.load_level(MAP):raise RuntimeError("Could not load v503")
cam=next((a for a in actors.get_all_level_actors() if a.get_actor_label()=="LB_CAM_InboundCoilDelivery_Orientation_v503"),None)
if cam is None:raise RuntimeError("Missing v503 camera")
...
def done(_dt):
 ...
 unreal.log(("LINE_BOSS_INBOUND_ORIENTATION_V503_CAPTURE_PASS " if OUT.exists() and OUT.stat().st_size>1024 else "LINE_BOSS_INBOUND_ORIENTATION_V503_CAPTURE_FAIL ")+str(OUT));unreal.SystemLibrary.quit_editor()
