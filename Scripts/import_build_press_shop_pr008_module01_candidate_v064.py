"""Import detailed Pro PR-008 Module 01 into isolated candidate v064."""
import json
...
RECORDS = json.loads((SOURCE / "pr008_module01_entry_loop_manifest_v001.json").read_text(encoding="utf-8"))
BASE = "/Game/LineBoss/Maps/LB_PressShop_PR008ProEntryLoopCandidate_v063"
MAP = "/Game/LineBoss/Maps/LB_PressShop_PR008Module01Candidate_v064"
DEST = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Module01"
MAT = "/Game/LineBoss/Stations/Press/PR008/Detailed_v001/Materials"
PREFIX = "LB_PR008_V064_"
DATUM = unreal.Vector(-500.0, -2000.0, 0.0)
YAW = -90.0
OUT = ROOT / "Saved/Audits/press_shop_pr008_module01_candidate_v064.json"
...
def make_material(name, colour, metallic, roughness):
    path = f"{MAT}/{name}"
    asset = library.load_asset(path) if library.does_asset_exist(path) else tools.create_asset(
...
materials = {
    "charcoal": make_material("M_CA_MW_PR008_FoundryCharcoal_v001", (0.0144, 0.0176, 0.0219), 0.72, 0.43),
    "green": make_material("M_CA_MW_PR008_CairnwellGreen_v001", (0.0137, 0.0704, 0.0578), 0.52, 0.46),
    "yellow": make_material("M_CA_MW_PR008_SafetyYellow_v001", (0.8879, 0.5333, 0.0), 0.28, 0.48),
    "steel": make_material("M_CA_MW_PR008_GroundSteel_v001", (0.27, 0.30, 0.32), 0.92, 0.25),
    "galv": make_material("M_CA_MW_PR008_Galvanised_v001", (0.37, 0.40, 0.41), 0.84, 0.43),
    "rubber": make_material("M_CA_MW_PR008_Rubber_v001", (0.015, 0.018, 0.019), 0.05, 0.72),
    "sensor": make_material("M_CA_MW_PR008_SensorGlass_v001", (0.01, 0.055, 0.075), 0.38, 0.18),
    "white": make_material("M_CA_MW_PR008_LabelPlate_v001", (0.68, 0.70, 0.67), 0.14, 0.60),
    "red": make_material("M_CA_MW_PR008_EStopRed_v001", (0.55, 0.006, 0.003), 0.10, 0.40),
}
...
def choose_material(slot_name):
    ...
    if "groundsteel" in value or "steel" in value:
        return materials["steel"]
    if "galvan" in value:
        return materials["galv"]
    if "rubber" in value:
...
map_file = ROOT / "Content/LineBoss/Maps/LB_PressShop_PR008Module01Candidate_v064.umap"
if not map_file.exists():
    if not library.duplicate_asset(BASE, MAP):
        raise RuntimeError("Could not duplicate v063 to v064")
    if not library.save_asset(MAP, only_if_is_dirty=False):
        raise RuntimeError("Could not save prepared v064 map")
    unreal.log("LINE_BOSS_PR008_V064_PREPARE_PASS__RERUN_FOR_POPULATION")
...
# Hide only the corresponding engineering cage in this derivative.
cage = next((actor for actor in actors_api.get_all_level_actors()
             if actor.get_actor_label() == "LB_PR008_V062_01_EntryGuideLoopControl"), None)
if cage is None:
    raise RuntimeError("Missing Pro Module 01 engineering cage")
...
for record in RECORDS:
    ...
    if not isinstance(mesh, unreal.StaticMesh):
        raise RuntimeError(f"Missing detailed Module 01 mesh {record['name']}")
    ...
    actor.tags = [unreal.Name(value) for value in (
        "LB.Asset.Candidate.v064", "LB.Asset.CandidateNotPromoted", "LB.Station.PR008",
        "LB.Module.PR008.01.EntryLoop", "LB.Authority.CairnwellRemainingMachineryPack.v1")]
    ...
    placements.append({
        "actor": actor.get_actor_label(), "role": record["role"], "movable": record["movable"],
        "world_location_cm": [world.x, world.y, world.z], "collision": record["collision"]})


def text(label, value, location, z_size, colour):
    actor = actors_api.spawn_actor_from_class(
        unreal.TextRenderActor, unreal.Vector(*location), unreal.Rotator(yaw=180))
    actor.set_actor_label(PREFIX + "TEXT_" + label)
    actor.tags = [unreal.Name(value) for value in (
        "LB.Asset.Candidate.v064", "LB.Asset.CandidateNotPromoted", "LB.Station.PR008.Identity")]
    component = actor.text_render
    component.set_text(value)
    component.set_world_size(z_size)
...
identity = [
    text("Brand", "CAIRNWELL / MOORCROSS", (-1007.0, -2000.0, 158.5), 2.7, unreal.Color(45, 130, 105, 255)),
    text("Station", "PR-008  ENTRY / LOOP", (-1007.0, -2000.0, 151.5), 3.1, unreal.Color(35, 38, 40, 255)),
]
...
def camera(label, location, target, fov):
    ...
    actor.tags = [unreal.Name(value) for value in (
        "LB.Camera.Validation", "LB.Camera.Fixed.PR008.v064", "LB.Asset.CandidateNotPromoted")]
...
cameras = [
    camera("Module01Operator", (-1450, -2920, 360), (-945, -2000, 105), 49),
    camera("Module01Inspection", (-1300, -2200, 300), (-960, -2000, 115), 50),
    camera("Module01Elevated", (-1550, -3100, 720), (-945, -2000, 100), 54),
    camera("Module01Connected", (-2050, -3600, 850), (-1150, -2000, 115), 58),
]
...
for actor in created:
    ...
    values_min = [origin.x - extent.x, origin.y - extent.y, origin.z - extent.z]
    values_max = [origin.x + extent.x, origin.y + extent.y, origin.z + extent.z]
    mins = [min(mins[i], values_min[i]) for i in range(3)]
    maxs = [max(maxs[i], values_max[i]) for i in range(3)]
expected_min = [-1007.5, -2115.0, 12.5]
expected_max = [-882.5, -1885.0, 177.5]
within = all(mins[i] >= expected_min[i] - 0.1 and maxs[i] <= expected_max[i] + 0.1 for i in range(3))
if not within:
    raise RuntimeError(f"Imported Module 01 exceeds Pro envelope: min={mins} max={maxs}")
...
payload = {
    "$schema": "line-boss/audit/press-shop-pr008-module01-candidate-v064/v1",
    "generated_utc": datetime.now(timezone.utc).isoformat(),
    "status": "PRO_MODULE01_DETAILED_IMPORT_ASSEMBLY_AND_ENVELOPE_CONTAINMENT_PASS__RUNTIME_COLLISION_NAVIGATION_AND_VISUAL_GATES_REQUIRED__NOT_PROMOTED",
    "map": MAP, "base_map": BASE, "source": str(SOURCE),
    "module_id": "01", "semantic_actor_count": len(created),
    "moving_actor_count": sum(1 for record in RECORDS if record["movable"]),
    "placements": placements,
    "measured_world_bounds_cm": {"min": mins, "max": maxs},
    "expected_world_envelope_cm": {"min": expected_min, "max": expected_max},
    "within_pro_envelope": within,
    "hidden_engineering_cage": cage.get_actor_label(),
...
unreal.log(f"LINE_BOSS_PR008_V064_BUILD_PASS actors={len(created)} moving={payload['moving_actor_count']}")
unreal.SystemLibrary.quit_editor()
