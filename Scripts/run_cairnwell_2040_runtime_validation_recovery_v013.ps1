...
param(
    ...
    [ValidateSet('VALIDATE_CAIRNWELL_2040_RUNTIME_V001_V009_IMPORT_V013_ONCE')]
...
$V012Contract = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v012_contract.json'
$V012Sidecar = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v012_contract.sha256'
$RecoveryContract = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v013_contract.json'
$RecoverySidecar = Join-Path $Root 'Scripts\cairnwell_2040_runtime_v001_recovery_v013_contract.sha256'
$RecoveryTool = Join-Path $Root 'Scripts\prepare_cairnwell_2040_runtime_v001_recovery_v013.py'
$LogRetryHelper = Join-Path $Root 'Scripts\cairnwell_2040_runtime_log_retry_v003.ps1'
$Validator = Join-Path $Root 'Scripts\validate_cairnwell_2040_runtime_recovery_v013.py'
...
$RecoveryAuditRoot = Join-Path $AuditRoot 'Recovery_v013'
...
$SummaryPath = Join-Path $RunRoot 'lane_summary_recovery_v013.json'
$ValidationReceipt = Join-Path $RunRoot 'fresh_process_validation_receipt_recovery_v013.json'
$ValidationFailure = Join-Path $RunRoot 'fresh_process_validation_failure_recovery_v013.json'
$ValidationStatus = 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_DISTINCT_FRESH_PROCESS__READ_ONLY_RELOAD_OF_V009_PASS_IMPORT__EXACT_PERSISTED_DEPENDENCIES__ZERO_CACHE_DELETION_OR_WRITE__11_PACKAGE_HASHES_UNCHANGED'
$SummaryPass = 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_GUARDED_VALIDATION_ONLY_OF_V009_PASS_IMPORT'
...
function Invoke-GuardedValidator {
    $LogPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v013.log'
    $StdoutPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v013.stdout.log'
    $StderrPath = Join-Path $RunRoot 'fresh_process_validation_recovery_v013.stderr.log'
    $Arguments = @(
        ('"{0}"' -f $Project),
        $EngineEntry,
    ...
    if (-not $Exited) {
        ...
        throw 'Cairnwell v013 read-only validator timed out after 3600 seconds'
    }
    $Process.WaitForExit()
    $Process.Refresh()
    if ($null -eq $Process.ExitCode) {
        throw 'Cairnwell v013 read-only validator lost its process exit code'
    }
    $LogEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $LogPath -Label 'v013 validator abslog' -TimeoutMilliseconds 15000
    $StdoutEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $StdoutPath -Label 'v013 validator redirected stdout' -TimeoutMilliseconds 15000
    $StderrEvidence = Get-LBFileEvidenceWithBoundedReadRetry `
        -Path $StderrPath -Label 'v013 validator redirected stderr' -TimeoutMilliseconds 15000
    ...
    if ([int]$Process.ExitCode -ne 0 -or $FoundFatal.Count -gt 0) {
        throw "V013 validator failed strict exit/log/zero-UBT gate: exit=$($Process.ExitCode) fatal=$($FoundFatal -join ',')"
    }
    $PreLoadLine = 'CleanupOrphanedCacheFiles (PreLoad): 1 .ref files found, 1 referenced binaries'
    $PostWriteLine = 'CleanupOrphanedCacheFiles (PostWrite): 1 .ref files found, 1 referenced binaries'
    $ZeroMutationLine = 'CleanupOrphanedCacheFiles: 1 total binaries, 1 referenced (kept), 0 old-style pre-migration (kept), 0 orphans deleted, 0 orphans locked (kept)'
    $PerPrimaryLog = [ordered]@{}
    foreach ($Entry in @(
            [pscustomobject]@{ Name = 'fresh_process_validation_recovery_v013.log'; Text = $LogEvidence.text },
            [pscustomobject]@{ Name = 'fresh_process_validation_recovery_v013.stdout.log'; Text = $StdoutEvidence.text })) {
        $Lines = @([Regex]::Split([string]$Entry.Text, "`r?`n"))
        $PreLoadIndices = @()
        $PostWriteIndices = @()
        for ($Index = 0; $Index -lt $Lines.Count; $Index++) {
            if ($Lines[$Index].EndsWith($PreLoadLine, [StringComparison]::Ordinal)) { $PreLoadIndices += $Index }
            if ($Lines[$Index].EndsWith($PostWriteLine, [StringComparison]::Ordinal)) { $PostWriteIndices += $Index }
        }
        if ($PreLoadIndices.Count -ne 1 -or $PostWriteIndices.Count -ne 1) {
            throw "V013 cache cleanup exact phase-count drift: $($Entry.Name)"
        }
        foreach ($PhaseIndex in @($PreLoadIndices[0], $PostWriteIndices[0])) {
            if ($PhaseIndex + 1 -ge $Lines.Count `
                    -or -not $Lines[$PhaseIndex + 1].EndsWith(
                        $ZeroMutationLine, [StringComparison]::Ordinal)) {
                throw "V013 cache cleanup exact adjacent zero-mutation result drift: $($Entry.Name)"
            }
        }
        $ZeroMutationCount = @($Lines | Where-Object {
            $_.EndsWith($ZeroMutationLine, [StringComparison]::Ordinal) }).Count
        if ($ZeroMutationCount -ne 2) {
            throw "V013 cache cleanup exact zero-mutation result count drift: $($Entry.Name)"
        }
        $PerPrimaryLog[$Entry.Name] = [ordered]@{
    ...
    $CleanupEvidence = [ordered]@{
        classification = 'UE58_CLEAR_CACHE_POSTWRITE_INFORMATIONAL_CLEANUP__ONE_REFERENCED_BINARY_KEPT__ZERO_ORPHANS_DELETED_OR_LOCKED__NO_CACHE_WRITE'
        per_primary_log = $PerPrimaryLog
    ...
    return [ordered]@{
        ...
        asset_registry_cache_cleanup = $CleanupEvidence
    }
}
...
foreach ($Path in @(
        ...
        $V012Contract,$V012Sidecar,
        $RecoveryContract,$RecoverySidecar,$RecoveryTool,$LogRetryHelper,$Validator)) {
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Required prepared v013 validation input missing: $Path"
    }
}
...
if (-not (Test-Path -LiteralPath (Join-Path $AuditRoot 'Recovery_v012') -PathType Container)) {
    throw 'Consumed recovery v012 wrapper-failure evidence root is absent'
}
if (Test-Path -LiteralPath $RecoveryAuditRoot) {
    throw "Recovery v013 is one-use and a result root already exists: $RecoveryAuditRoot"
}
...
$V012Hash = Assert-Sidecar $V012Contract $V012Sidecar 'Consumed failed-wrapper recovery v012 contract'
$RecoveryHash = Assert-Sidecar $RecoveryContract $RecoverySidecar 'Validation-only recovery v013 contract'
$RecoveryPayload = Get-Content -Raw -LiteralPath $RecoveryContract | ConvertFrom-Json
if ([string]$RecoveryPayload.'$schema' -cne 'lineboss/cairnwell-2040-runtime-v001/recovery-contract/v13' `
        -or [string]$RecoveryPayload.status -cne 'FROZEN__CAIRNWELL_2040_RUNTIME_V001_INCIDENT_CHAINED_RECOVERY_V013__READY_FOR_ONE_SHOT_VALIDATION_ONLY_OF_V009_PASS_IMPORT' `
        ...
        -or [string]$RecoveryPayload.failed_v012_wrapper.run_id -cne '20260815T163553Z-3dabbd20' `
        -or [string]$RecoveryPayload.failed_v012_wrapper.validation_receipt.sha256 -cne '4C6ADEB0C3C94AD227EDAACB735552895BB1B1C2B08077C3A1F9566E18FD0F65' `
        -or [string]$RecoveryPayload.failed_v012_wrapper.summary.sha256 -cne '5BDB57399DE607028E2A5E57B127374D25718E4F73B1FEF69379A55B057BB596' `
        -or -not [bool]$RecoveryPayload.failed_v012_wrapper.asset_registry_cache_before_after_and_current_exact `
        -or -not [bool]$RecoveryPayload.cache_cleanup_adjudication.post_exit_cache_snapshot_must_equal_pre_validation `
        -or -not [bool]$RecoveryPayload.legacy_asset_registry_cache_deletion_surface.exact_post_exit_absence_required `
        -or [int]$RecoveryPayload.legacy_asset_registry_cache_deletion_surface.exact_pre_validation_absence.matching_path_count -ne 0 `
        -or [string]$RecoveryPayload.environment_restoration_correction.absent_restore_value -cne '[System.Management.Automation.Language.NullString]::Value' `
        ...
        -or [bool]$RecoveryPayload.policy.v012_rerun_authorized `
        -or -not [bool]$RecoveryPayload.policy.generic_postwrite_cleanup_token_is_not_fatal `
        -or -not [bool]$RecoveryPayload.policy.exact_zero_deletion_cleanup_result_required `
        -or -not [bool]$RecoveryPayload.policy.actual_cache_write_or_delete_tokens_are_fatal `
        -or -not [bool]$RecoveryPayload.policy.legacy_cache_deletion_surface_absence_required `
        -or -not [bool]$RecoveryPayload.policy.nullstring_absent_environment_restoration_required `
        -or -not [bool]$RecoveryPayload.policy.strict_environment_restoration_equality_required `
    ...
    throw 'Incident-bound v013 validation contract identity/safety drift'
}
...
if ($V012Hash -cne '975A59EED82A8CC0574406ACEE707262C23A173259B1504D4E370586E6E126CB') {
    throw 'Consumed failed-wrapper v012 recovery contract hash drift'
}
$PreflightVerify = Invoke-RecoveryVerify '--verify-pre-validation' `
    ...
    'V013 pre-validation protected/source/v009/q6/package/v010 reverify'
Assert-NoProcesses

New-Item -ItemType Directory -Path $RunRoot | Out-Null
$Summary = [ordered]@{
    '$schema' = 'lineboss/audit/cairnwell-2040-runtime-v001/recovery-v013/validation-only-lane-summary/v13'
    ...
    v012_recovery_contract_sha256 = $V012Hash
    ...
    v012_validation_receipt_sha256 = '4C6ADEB0C3C94AD227EDAACB735552895BB1B1C2B08077C3A1F9566E18FD0F65'
    v012_wrapper_failure_summary_sha256 = '5BDB57399DE607028E2A5E57B127374D25718E4F73B1FEF69379A55B057BB596'
    ...
    post_exit_asset_registry_cache = $null
    post_exit_legacy_asset_registry_cache_absence = $null
...
try {
    ...
    if (Test-Path -LiteralPath $ValidationFailure) {
        throw 'V013 read-only validator emitted a failure receipt despite strict exit gate'
    }
    if (-not (Test-Path -LiteralPath $ValidationReceipt -PathType Leaf)) {
        throw 'V013 read-only validator PASS receipt is absent'
    }
    Assert-NoProcesses
    $Summary.post_exit_reverify = Invoke-RecoveryVerify '--verify-post-validation' `
        ...
        'V013 post-exit exact receipt/log/package/source/protected reverify' $RunRoot
    ...
    $Summary.post_exit_asset_registry_cache = $RecoveryPayload.asset_registry_cache_write_suppression.exact_pre_validation_snapshot
    $Summary.post_exit_legacy_asset_registry_cache_absence = $RecoveryPayload.legacy_asset_registry_cache_deletion_surface.exact_pre_validation_absence
}
catch {
    $CaughtError = $_
}
    ...
    foreach ($Row in $RestoreRows) {
        try {
            if ($null -eq $Row.Value) {
                ...
                [Environment]::SetEnvironmentVariable(
                    $Row.Name, [string]$Row.Value, 'Process')
            }
        }
        catch {
...
$ActualRunEnvironment = [Environment]::GetEnvironmentVariable($RunEnvironment, 'Process')
$ActualAckEnvironment = [Environment]::GetEnvironmentVariable($AckEnvironment, 'Process')
$ActualSkipUbtEnvironment = [Environment]::GetEnvironmentVariable($SkipUbtEnvironment, 'Process')
$RunRestored = if ($null -eq $OldRunEnvironment) {
    $null -eq $ActualRunEnvironment
} else { $ActualRunEnvironment -ceq $OldRunEnvironment }
$AckRestored = if ($null -eq $OldAckEnvironment) {
    $null -eq $ActualAckEnvironment
} else { $ActualAckEnvironment -ceq $OldAckEnvironment }
$SkipUbtRestored = if ($null -eq $OldSkipUbtEnvironment) {
    $null -eq $ActualSkipUbtEnvironment
} else { $ActualSkipUbtEnvironment -ceq $OldSkipUbtEnvironment }
...
if (-not $Summary.environment_restoration_verified -and $null -eq $CaughtError) {
    $CaughtError = "V013 environment restoration failed: $($RestoreErrors -join ',')"
}
try {
    Assert-NoProcesses
}
...
if ($null -ne $CaughtError) {
    $Summary.status = 'FAIL_CLOSED__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_VALIDATION_ONLY_LANE'
...
try {
    $FinalVerify = Invoke-RecoveryVerify '--verify-final' `
        ...
        'V013 final exact five-file/summary/receipt/process/log/package reverify' $RunRoot
    Assert-NoProcesses
}
catch {
    $Summary.status = 'FAIL_CLOSED__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_VALIDATION_ONLY_LANE'
...
Write-Output 'PASS__CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_VALIDATION_ONLY_LANE'
Write-Output "LINE_BOSS_CAIRNWELL_2040_RUNTIME_V001_RECOVERY_V013_SUMMARY=$SummaryPath"
