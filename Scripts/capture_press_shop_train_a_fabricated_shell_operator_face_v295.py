"""Capture one v295 operator-face shell comparison per clean process."""
import os, time
from pathlib import Path
import unreal
unreal.EditorPythonScripting.set_keep_python_script_alive(True)
MAP = "/Game/LineBoss/Maps/LB_PressShop_TrainAFabricatedShellOperatorFaceCandidate_v295"
CAPTURES = {"operator": ("LB_V295_CAM_TrainAOperator", "v295_train_a_operator.png"), "fabrication": ("LB_V295_CAM_TrainAFabrication", "v295_train_a_fabrication.png"), "overview": ("LB_V295_CAM_TrainAOverview", "v295_train_a_overview.png")}
capture_id = os.environ.get("LB_V295_CAPTURE", "operator").lower()
if capture_id not in CAPTURES: raise RuntimeError(capture_id)
label, filename = CAPTURES[capture_id]; output = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots/PressShopIntegration/v295_train_a_fabricated_shell" / filename
...
task = unreal.AutomationLibrary.take_high_res_screenshot(1920, 1080, str(output), camera=camera, mask_enabled=False, capture_hdr=False, comparison_tolerance=unreal.ComparisonTolerance.LOW, comparison_notes=f"Cairnwell v295 operator-face fabricated shell {capture_id}", delay=0, force_game_view=True)
if not task.is_valid_task(): raise RuntimeError("invalid screenshot task")
started = time.monotonic(); handle = None
def finish(_delta):
    global handle
    elapsed = time.monotonic() - started
    if elapsed >= 3 and output.exists() and output.stat().st_size >= 1024: unreal.log(f"LB_V295_CAPTURE_PASS id={capture_id} path={output}")
    elif elapsed < 70: return
    else: unreal.log_error(f"LB_V295_CAPTURE_FAIL id={capture_id} path={output}")
