"""One-shot UE 5.8 importer for the frozen clean-room vehicle-WIP family."""
...
PASS_STATUS = "PASS__HASH_GUARDED_FRESH_IMPORT__16_NATIVE_VEHICLE_ROLES__48_AUTHORED_LODS__V001"
...
def append_custom_lods(meshes: dict, baseline: dict, subsystem) -> dict:
    ...
    evidence = {"name": lane.INTERCHANGE_FBX_CVAR, "previous_value": previous,
                ...
                "custom_lods_requested": 32, "custom_lods_imported": [],
    ...
    try:
        ...
        if evidence["disabled_value"] != 0:
            lane.fail("could not activate legacy FBX custom-LOD importer")
        for key in sorted(meshes):
            for lod_index in (1, 2):
                ...
                evidence["custom_lods_imported"].append({"asset": key, "lod": lod_index,
    ...
    if import_error is not None:
        raise import_error
    if len(evidence["custom_lods_imported"]) != 32:
        lane.fail("authored custom LOD append count drift")
    return evidence


def configure_mesh(key: str, spec: dict, subsystem, mesh) -> None:
    ...
    if (int(unreal.EditorStaticMeshLibrary.get_simple_collision_count(mesh)) != 0
        ...
        lane.fail("fresh moving-WIP mesh unexpectedly contains collision: " + key)
    if lane.slot_names(mesh) != spec["lods"][0]["material_slots"]:
        lane.fail("LOD0 semantic material slots drift: " + key)
...
def main() -> None:
    ...
    record = {"$schema": "lineboss/audit/vehicle-wip-native-kit-v001/unreal-import/v1",
    ...
    try:
        ...
        if lane.DEST_DISK.exists() or lane.library.does_directory_exist(lane.DEST):
            lane.fail("target native namespace already exists; overwrite/reimport forbidden")
        if lane.library.list_assets(lane.DEST, recursive=True, include_folder=False):
            lane.fail("target native namespace already exists in asset registry")
        ...
        for key in keys:
            configure_mesh(key, baseline["assets"][key], subsystem, meshes[key])
        ...
        if registry != wanted_registry or set(disk) != wanted_disk:
            lane.fail("exact 16-package native registry/disk inventory drift")
        ...
        if source_after != source_before or protected_after != protected_before or prepared_lane_after != prepared_lane_before:
            lane.fail("protected source/project/lane changed during import")
        record.update({"status": PASS_STATUS, "baseline_sha256": lane.sha256(lane.BASELINE),
                       ...
                       "asset_count": 16, "lod_count_per_asset": 3, "source_fbx_count": 48,
                       "custom_lods_appended": 32, "new_material_or_texture_assets": 0,
                       "strict_per_asset_monotonic_triangles_verified": True,
                       "exact_one_uv_channel_per_lod_verified": True,
                       "zero_collision_all_moving_wip_verified": True,
                       "nanite_off_all_assets": True, "failures": [],
                       "automatic_cleanup": "NOT_PERFORMED__PARTIAL_ARTIFACTS_PRESERVED_FOR_EXPLICIT_REVIEW"})
        receipt.write_text(json.dumps(record, indent=2) + "\n", encoding="utf-8")
        unreal.log("LINE_BOSS_VEHICLE_WIP_NATIVE_KIT_V001_IMPORT_PASS")
        unreal.SystemLibrary.quit_editor()
    except Exception as error:
        record.update({"status": "FAIL_CLOSED__VEHICLE_WIP_NATIVE_KIT_V001_UNREAL_IMPORT",
        ...
        unreal.log_error("LINE_BOSS_VEHICLE_WIP_NATIVE_KIT_V001_IMPORT_FAIL: " + str(error))
        try:
            unreal.SystemLibrary.quit_editor()
        finally:
            raise
