"""Read-only v089 static/simple collision, binding and isolation audit."""
import json
import sys
...
from press_shop_pr009_transfer_guide_collision_v089_config import TARGET_MAP, STATIC_DEST, MOVING_DEST
...
fixed = [actor for actor in actors if unreal.Name("LB.Collision.FixedChassis.v089") in actor.tags]
movers = [actor for actor in actors if unreal.Name("LB.Collision.SelectiveMover.v089") in actor.tags]
modular = [actor for actor in actors if actor.get_actor_label().startswith("LB_PR009_V089_MOD_")]
...
guide_rows = [row for row in interface_rows if "TransferGuides_01" in row["actor"]]
...
if any("COMPLEX_AS_SIMPLE" in (row["trace_flag"] or "").upper() for row in static_rows + fixed_rows + mover_rows + interface_rows):
    failures.append("complex-as-simple remains in v089 release scope")
...
if len(guide_rows) != 1 or guide_rows[0]["primitives"] != {"box":2,"sphere":0,"capsule":0,"convex":0,"total":2}:
    failures.append(f"authored open-channel transfer-guide collision mismatch {guide_rows}")
elif "COMPLEX_AS_SIMPLE" in (guide_rows[0]["trace_flag"] or "").upper() or "BLOCKALL" not in (guide_rows[0]["profile"] or "").upper():
    failures.append(f"transfer-guide collision semantics mismatch {guide_rows[0]}")
...
for build_row in build["static_groups"]:
    ...
    if not all(proof[key] for key in ("vertices_match","triangles_match","bounds_match","materials_match")): failures.append(f"visual duplicate mismatch {build_row['group']}")
...
payload = {"$schema":"cairnwell/audit/pr009-release-collision-static-v089/v1","generated_utc":datetime.now(timezone.utc).isoformat(),
           ...
           "authored_open_channel_transfer_guides":guide_rows,
...
unreal.log(f"PR009_V089_STATIC_COLLISION_{'PASS' if not failures else 'FAIL'} output={out}")
