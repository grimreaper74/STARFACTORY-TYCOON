...
bool FLBNativeAGVInboundSourceModeTest::RunTest(const FString& Parameters)
{
    ...
    TestFalse(TEXT("A Meshy-like path is rejected even when it names an AGV"),
    ...
    TestEqual(TEXT("Source-aware inbound save schema is v6"), NativeSave.SaveVersion, 6);
    TestEqual(TEXT("Native source mode persists"), NativeSave.SourceMode,
        ELBInboundDeliverySourceMode::NativeAGVArrival);
    TestTrue(TEXT("Native v6 save restores into the same proved source mode"),
