 ...
 */
class AGENTZETACTIONS_API FAgentZetWidgetActions : public IAgentZetActionExecutor
{
public:
	FAgentZetWidgetActions();
	virtual ~FAgentZetWidgetActions();
...
private:
	...
	FAgentZetActionResult ExecuteCreateWidgetBlueprint(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Add a widget to the tree hierarchy. */
	FAgentZetActionResult ExecuteAddWidget(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Configure the layout slot of a widget (anchors, offsets, padding, alignment, fill). */
	FAgentZetActionResult ExecuteSetWidgetSlot(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Set a property on a named widget via reflection. */
	FAgentZetActionResult ExecuteSetWidgetProperty(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Set font properties on a text widget (FSlateFontInfo). */
	FAgentZetActionResult ExecuteSetWidgetFont(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Set a brush (image/texture/solid color) on Image, Button, Border, ProgressBar widgets. */
	FAgentZetActionResult ExecuteSetWidgetBrush(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Bind a widget event (OnClicked, etc.) to a K2 event node in the EventGraph. */
	FAgentZetActionResult ExecuteBindWidgetEvent(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Remove a widget from the tree. */
	FAgentZetActionResult ExecuteRemoveWidget(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Read-only: return the full widget tree hierarchy as JSON. */
	FAgentZetActionResult ExecuteGetWidgetTree(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);

	/** Compile a Widget Blueprint and return errors/warnings. */
	FAgentZetActionResult ExecuteCompileWidgetBlueprint(const TSharedRef<FJsonObject>& Params, FAgentZetActionResult& Result);
	...
	static UWidgetBlueprint* LoadWidgetBP(const FString& AssetPath, FAgentZetActionResult& Result);

	/** Find a widget by name in a Widget Blueprint, adding error to Result if not found. */
	static UWidget* FindWidgetByName(UWidgetBlueprint* WidgetBP, const FString& WidgetName, FAgentZetActionResult& Result);
