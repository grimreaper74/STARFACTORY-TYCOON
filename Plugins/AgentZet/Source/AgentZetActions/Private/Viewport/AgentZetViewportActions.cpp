...
#define LOCTEXT_NAMESPACE "AgentZetViewportActions"

// ============================================================================
// Lifecycle
// ============================================================================

FAgentZetViewportActions::FAgentZetViewportActions() {}
FAgentZetViewportActions::~FAgentZetViewportActions() {}
...
FName FAgentZetViewportActions::GetActionName() const { return FName(TEXT("Viewport")); }
FText FAgentZetViewportActions::GetDisplayName() const { return LOCTEXT("DisplayName", "Viewport Capture"); }
EAgentZetActionCategory FAgentZetViewportActions::GetCategory() const { return EAgentZetActionCategory::General; }
EAgentZetRiskLevel FAgentZetViewportActions::GetDefaultRiskLevel() const { return EAgentZetRiskLevel::Low; }
bool FAgentZetViewportActions::CanUndo() const { return false; }
bool FAgentZetViewportActions::UndoAction() { return false; }

TArray<FString> FAgentZetViewportActions::GetSupportedToolNames() const
{
	return { TEXT("capture_viewport") };
}

bool FAgentZetViewportActions::ValidateParams(const TSharedRef<FJsonObject>& Params, TArray<FString>& OutErrors) const
{
	// No required params — all optional
	return true;
}

FAgentZetActionPlan FAgentZetViewportActions::PreviewAction(const TSharedRef<FJsonObject>& Params)
{
...
FAgentZetActionResult FAgentZetViewportActions::ExecuteAction(const TSharedRef<FJsonObject>& Params)
{
...
FAgentZetActionResult FAgentZetViewportActions::ExecuteCaptureViewport(
	...
	FAgentZetActionResult& Result)
{
	...
	UE_LOG(LogAgentZet, Log, TEXT("ViewportActions: Captured viewport %dx%d (%d pixels)"), Width, Height, Pixels.Num());
	...
	UE_LOG(LogAgentZet, Log, TEXT("ViewportActions: Base64 output: %d chars (~%d tokens)"), Base64Len, EstimatedTokens);
...
FString FAgentZetViewportActions::EncodePixelsToBase64(
	...
	int32 JpegQuality)
{
	...
	{
		UE_LOG(LogAgentZet, Error, TEXT("ViewportActions: Failed to create JPEG ImageWrapper"));
		return FString();
	}
	...
	{
		UE_LOG(LogAgentZet, Error, TEXT("ViewportActions: Failed to set raw pixel data on ImageWrapper"));
		return FString();
	}
	...
	{
		UE_LOG(LogAgentZet, Error, TEXT("ViewportActions: JPEG compression returned empty data"));
		return FString();
	}

	UE_LOG(LogAgentZet, Log, TEXT("ViewportActions: JPEG encoded %dx%d Q%d -> %d bytes (vs PNG would be ~%dx larger)"),
