...
FAgentZetGeminiClient::FAgentZetGeminiClient()
...
void FAgentZetGeminiClient::SetApiKey(const FString& InApiKey) { ApiKey = InApiKey; }
void FAgentZetGeminiClient::SetModel(const FString& InModelId) { ModelId = InModelId; }
void FAgentZetGeminiClient::SetBaseUrl(const FString& InBaseUrl) { BaseUrl = InBaseUrl; }
void FAgentZetGeminiClient::SetMaxTokens(int32 InMaxTokens) { MaxTokens = InMaxTokens; }
void FAgentZetGeminiClient::SetThinkingBudget(int32 InBudget) { ThinkingBudgetTokens = InBudget; }
void FAgentZetGeminiClient::SetReasoningEffort(EAgentZetReasoningEffort InEffort) { ReasoningEffort = InEffort; }

FString FAgentZetGeminiClient::ReasoningEffortToGeminiString(EAgentZetReasoningEffort Effort)
{
...
void FAgentZetGeminiClient::SendMessage(
	...
	const TArray<TSharedPtr<FJsonObject>>& ToolSchemas)
{
	if (bRequestInFlight)
	{
		UE_LOG(LogAgentZet, Warning, TEXT("GeminiClient: Request already in flight. Ignoring."));
		return;
	}

	if (ApiKey.IsEmpty())
	{
		UE_LOG(LogAgentZet, Error, TEXT("GeminiClient: API key not set."));
		ErrorReceivedDelegate.Broadcast(FAgentZetHTTPError::ConnectionFailed(TEXT("Google Gemini")));
	...
	CurrentRequest->OnRequestProgress64().BindRaw(this, &FAgentZetGeminiClient::HandleRequestProgress);
	CurrentRequest->OnProcessRequestComplete().BindRaw(this, &FAgentZetGeminiClient::HandleRequestComplete);

	if (CurrentRequest->ProcessRequest())
	{
		RequestStartedDelegate.Broadcast();
		UE_LOG(LogAgentZet, Log, TEXT("GeminiClient: Request started — Model=%s"), *ModelId);
	}
	else
	{
		bRequestInFlight = false;
...
void FAgentZetGeminiClient::CancelRequest()
{
...
bool FAgentZetGeminiClient::IsRequestInFlight() const
{
	return bRequestInFlight;
}

TSharedPtr<FJsonObject> FAgentZetGeminiClient::BuildRequestBody(
	...
	const TArray<TSharedPtr<FJsonObject>>& ToolSchemas) const
{
	...
	else if (bIsEffortModel && ReasoningEffort != EAgentZetReasoningEffort::Disabled)
	{
...
TArray<TSharedPtr<FJsonValue>> FAgentZetGeminiClient::ConvertContents(
...
TSharedPtr<FJsonObject> FAgentZetGeminiClient::ConvertToolSchemas(
	const TArray<TSharedPtr<FJsonObject>>& ToolSchemas) const
{
...
void FAgentZetGeminiClient::HandleRequestProgress(
...
void FAgentZetGeminiClient::HandleRequestComplete(
	FHttpRequestPtr Request, FHttpResponsePtr Response, bool bConnected)
{
	...
	if (Code != 200)
	{
		FAgentZetHTTPError Err = FAgentZetHTTPError::FromStatusCode(Code, Response->GetContentAsString(), TEXT("Google Gemini"));
...
void FAgentZetGeminiClient::ProcessResponseChunk(const FString& JsonLine)
{
...
void FAgentZetGeminiClient::ProcessCandidateParts(const TArray<TSharedPtr<FJsonValue>>& Parts)
{
	for (const TSharedPtr<FJsonValue>& PartVal : Parts)
	{
		...
		if ((*PartObj)->TryGetObjectField(TEXT("functionCall"), FuncCall))
		{
			...
			UE_LOG(LogAgentZet, Log, TEXT("GeminiClient: Tool call: %s"), *FuncName);
		}
...
void FAgentZetGeminiClient::ExtractTokenUsage(const TSharedPtr<FJsonObject>& UsageMetadata)
{
...
void FAgentZetGeminiClient::FinalizeResponse()
{
	if (bRequestCancelled) return;
	...
	for (const FAgentZetToolCall& TC : PendingToolCalls)
	{
