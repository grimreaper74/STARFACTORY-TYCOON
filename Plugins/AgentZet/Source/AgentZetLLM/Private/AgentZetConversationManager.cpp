...
FAgentZetConversationManager::FAgentZetConversationManager()
...
FAgentZetMessage& FAgentZetConversationManager::AddUserMessage(const FString& Content)
{
	FAgentZetMessage& Msg = History.AddDefaulted_GetRef();
	Msg.MessageId = FGuid::NewGuid();
	Msg.Role = EAgentZetMessageRole::User;
	Msg.Content = Content;
...
FAgentZetMessage& FAgentZetConversationManager::AddAssistantMessage(const FString& Content)
{
	FAgentZetMessage& Msg = History.AddDefaulted_GetRef();
	Msg.MessageId = FGuid::NewGuid();
	Msg.Role = EAgentZetMessageRole::Assistant;
	Msg.Content = Content;
	Msg.Timestamp = FDateTime::UtcNow();
	OnMessageAdded.Broadcast(Msg);
	return Msg;
...
FAgentZetMessage& FAgentZetConversationManager::AddAssistantMessageFull(const FAgentZetMessage& SourceMessage)
{
	FAgentZetMessage& Msg = History.AddDefaulted_GetRef();
...
FAgentZetMessage& FAgentZetConversationManager::AddSystemMessage(const FString& Content)
{
	FAgentZetMessage& Msg = History.AddDefaulted_GetRef();
	Msg.MessageId = FGuid::NewGuid();
	Msg.Role = EAgentZetMessageRole::System;
	Msg.Content = Content;
	Msg.Timestamp = FDateTime::UtcNow();
	OnMessageAdded.Broadcast(Msg);
	return Msg;
...
FAgentZetMessage& FAgentZetConversationManager::AddToolResultMessage(const FString& ToolUseId, const FString& Content, bool bIsError)
{
	FAgentZetMessage& Msg = History.AddDefaulted_GetRef();
	Msg.MessageId = FGuid::NewGuid();
	Msg.Role = EAgentZetMessageRole::ToolResult;
	Msg.Content = Content;
...
int32 FAgentZetConversationManager::InjectSyntheticToolResultsForOrphans()
{
	...
	for (const FAgentZetMessage& Msg : History)
	{
		...
		else if (Msg.Role == EAgentZetMessageRole::ToolResult && !Msg.ToolUseId.IsEmpty())
		{
	...
	for (const FString& UseId : AllToolUseIds)
	{
		if (!AllToolResultIds.Contains(UseId))
		{
			...
			FAgentZetMessage& SyntheticMsg = History.AddDefaulted_GetRef();
			SyntheticMsg.MessageId = FGuid::NewGuid();
			SyntheticMsg.Role = EAgentZetMessageRole::ToolResult;
...
void FAgentZetConversationManager::AppendStreamingText(const FGuid& MessageId, const FString& DeltaText)
{
	FAgentZetMessage* Msg = GetMessageById(MessageId);
	if (Msg)
	{
...
void FAgentZetConversationManager::FinalizeStreamingMessage(const FGuid& MessageId)
{
	FAgentZetMessage* Msg = GetMessageById(MessageId);
	if (Msg)
	{
...
void FAgentZetConversationManager::EvictOldToolResults(TArray<FAgentZetMessage>& Messages)
{
	...
	{
		if (Messages[i].Role == EAgentZetMessageRole::Assistant)
		{
			LastAssistantIdx = i;
			break;
		}
	...
	{
		if (Messages[i].Role == EAgentZetMessageRole::ToolResult
	...
	{
		FAgentZetMessage& Msg = Messages[i];
		if (Msg.Role != EAgentZetMessageRole::ToolResult) continue;
		...
		{
			if (Messages[j].Role == EAgentZetMessageRole::Assistant)
			{
				bConsumed = true;
				break;
			}
...
TArray<FAgentZetMessage> FAgentZetConversationManager::GetEffectiveHistory() const
{
	...
	if (LastSummaryIdx != -1)
	{
		...
		TArray<FAgentZetMessage> MessagesFromSummary(History.GetData() + LastSummaryIdx, History.Num() - LastSummaryIdx);
		...
		for (const FAgentZetMessage& Msg : MessagesFromSummary)
		{
			if (Msg.Role == EAgentZetMessageRole::Assistant && !Msg.ContentBlocksJson.IsEmpty())
			{
		...
		TArray<FAgentZetMessage> Filtered;
		for (const FAgentZetMessage& Msg : MessagesFromSummary)
		{
		...
		for (const FAgentZetMessage& Msg : Filtered)
		{
	...
	if (Result.Num() > 0)
	{
		...
		for (const FAgentZetMessage& R : Result)
		{
			if (R.Role == EAgentZetMessageRole::ToolResult && !R.ToolUseId.IsEmpty())
			{
		...
		{
			if (Result[i].Role == EAgentZetMessageRole::Assistant && !Result[i].ContentBlocksJson.IsEmpty())
			{
		...
		for (int32 i = 0; i < Result.Num(); ++i)
		{
			...
			const FAgentZetMessage& Msg = Result[i];
			if (Msg.Role != EAgentZetMessageRole::Assistant || Msg.ContentBlocksJson.IsEmpty()) continue;
...
int32 FAgentZetConversationManager::TruncateConversation(float FracToRemove)
{
	...
	FAgentZetMessage TruncationMarker;
	TruncationMarker.MessageId = FGuid::NewGuid();
	TruncationMarker.Role = EAgentZetMessageRole::System;
...
void FAgentZetConversationManager::CleanupAfterTruncation()
{
...
TArray<FAgentZetMessage> FAgentZetConversationManager::GetPrunedHistory(int32 MaxTokens) const
{
	TArray<FAgentZetMessage> Pruned;
	int32 TotalTokens = 0;
...
void FAgentZetConversationManager::ClearHistory()
{
	History.Empty();
}

int32 FAgentZetConversationManager::EstimateTotalTokens() const
{
	int32 Total = 0;
	for (const FAgentZetMessage& Msg : History)
	{
...
void FAgentZetConversationManager::SetMaxTokenBudget(int32 InMaxTokens)
{
	MaxTokenBudget = FMath::Max(1000, InMaxTokens);
}

FAgentZetMessage* FAgentZetConversationManager::GetMessageById(const FGuid& MessageId)
{
	return History.FindByPredicate([&MessageId](const FAgentZetMessage& Msg)
	{
		return Msg.MessageId == MessageId;
	});
}

FAgentZetMessage* FAgentZetConversationManager::GetLastMessage()
{
	return History.Num() > 0 ? &History.Last() : nullptr;
}

int32 FAgentZetConversationManager::EstimateMessageTokens(const FAgentZetMessage& Message)
{
	return (Message.Content.Len() / ApproxCharsPerToken) + 10;
}

FString FAgentZetConversationManager::GenerateUniqueId()
{
...
FString FAgentZetConversationManager::GetConversationSaveDir()
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("AgentZet"), TEXT("Conversations"));
}

FString FAgentZetConversationManager::GetTasksBaseDir()
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("AgentZet"), TEXT("Tasks"));
}

TSharedPtr<FJsonObject> FAgentZetConversationManager::MessageToJson(const FAgentZetMessage& Message)
{
	...
	switch (Message.Role)
	{
	case EAgentZetMessageRole::User: RoleStr = TEXT("user"); break;
	case EAgentZetMessageRole::Assistant: RoleStr = TEXT("assistant"); break;
	case EAgentZetMessageRole::System: RoleStr = TEXT("system"); break;
	case EAgentZetMessageRole::ToolResult: RoleStr = TEXT("tool_result"); break;
	case EAgentZetMessageRole::Error: RoleStr = TEXT("error"); break;
...
FAgentZetMessage FAgentZetConversationManager::MessageFromJson(const TSharedPtr<FJsonObject>& JsonObj)
{
	FAgentZetMessage Msg;
	...
	{
		if (RoleStr == TEXT("user")) Msg.Role = EAgentZetMessageRole::User;
		else if (RoleStr == TEXT("assistant")) Msg.Role = EAgentZetMessageRole::Assistant;
		else if (RoleStr == TEXT("system")) Msg.Role = EAgentZetMessageRole::System;
		else if (RoleStr == TEXT("tool_result")) Msg.Role = EAgentZetMessageRole::ToolResult;
		else if (RoleStr == TEXT("error")) Msg.Role = EAgentZetMessageRole::Error;
	}
...
bool FAgentZetConversationManager::SaveSession(const FString& FilePath) const
{
	...
	if (bSuccess)
	{
		UE_LOG(LogAgentZet, Log, TEXT("ConversationManager: Saved %d messages to %s"),
			MessagesArray.Num(), *FilePath);
	}
	else
	{
		UE_LOG(LogAgentZet, Error, TEXT("ConversationManager: Failed to save to %s"), *FilePath);
	}

	return bSuccess;
}

bool FAgentZetConversationManager::SaveApiHistory(const FString& FilePath) const
{
	// Ensure directory exists
	FString Dir = FPaths::GetPath(FilePath);
	IFileManager::Get().MakeDirectory(*Dir, true);
	...
	if (bSuccess)
	{
		UE_LOG(LogAgentZet, Log, TEXT("ConversationManager: Saved %d API messages to %s"),
			MessagesArray.Num(), *FilePath);
	}
	else
	{
		UE_LOG(LogAgentZet, Error, TEXT("ConversationManager: Failed to save API history to %s"), *FilePath);
	}

	return bSuccess;
}

bool FAgentZetConversationManager::LoadSession(const FString& FilePath)
{
	if (!FPaths::FileExists(FilePath))
	{
		UE_LOG(LogAgentZet, Warning, TEXT("ConversationManager: File not found: %s"), *FilePath);
		return false;
	}
	...
	{
		UE_LOG(LogAgentZet, Error, TEXT("ConversationManager: Failed to read: %s"), *FilePath);
		return false;
	}
	...
	{
		UE_LOG(LogAgentZet, Error, TEXT("ConversationManager: Failed to parse JSON: %s"), *FilePath);
		return false;
	}
	...
	if (Root->TryGetArrayField(TEXT("messages"), MessagesArray))
	{
		for (const TSharedPtr<FJsonValue>& MsgVal : *MessagesArray)
		{
			...
			{
				FAgentZetMessage Msg = MessageFromJson(*MsgObj);
				History.Add(Msg);
			}
		}
	}

	UE_LOG(LogAgentZet, Log, TEXT("ConversationManager: Loaded %d messages from %s"),
		History.Num(), *FilePath);

	return true;
}

FString FAgentZetConversationManager::ExportAsText() const
{
	FString Output;
	Output += TEXT("=== AgentZet Conversation Export ===\n");
	...
	for (const FAgentZetMessage& Msg : History)
	{
		...
		switch (Msg.Role)
		{
		case EAgentZetMessageRole::User: RolePrefix = TEXT("[USER]"); break;
		case EAgentZetMessageRole::Assistant: RolePrefix = TEXT("[ASSISTANT]"); break;
		case EAgentZetMessageRole::System: RolePrefix = TEXT("[SYSTEM]"); break;
		case EAgentZetMessageRole::ToolResult: RolePrefix = FString::Printf(TEXT("[TOOL_RESULT:%s]"), *Msg.ToolUseId); break;
		case EAgentZetMessageRole::Error: RolePrefix = TEXT("[ERROR]"); break;
