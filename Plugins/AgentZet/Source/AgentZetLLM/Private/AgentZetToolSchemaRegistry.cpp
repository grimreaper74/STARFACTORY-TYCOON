...
FAgentZetToolSchemaRegistry::FAgentZetToolSchemaRegistry()
{
}
...
FString FAgentZetToolSchemaRegistry::GetSchemasDirectory() const
{
	...
	{
		PluginBaseDir = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("AgentZet"));
...
void FAgentZetToolSchemaRegistry::LoadAllSchemas()
{
	FString SchemasDir = GetSchemasDirectory();

	if (!FPaths::DirectoryExists(SchemasDir))
	{
		UE_LOG(LogAgentZet, Warning, TEXT("ToolSchemaRegistry: Schemas directory not found: %s"), *SchemasDir);
		return;
	}
	...
	UE_LOG(LogAgentZet, Log, TEXT("ToolSchemaRegistry: Loaded %d tool schemas from %d files."),
		ToolSchemas.Num(), SchemaFiles.Num());
}

bool FAgentZetToolSchemaRegistry::LoadSchemaFile(const FString& FilePath)
{
	FString FileContent;
	if (!FFileHelper::LoadFileToString(FileContent, *FilePath))
	{
		UE_LOG(LogAgentZet, Error, TEXT("ToolSchemaRegistry: Failed to read file: %s"), *FilePath);
		return false;
	}
	...
	if (FJsonSerializer::Deserialize(Reader, RootObj) && RootObj.IsValid())
	{
		...
		if (RootObj->TryGetStringField(TEXT("schema_version"), SchemaVersion))
		{
			...
			if (RootObj->TryGetArrayField(TEXT("tools"), ToolsArray))
			{
				...
				UE_LOG(LogAgentZet, Verbose, TEXT("ToolSchemaRegistry: Loaded %s v%s (%d tools)"),
	...
	UE_LOG(LogAgentZet, Error, TEXT("ToolSchemaRegistry: Failed to parse schema file: %s"), *FilePath);
	return false;
}
...
TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetAllSchemas() const
{
...
TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetSchemasByCategory(const FString& Category) const
{
	TArray<TSharedPtr<FJsonObject>> Result;
	for (const auto& Pair : ToolSchemas)
	{
...
TSharedPtr<FJsonObject> FAgentZetToolSchemaRegistry::GetSchemaByName(const FString& ToolName) const
{
	const TSharedPtr<FJsonObject>* Found = ToolSchemas.Find(ToolName);
	return Found ? *Found : nullptr;
}

bool FAgentZetToolSchemaRegistry::IsToolRegistered(const FString& ToolName) const
{
	return ToolSchemas.Contains(ToolName);
}

TArray<FString> FAgentZetToolSchemaRegistry::GetAllToolNames() const
{
...
void FAgentZetToolSchemaRegistry::SetToolEnabled(const FString& ToolName, bool bEnabled)
{
	if (bEnabled)
	{
...
bool FAgentZetToolSchemaRegistry::IsToolEnabled(const FString& ToolName) const
{
	return !DisabledTools.Contains(ToolName);
}

TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetEnabledSchemas() const
{
	TArray<TSharedPtr<FJsonObject>> Result;
	for (const auto& Pair : ToolSchemas)
	{
...
void FAgentZetToolSchemaRegistry::SyncWithRegisteredTools(const TArray<FString>& RegisteredToolNames)
{
	...
	if (DisabledCount > 0)
	{
		UE_LOG(LogAgentZet, Log, TEXT("ToolSchemaRegistry: SyncWithRegisteredTools disabled %d schemas with no registered executor."), DisabledCount);
	}
}
...
const TArray<FString>& FAgentZetToolSchemaRegistry::GetAlwaysAvailableTools()
{
...
TArray<FString> FAgentZetToolSchemaRegistry::GetAllowedToolNamesForMode(EAgentZetAgentMode Mode)
{
	// These are substring/prefix patterns — a tool is allowed if its name CONTAINS one of these
	switch (Mode)
	{
	case EAgentZetAgentMode::Orchestrator:
	...
	case EAgentZetAgentMode::General:
		// All tools
		return {};  // Empty = allow all

	case EAgentZetAgentMode::Blueprint:
		return {
			TEXT("blueprint"),
			TEXT("read_file"),
			TEXT("get_file"),
	...
	case EAgentZetAgentMode::CppCode:
		return {
			TEXT("read_file"),
			TEXT("get_file"),
			TEXT("list_files"),
	...
	case EAgentZetAgentMode::Architect:
...
TSet<FString> FAgentZetToolSchemaRegistry::GetAllowedCategoriesForMode(EAgentZetAgentMode Mode) const
{
	switch (Mode)
	{
	case EAgentZetAgentMode::General:
...
TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetSchemasForMode(EAgentZetAgentMode Mode) const
{
	// General mode = all enabled schemas
	if (Mode == EAgentZetAgentMode::General)
	{
...
FString FAgentZetToolSchemaRegistry::GetModeRoleDefinition(EAgentZetAgentMode Mode)
{
	switch (Mode)
	{
	case EAgentZetAgentMode::General:
		return TEXT(
			"You are AgentZet, an AI assistant for Unreal Engine development. "
	...
	case EAgentZetAgentMode::Blueprint:
		return TEXT(
			"You are AgentZet in Blueprint mode. Your role is to design and implement Blueprint logic. "
	...
	case EAgentZetAgentMode::CppCode:
		return TEXT(
			"You are AgentZet in C++ Code mode. Your role is to write, modify, and compile Unreal Engine C++ code. "
	...
	case EAgentZetAgentMode::Architect:
		return TEXT(
			"You are AgentZet in Architect mode. Your role is to analyze, plan, and explain — NOT to make changes. "
	...
	case EAgentZetAgentMode::Debug:
		return TEXT(
			"You are AgentZet in Debug mode. Your role is to diagnose and fix issues. "
	...
	case EAgentZetAgentMode::Asset:
		return TEXT(
			"You are AgentZet in Asset mode. Your role is to work with Unreal Engine assets: "
	...
	case EAgentZetAgentMode::Orchestrator:
		return TEXT(
			"You are AgentZet in Orchestrator mode. Your role is to coordinate complex, "
	...
	return TEXT("You are AgentZet, an AI assistant for Unreal Engine development.");
}

FString FAgentZetToolSchemaRegistry::GetModeDisplayName(EAgentZetAgentMode Mode)
{
	switch (Mode)
	{
	case EAgentZetAgentMode::General:      return TEXT("General");
	case EAgentZetAgentMode::Blueprint:    return TEXT("Blueprint");
	case EAgentZetAgentMode::CppCode:      return TEXT("C++ Code");
	case EAgentZetAgentMode::Architect:    return TEXT("Architect");
	case EAgentZetAgentMode::Debug:        return TEXT("Debug");
	case EAgentZetAgentMode::Asset:        return TEXT("Asset");
	case EAgentZetAgentMode::Orchestrator: return TEXT("Orchestrator");
	}
	return TEXT("General");
}

TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetEssentialSchemas() const
{
	// =========================================================================
	...
	UE_LOG(LogAgentZet, Log, TEXT("ToolSchemaRegistry: GetEssentialSchemas() returned %d tools (from %d total)."),
...
TArray<TSharedPtr<FJsonObject>> FAgentZetToolSchemaRegistry::GetTier1Schemas() const
{
	...
	UE_LOG(LogAgentZet, Log, TEXT("ToolSchemaRegistry: GetTier1Schemas() returned %d tools (from %d total). ~%d tokens saved vs full set."),
		Result.Num(), ToolSchemas.Num(), (ToolSchemas.Num() - Result.Num()) * 60);

	return Result;
}

FString FAgentZetToolSchemaRegistry::GetToolInfoString(const FString& ToolName) const
{
...
FString FAgentZetToolSchemaRegistry::GetModeWhenToUse(EAgentZetAgentMode Mode)
{
	switch (Mode)
	{
	case EAgentZetAgentMode::General:
