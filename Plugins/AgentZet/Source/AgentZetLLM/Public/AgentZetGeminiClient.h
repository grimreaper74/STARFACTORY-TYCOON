 ...
 *   - Tool calling via functionDeclarations (maps to AgentZet tool schemas)
 ...
 */
class AGENTZETLLM_API FAgentZetGeminiClient : public IAgentZetLLMClient
{
public:
	FAgentZetGeminiClient();
	virtual ~FAgentZetGeminiClient();
	...
	void SetReasoningEffort(EAgentZetReasoningEffort InEffort); // Gemini 3.x
...
private:
	...
	TArray<TSharedPtr<FJsonValue>> ConvertContents(const TArray<FAgentZetMessage>& Messages) const;
	...
	static FString ReasoningEffortToGeminiString(EAgentZetReasoningEffort Effort);
