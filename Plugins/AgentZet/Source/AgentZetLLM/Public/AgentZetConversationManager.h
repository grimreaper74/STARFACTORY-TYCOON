 ...
 */
class AGENTZETLLM_API FAgentZetConversationManager
{
public:
	FAgentZetConversationManager();
	~FAgentZetConversationManager();

	/** Add a user message to the conversation */
	FAgentZetMessage& AddUserMessage(const FString& Content);

	/** Add an assistant message to the conversation (text-only, no content blocks) */
	FAgentZetMessage& AddAssistantMessage(const FString& Content);
	...
	FAgentZetMessage& AddAssistantMessageFull(const FAgentZetMessage& SourceMessage);

	/** Add a system message to the conversation */
	FAgentZetMessage& AddSystemMessage(const FString& Content);

	/** Add a tool result message to the conversation */
	FAgentZetMessage& AddToolResultMessage(const FString& ToolUseId, const FString& Content, bool bIsError = false);
	...
	const TArray<FAgentZetMessage>& GetHistory() const { return History; }
	 ...
	 */
	TArray<FAgentZetMessage> GetEffectiveHistory() const;
	...
	TArray<FAgentZetMessage> GetPrunedHistory(int32 MaxTokenBudget) const;
	...
	FAgentZetMessage* GetMessageById(const FGuid& MessageId);

	/** Get the last message in the conversation */
	FAgentZetMessage* GetLastMessage();
	 ...
	 *  Returns: {ProjectSavedDir}/AgentZet/Tasks/ */
	static FString GetTasksBaseDir();

	/** Delegate: fired when a new message is added */
	FOnAgentZetMessageAdded OnMessageAdded;
...
private:
	...
	static int32 EstimateMessageTokens(const FAgentZetMessage& Message);
	 ...
	 */
	static void EvictOldToolResults(TArray<FAgentZetMessage>& Messages);

	/** Serialize a message to JSON */
	static TSharedPtr<FJsonObject> MessageToJson(const FAgentZetMessage& Message);

	/** Deserialize a message from JSON */
	static FAgentZetMessage MessageFromJson(const TSharedPtr<FJsonObject>& JsonObj);
