 ...
 */
struct AGENTZETENGINE_API FAgentZetTaskHistoryItem
{
	...
	FAgentZetTokenUsage TotalTokenUsage;
 ...
 * Provides a browsable history of all past AgentZet sessions.
 *
 * STORAGE:
 *   Saved/AgentZet/TaskHistory/task_history.json
 ...
 */
class AGENTZETENGINE_API FAgentZetTaskHistory
{
public:
	FAgentZetTaskHistory();
	~FAgentZetTaskHistory();
	...
	void RecordTask(const FAgentZetTaskHistoryItem& Item);

	/** Update an existing task's metadata */
	void UpdateTask(const FString& TabId, const FAgentZetTaskHistoryItem& UpdatedItem);
	...
	TArray<FAgentZetTaskHistoryItem> GetHistory() const;

	/** Get a specific task by ID */
	const FAgentZetTaskHistoryItem* GetTask(const FString& TabId) const;
...
private:
	...
	TArray<FAgentZetTaskHistoryItem> HistoryItems;
