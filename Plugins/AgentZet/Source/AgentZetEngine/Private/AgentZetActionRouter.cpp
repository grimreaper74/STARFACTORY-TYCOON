...
FAgentZetActionRouter::FAgentZetActionRouter() {}
FAgentZetActionRouter::~FAgentZetActionRouter() {}

void FAgentZetActionRouter::RegisterExecutor(TSharedRef<IAgentZetActionExecutor> Executor)
{
	for (const FString& ToolName : Executor->GetSupportedToolNames())
	{
		ExecutorMap.Add(ToolName, Executor);
		UE_LOG(LogAgentZet, Log, TEXT("ActionRouter: Registered executor for tool '%s'"), *ToolName);
	}
}

void FAgentZetActionRouter::ClearExecutors()
{
	ExecutorMap.Empty();
}
...
FAgentZetActionResult FAgentZetActionRouter::RouteToolCall(const FAgentZetToolCall& ToolCall)
{
	TSharedPtr<IAgentZetActionExecutor> Executor = FindExecutorForTool(ToolCall.ToolName);
	if (!Executor.IsValid())
	{
	...
	// paths, which freezes the editor. We catch this early and return a clean error.
	{
		...
		if (!ValidateAssetPathParam(ToolCall.InputParams, ValidationError))
		{
			...
			UE_LOG(LogAgentZet, Warning, TEXT("ActionRouter: Blocked tool '%s' — invalid asset_path: %s"),
...
FAgentZetActionPlan FAgentZetActionRouter::PreviewToolCall(const FAgentZetToolCall& ToolCall)
{
	TSharedPtr<IAgentZetActionExecutor> Executor = FindExecutorForTool(ToolCall.ToolName);
	if (!Executor.IsValid())
	{
...
TArray<FName> FAgentZetActionRouter::GetRegisteredExecutorNames() const
{
	TArray<FName> Names;
...
TArray<FString> FAgentZetActionRouter::GetRegisteredToolNames() const
{
...
TSharedPtr<IAgentZetActionExecutor> FAgentZetActionRouter::FindExecutorForTool(const FString& ToolName) const
{
	const TSharedRef<IAgentZetActionExecutor>* Found = ExecutorMap.Find(ToolName);
	return Found ? TSharedPtr<IAgentZetActionExecutor>(*Found) : nullptr;
}
