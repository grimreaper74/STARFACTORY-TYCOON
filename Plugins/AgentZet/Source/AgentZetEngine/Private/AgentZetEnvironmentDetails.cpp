...
FAgentZetEnvironmentDetails::FAgentZetEnvironmentDetails()
{
}
...
void FAgentZetEnvironmentDetails::SetFileContextTracker(FAgentZetFileContextTracker* InTracker)
{
	FileContextTracker = InTracker;
}

void FAgentZetEnvironmentDetails::SetIgnoreController(FAgentZetIgnoreController* InIgnoreController)
{
	IgnoreController = InIgnoreController;
}

FString FAgentZetEnvironmentDetails::Build(
...
FString FAgentZetEnvironmentDetails::GetOpenFilesSection() const
{
...
FString FAgentZetEnvironmentDetails::GetActiveLevelSection() const
{
...
FString FAgentZetEnvironmentDetails::GetSelectedActorsSection() const
{
...
FString FAgentZetEnvironmentDetails::GetCompileErrorsSection() const
{
...
FString FAgentZetEnvironmentDetails::GetContextWindowSection(float UsagePercent) const
{
	FString Status;
	FString Advice;
...
FString FAgentZetEnvironmentDetails::GetTodoSummarySection(const TArray<FAgentZetTodoItem>& Todos) const
{
	if (Todos.Num() == 0)
	{
		return FString();
	...
	int32 Completed = 0;
	for (const FAgentZetTodoItem& Todo : Todos)
	{
		if (Todo.Status == EAgentZetTodoStatus::Completed) Completed++;
	}
	...
	for (const FAgentZetTodoItem& Todo : Todos)
	{
		...
		switch (Todo.Status)
		{
		case EAgentZetTodoStatus::Completed:  Marker = TEXT("[x]"); break;
		case EAgentZetTodoStatus::InProgress: Marker = TEXT("[-]"); break;
...
FString FAgentZetEnvironmentDetails::GetStaleFilesSection() const
{
	...
	return const_cast<FAgentZetFileContextTracker*>(FileContextTracker)->BuildStaleContextWarning();
}

FString FAgentZetEnvironmentDetails::GetSessionInfoSection(const FString& TabTitle, int32 LoopIteration) const
{
...
bool FAgentZetEnvironmentDetails::IsPathAllowed(const FString& Path) const
{
	if (!IgnoreController)
	{
		return true;
...
TArray<FString> FAgentZetEnvironmentDetails::GatherCompileErrors() const
{
...
TArray<FString> FAgentZetEnvironmentDetails::GatherOpenFiles() const
{
...
TArray<FString> FAgentZetEnvironmentDetails::GatherSelectedActors() const
{
...
FString FAgentZetEnvironmentDetails::GetActiveLevelName() const
{
#if WITH_EDITOR
	if (GEditor && GEditor->GetEditorWorldContext().World())
	{
