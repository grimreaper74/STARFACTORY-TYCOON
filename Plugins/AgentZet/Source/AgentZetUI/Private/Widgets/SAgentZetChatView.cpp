...
void SAgentZetChatView::Construct(const FArguments& InArgs)
{
...
void SAgentZetChatView::AddMessage(const FAgentZetMessage& Message)
{
...
void SAgentZetChatView::UpdateStreamingMessage(const FGuid& MessageId, const FString& DeltaText, EAgentZetMessageRole Role)
{
...
void SAgentZetChatView::FlushStreamingBuffer()
{
	...
	// Find the last SAgentZetMessage child widget and append text to it
	...
	if (Children && Children->Num() > 0)
	{
		...
		TSharedPtr<SAgentZetMessage> MsgWidget = StaticCastSharedRef<SAgentZetMessage>(LastChild);
		if (MsgWidget.IsValid())
		{
			if (StreamingTargetRole == EAgentZetMessageRole::None || StreamingTargetRole == MsgWidget->GetMessageData().Role)
			{
			...
			else
			{
				...
				FAgentZetMessage NewMsg(StreamingTargetRole, TextToFlush);
				AddMessage(NewMsg);
			}
		}
	}
...
void SAgentZetChatView::MarkLastToolCompleted(bool bSuccess)
{
	...
	for (int32 i = Children->Num() - 1; i >= 0; --i)
	{
		...
		TSharedPtr<SAgentZetMessage> MsgWidget = StaticCastSharedRef<SAgentZetMessage>(Child);
...
void SAgentZetChatView::ClearMessages()
{
	// Flush any pending streaming text first
	FlushStreamingBuffer();
	StreamingTargetId.Invalidate();
	StreamingTargetRole = EAgentZetMessageRole::None;
...
void SAgentZetChatView::ScrollToBottom()
{
...
bool SAgentZetChatView::IsScrolledToBottom() const
{
...
void SAgentZetChatView::ShowResumptionBar(const FString& TimeAgoText)
{
...
void SAgentZetChatView::HideResumptionBar()
{
