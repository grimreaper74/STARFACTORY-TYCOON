...
#include "Widgets/SAgentZetMessage.h"
...
bool SAgentZetMessage::DetectToolCallFromContent(const FString& Content)
{
...
FString SAgentZetMessage::ExtractToolNameFromContent(const FString& Content)
{
...
void SAgentZetMessage::Construct(const FArguments& InArgs)
{
	...
	{
	case EAgentZetMessageRole::User:      RoleLabel = TEXT("You"); RoleColor = FLinearColor(0.3f, 0.6f, 1.0f); break;
	case EAgentZetMessageRole::Assistant: RoleLabel = TEXT("AgentZet"); RoleColor = FLinearColor(0.2f, 0.9f, 0.4f); break;
	case EAgentZetMessageRole::System:    RoleLabel = TEXT("System"); RoleColor = FLinearColor(0.7f, 0.7f, 0.7f); break;
	case EAgentZetMessageRole::ToolResult:RoleLabel = TEXT("Tool"); RoleColor = FLinearColor(1.0f, 0.8f, 0.2f); break;
	case EAgentZetMessageRole::Error:     RoleLabel = TEXT("Error"); RoleColor = FLinearColor(1.0f, 0.2f, 0.2f); break;
	default: break;
	}
	...
	bool bIsError = (MessageData.Role == EAgentZetMessageRole::Error);
	...
	else if (bIsCollapsible)
	{
		...
		BodyWidget = SNew(SExpandableArea)
			...
			[
				SNew(STextBlock)
				.Text(this, &SAgentZetMessage::GetHeaderText)
				.ColorAndOpacity(FSlateColor(FLinearColor(0.6f, 0.6f, 0.6f)))
			]
			.BodyContent()
			[
...
void SAgentZetMessage::AppendText(const FString& DeltaText)
{
...
void SAgentZetMessage::MarkToolCompleted(bool bSuccess)
{
	bToolCompleted = true;
...
bool SAgentZetMessage::IsToolCallMessage() const
{
	...
	if (MessageData.Role == EAgentZetMessageRole::Assistant && DetectToolCallFromContent(MessageData.Content))
	{
		return true;
	}
	return false;
...
FLinearColor SAgentZetMessage::GetBorderColor() const
{
	switch (MessageData.Role)
	{
	...
	case EAgentZetMessageRole::Error:
		return FLinearColor(1.0f, 0.2f, 0.2f); // red

	case EAgentZetMessageRole::System:
...
FText SAgentZetMessage::GetHeaderText() const
{
	FString FirstLine;
	int32 NewlineIndex;
	if (MessageData.Content.FindChar('\n', NewlineIndex))
