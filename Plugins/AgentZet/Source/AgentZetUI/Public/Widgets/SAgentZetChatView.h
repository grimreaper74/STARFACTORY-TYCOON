 ...
 */
class AGENTZETUI_API SAgentZetChatView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SAgentZetChatView) {}
	...
	void AddMessage(const FAgentZetMessage& Message);

	/** Update a streaming message with new delta text */
	void UpdateStreamingMessage(const FGuid& MessageId, const FString& DeltaText, EAgentZetMessageRole Role = EAgentZetMessageRole::None);
...
private:
	...
	TOptional<EAgentZetMessageRole> LastMessageRole;
	...
	EAgentZetMessageRole StreamingTargetRole = EAgentZetMessageRole::None;
