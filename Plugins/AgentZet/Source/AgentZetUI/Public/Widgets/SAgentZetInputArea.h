...
#include "AgentZetTypes.h"  // for EConversationState

DECLARE_DELEGATE_OneParam(FOnAgentZetPromptSubmitted, const FString& /*PromptText*/);
DECLARE_DELEGATE(FOnAgentZetStopRequested);
 ...
 */
class AGENTZETUI_API SAgentZetInputArea : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SAgentZetInputArea) {}
		SLATE_EVENT(FOnAgentZetPromptSubmitted, OnPromptSubmitted)
		SLATE_EVENT(FOnAgentZetStopRequested, OnStopRequested)
...
private:
	...
	// ---- State ----
	FOnAgentZetPromptSubmitted OnPromptSubmitted;
	FOnAgentZetStopRequested OnStopRequested;
