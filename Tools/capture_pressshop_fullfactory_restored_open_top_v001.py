"""Read-only open-top capture of the restored Press Shop; map is never saved."""
import hashlib
import json
...
OUT = ROOT / "01_restored_full_factory_open_top.png"
RECEIPT = ROOT / "01_restored_full_factory_open_top_receipt.json"
PROTECTED = {
...
def static_mesh_path(actor):
    try:
        ...
        return mesh.get_path_name().lower() if mesh else ""
    except Exception:
        return ""
...
def finish(status, **extra):
    ...
    RECEIPT.write_text(json.dumps({
        ...
        "render_path": str(OUT), "map_saved": False,
        "hidden_open_top_actors": HIDDEN,
        "protected_sha256_after": {str(path): digest(path) for path in PROTECTED}, **extra,
...
try:
    if OUT.exists() or RECEIPT.exists():
        raise RuntimeError("refusing to overwrite open-top proof")
    ...
    hide_terms = ("roof", "ceiling", "skylight panel", "roofliner", "roof liner")
    for actor in actors:
        signature = (actor.get_actor_label() + " " + static_mesh_path(actor)).lower()
        if any(term in signature for term in hide_terms) and not isinstance(actor, unreal.Light):
