...
import os
tag = os.environ.get("LOD_TAG", "LOD1")
target = {"LOD1": 40000, "LOD2": 8000}[tag]
dec = car.modifiers.new("Dec", "DECIMATE")
dec.ratio = target / max(len(car.data.polygons), 1)
bpy.context.view_layer.objects.active = car
bpy.ops.object.modifier_apply(modifier="Dec")
name = "SM_LB_DevCar_Concept{}_v003".format(tag)
car.name = name
print("TRIS", tag, len(car.data.polygons))
