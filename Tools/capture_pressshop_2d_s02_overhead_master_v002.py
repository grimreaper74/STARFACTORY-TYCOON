"""Capture the locked high-overhead S02 master in its isolated v003 candidate."""
import hashlib
import json
...
ROOT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots" / "PressShop2126" / "Factorio2p5DFull_v003OverheadSprites"
OUT = ROOT / "01_s02_overhead_master_overview.exr"
...
try:
    if OUT.exists() or RECEIPT.exists():
        raise RuntimeError("refusing to overwrite v003 overhead capture evidence")
    ...
    if not unreal.EditorLoadingAndSavingUtils.load_map(MAP):
        raise RuntimeError("could not load v003 overhead candidate")
    ...
    if not isinstance(camera, unreal.CameraActor) or not isinstance(sprite, unreal.StaticMeshActor):
        raise RuntimeError("overview camera or locked S02 overhead sprite unavailable")
    ...
    def tick(_delta):
        ...
        if elapsed >= 10.0 and not EXPORTED:
            ...
            try:
                ...
                finish("FAIL__S02_OVERHEAD_MASTER_NATIVE_CAPTURE", error=repr(exc), elapsed_seconds=round(elapsed, 2))
            ...
            if size != LAST_SIZE:
                ...
                if after != protected_before:
                    finish("FAIL__PROTECTED_MAP_CHANGED_DURING_S02_OVERHEAD_CAPTURE", protected_sha256_before=protected_before, protected_sha256_after=after)
                else:
                    finish("PASS__S02_OVERHEAD_MASTER_NATIVE_CAPTURE", bytes=size, sha256=digest(OUT), elapsed_seconds=round(elapsed, 2), protected_sha256_before=protected_before)
        elif elapsed >= 70.0:
            finish("FAIL__S02_OVERHEAD_MASTER_NATIVE_CAPTURE", error="SceneCapture2D did not export a valid EXR", elapsed_seconds=round(elapsed, 2))
    HANDLE = unreal.register_slate_post_tick_callback(tick)
except Exception as exc:
    finish("FAIL__S02_OVERHEAD_MASTER_NATIVE_CAPTURE", error=repr(exc))
