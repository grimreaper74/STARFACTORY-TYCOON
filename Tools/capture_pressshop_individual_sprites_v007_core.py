"""Capture the readable S02-S06 core of the v007 Press Shop candidate."""
import hashlib
import json
...
OUT = ROOT / "03_press_core_cleaned_management_overview.png"
RECEIPT = ROOT / "capture_press_core_cleaned_management_overview_receipt.json"
PROTECTED_MAPS = {
...
def finish(status, **extra):
    ...
    RECEIPT.write_text(json.dumps({"status": status, "map": MAP, "render_path": str(OUT), "no_map_save": True, "protected_sha256_after": {str(path): digest(path) for path in PROTECTED_MAPS}, **extra}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
try:
    if OUT.exists() or RECEIPT.exists():
        raise RuntimeError("refusing to overwrite core overview proof")
    for path, expected in PROTECTED_MAPS.items():
        if not path.is_file() or digest(path) != expected:
            raise RuntimeError("protected map missing or changed")
    ...
    if not isinstance(camera, unreal.CameraActor):
        raise RuntimeError("locked camera unavailable")
    rotation = camera.get_actor_rotation()
    if abs(rotation.pitch + 60.0) > 0.2:
        raise RuntimeError("camera pitch is not -60 degrees")
    forward = unreal.MathLibrary.get_forward_vector(rotation)
    target = unreal.Vector(400.0, 0.0, 380.0)
    location = unreal.Vector(target.x - forward.x * 20000.0, target.y - forward.y * 20000.0, target.z - forward.z * 20000.0)
    ...
    def tick(_delta):
        ...
        if elapsed >= 12.0 and not EXPORTED:
            ...
            try:
                ...
                finish("FAIL__PRESS_CORE_MANAGEMENT_CAPTURE", error=repr(exc), elapsed_seconds=round(elapsed, 2))
            ...
            if size != LAST_SIZE:
                ...
                if after != before:
                    finish("FAIL__PROTECTED_MAP_CHANGED_DURING_CORE_CAPTURE", protected_sha256_before=before, protected_sha256_after=after)
                else:
                    finish("PASS__PRESS_CORE_MANAGEMENT_CAPTURE", bytes=size, sha256=digest(OUT), elapsed_seconds=round(elapsed, 2), protected_sha256_before=before, camera_rotation=[round(rotation.pitch, 3), round(rotation.yaw, 3), round(rotation.roll, 3)], capture_location_cm=[round(location.x, 3), round(location.y, 3), round(location.z, 3)], target_cm=[target.x, target.y, target.z], orthographic_width_cm=8500.0)
        elif elapsed >= 80.0:
            finish("FAIL__PRESS_CORE_MANAGEMENT_CAPTURE", error="SceneCapture2D did not export a valid PNG", elapsed_seconds=round(elapsed, 2))
    HANDLE = unreal.register_slate_post_tick_callback(tick)
except Exception as exc:
    finish("FAIL__PRESS_CORE_MANAGEMENT_CAPTURE", error=repr(exc))
