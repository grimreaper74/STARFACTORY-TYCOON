"""Close the outbound camera onto its real vision gate, robot and stillage."""
...
REPORT = PROJECT / "Saved" / "Audits" / "PressShopIntegration" / "pressshop_2126_outbound_camera_v023.json"
TAG = unreal.Name("LB.PressShop.2126.OutboundCamera.v023")
...
if any(TAG in actor.tags for actor in actors.values()):
    raise RuntimeError("Outbound camera v023 already applied")
...
for label in ("OUTBOUND | real vision inspection gate", "OUTBOUND | inspected panel stillage", "ROBOT | S06 | vision stack robot"):
    if label not in actors:
        raise RuntimeError("Expected outbound equipment missing: " + label)

location = unreal.Vector(14500.0, -5000.0, 1100.0)
target = unreal.Vector(9700.0, 0.0, 650.0)
...
REPORT.write_text(json.dumps({
    "status": "PASS__OUTBOUND_CAMERA_CLOSED_ON_EXISTING_AUTOMATION",
    "camera": {"location_cm": [14500, -5000, 1100], "target_cm": [9700, 0, 650], "focal_length_mm": 52.0},
    "geometry_changed": False,
    "protected_v438_sha256_before": protected_before,
    "protected_v438_sha256_after": protected_after,
}, indent=2), encoding="utf-8")
unreal.log("PRESSSHOP_2126_OUTBOUND_CAMERA_V023_PASS")
