"""Import and mount the 2126 AI inspection/metrology cell beside the final press."""
import hashlib
import json
import math
...
SOURCE = PROJECT / "SourceArt" / "PressShop2126" / "Sprites" / "T_CA_MW_2126_AIInspectionMetrologyCell_v001.png"
TEXTURE_NAME = "T_CA_MW_2126_AIInspectionMetrologyCell_v001"
MATERIAL_NAME = "M_CA_MW_2126_AIInspectionMetrologyCell_UnlitMasked_v001"
ACTOR_LABEL = "2126 OUTBOUND | AI inspection and metrology cell"
RECEIPT = PROJECT / "Saved" / "Audits" / "PressShop2126" / "import_mount_ai_inspection_cell_v001_receipt.json"
PROTECTED = {
...
if not SOURCE.is_file():
    raise RuntimeError("inspection source sprite missing")
...
if unreal.EditorAssetLibrary.does_asset_exist(texture_path) or unreal.EditorAssetLibrary.does_asset_exist(material_path):
    raise RuntimeError("refusing to overwrite existing inspection assets")
...
if len(paths) != 1:
    raise RuntimeError(f"inspection texture import failed: {paths}")
texture = unreal.load_asset(paths[0])
if not isinstance(texture, unreal.Texture2D):
    raise RuntimeError("inspection PNG did not import as Texture2D")
...
if not isinstance(material, unreal.Material):
    raise RuntimeError("could not create inspection material")
...
if not mel.connect_material_property(sample, "RGB", unreal.MaterialProperty.MP_EMISSIVE_COLOR):
    raise RuntimeError("inspection colour connection failed")
if not mel.connect_material_property(sample, "A", unreal.MaterialProperty.MP_OPACITY_MASK):
    raise RuntimeError("inspection alpha connection failed")
...
if any(a.get_actor_label() == ACTOR_LABEL for a in actors):
    raise RuntimeError("inspection cell already mounted")
...
anchor = unreal.Vector(-550.0, 4550.0, 480.0)
...
card.set_actor_scale3d(unreal.Vector(27.0, 18.0, 1.0))
if dot(unreal.MathLibrary.get_up_vector(card_rotation), camera_forward) < 0.999:
    raise RuntimeError("inspection sprite does not face fixed camera")

if not unreal.EditorLoadingAndSavingUtils.save_current_level():
    raise RuntimeError("inspection integration did not save")
unreal.EditorLoadingAndSavingUtils.save_dirty_packages(True, True)
after = {str(path): digest(path) for path in PROTECTED}
if after != before:
    raise RuntimeError("protected maps changed during inspection integration")

RECEIPT.parent.mkdir(parents=True, exist_ok=True)
RECEIPT.write_text(json.dumps({
    "status": "PASS__2126_AI_INSPECTION_CELL_MOUNTED",
    "map": MAP,
    ...
    "actor_scale": [27.0, 18.0, 1.0],
    "flow_axis": "+X from final press bay toward outbound",
    "protected_sha256_before": before,
    "protected_sha256_after": after,
}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
unreal.log(f"PRESSSHOP_2126_AI_INSPECTION_MOUNT_PASS receipt={RECEIPT}")
