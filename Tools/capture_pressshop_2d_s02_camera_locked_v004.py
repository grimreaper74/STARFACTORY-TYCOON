"""Capture the v004 camera-locked overhead S02 without saving the map."""
import hashlib
import json
...
ROOT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots" / "PressShop2126" / "Factorio2p5DFull_v004CameraLockedSprites"
OUT = ROOT / "04_s02_camera_locked_whitekey093_rgba8.png"
RECEIPT = ROOT / "capture_whitekey093_rgba8_receipt.json"
PROTECTED = {
...
def finish(status, **extra):
    ...
    RECEIPT.write_text(json.dumps({
        "status": status, "map": MAP, "camera": CAMERA_LABEL,
        "render_path": str(OUT), "capture_method": "native_scene_capture_2d_transient",
        "no_map_save": True,
...
try:
    if OUT.exists() or RECEIPT.exists():
        raise RuntimeError("refusing to overwrite v004 capture evidence")
    ...
    if not unreal.EditorLoadingAndSavingUtils.load_map(MAP):
        raise RuntimeError("could not load v004 camera-locked candidate")
    ...
    # CanvasRenderTarget2D defaults to FloatRGBA, which UE intentionally writes
    # only as EXR. Request an 8-bit sRGB target before first scene capture so the
    # review frame is an engine-authored PNG, not a post-processed conversion.
    ...
    def tick(_delta):
        ...
        if elapsed >= 10.0 and not EXPORTED:
            ...
            try:
                ...
                finish("FAIL__S02_CAMERA_LOCKED_NATIVE_CAPTURE", error=repr(exc), elapsed_seconds=round(elapsed, 2))
            ...
            if size != LAST_SIZE:
                ...
                if after != protected_before:
                    finish("FAIL__PROTECTED_MAP_CHANGED_DURING_S02_CAMERA_LOCKED_CAPTURE", protected_sha256_before=protected_before, protected_sha256_after=after)
                else:
                    finish("PASS__S02_CAMERA_LOCKED_NATIVE_CAPTURE", bytes=size, sha256=digest(OUT), elapsed_seconds=round(elapsed, 2), protected_sha256_before=protected_before)
        elif elapsed >= 70.0:
            finish("FAIL__S02_CAMERA_LOCKED_NATIVE_CAPTURE", error="SceneCapture2D did not export a valid EXR", elapsed_seconds=round(elapsed, 2))
    HANDLE = unreal.register_slate_post_tick_callback(tick)
except Exception as exc:
    finish("FAIL__S02_CAMERA_LOCKED_NATIVE_CAPTURE", error=repr(exc))
