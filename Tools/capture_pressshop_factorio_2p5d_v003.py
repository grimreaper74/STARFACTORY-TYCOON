"""Render the fixed-isometric 2.5D press-cell candidate without saving the map."""
import hashlib
import json
...
CAMERA_LABEL = "CAM | 2.5D fixed isometric press-cell proof"
ROOT = Path(unreal.Paths.project_saved_dir()) / "ValidationScreenshots" / "PressShop2126" / "Factorio2p5D_v005"
OUT = ROOT / "01_fixed_isometric_press_cell.exr"
...
def finish(status, **extra):
    ...
    for path, expected in EXPECTED.items():
        if digest(path) != expected:
            status = "FAIL__PROTECTED_MAP_CHANGED_DURING_2P5D_CAPTURE"
...
try:
    if OUT.exists() or RECEIPT.exists():
        raise RuntimeError("refusing to overwrite existing 2.5D capture evidence")
    if any(digest(path) != expected for path, expected in EXPECTED.items()):
        raise RuntimeError("protected baseline mismatch before capture")
    ROOT.mkdir(parents=True, exist_ok=True)
    ...
    if not isinstance(camera, unreal.CameraActor):
        raise RuntimeError("fixed 2.5D camera unavailable")
    ...
    if not isinstance(CAPTURE, unreal.SceneCapture2D):
        raise RuntimeError("could not create scene capture")
    ...
    def tick(_delta):
        ...
        if elapsed >= 8.0 and not EXPORTED:
            ...
            try:
                ...
                finish("FAIL__2P5D_NATIVE_SCENE_CAPTURE", error=repr(exc), elapsed_seconds=round(elapsed, 2))
            ...
            if size != LAST_SIZE:
                ...
                finish("PASS__2P5D_NATIVE_SCENE_CAPTURE", bytes=size, sha256=digest(OUT), elapsed_seconds=round(elapsed, 2))
        elif elapsed >= 60.0:
            finish("FAIL__2P5D_NATIVE_SCENE_CAPTURE", error="SceneCapture2D did not export a valid PNG", elapsed_seconds=round(elapsed, 2))

    HANDLE = unreal.register_slate_post_tick_callback(tick)
except Exception as exc:
    finish("FAIL__2P5D_NATIVE_SCENE_CAPTURE", error=repr(exc))
